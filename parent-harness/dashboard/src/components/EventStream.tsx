...
export function EventStream({ events }: EventStreamProps) {
  ...
  const filteredEvents = useMemo(() => {
    return events.filter(event => {
      const matchesSearch = searchQuery === '' ||
        ...
        (event.agentId?.toLowerCase().includes(searchQuery.toLowerCase()) ?? false)
  ...
  const isFiltered = searchQuery !== '' || severityFilter !== 'all' || typeFilter !== 'all'
      ...
      <div className="flex-1 overflow-y-auto space-y-1">
        {filteredEvents.map((event) => (
          ...
          >
            ...
            {event.severity && (
              <span className={`text-xs font-mono ${severityColors[event.severity] || 'text-gray-400'}`}>
