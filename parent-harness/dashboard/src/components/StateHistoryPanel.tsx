...
export function StateHistoryPanel({ taskId }: StateHistoryPanelProps) {
  ...
  if (history.length === 0) {
    return (
      <div className="text-gray-500 text-sm">No state history recorded</div>
    );
  }
      ...
      <div className="relative">
        ...
        {history.map((change) => (
          <div key={change.id} className="relative pl-8 pb-4">
            {/* Timeline dot */}
            <div
              className={`absolute left-1.5 w-3 h-3 rounded-full ${statusColors[change.to_status] || "bg-gray-500"}`}
            />
              ...
              <div className="flex items-center gap-2 text-xs">
                ...
                <span
                  className={`px-1.5 py-0.5 rounded text-xs font-medium ${
                    change.to_status === "completed"
                      ? "bg-green-900 text-green-300"
                      : change.to_status === "failed"
                        ? "bg-red-900 text-red-300"
                        : change.to_status === "in_progress"
