      ...
      {isOpen && (
        <div className="absolute top-full left-0 mt-2 w-64 bg-gray-800 rounded-lg shadow-xl border border-gray-700 z-50 p-4">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className="text-white font-mono">
                {budget.daily.tokens.total.toLocaleString()}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-400">Token Limit:</span>
              <span className="text-white font-mono">
                {budget.daily.limit_tokens.toLocaleString()}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-400">Est. Cost:</span>
              <span className="text-white font-mono">
                ${budget.daily.cost_usd.toFixed(2)}
              </span>
            </div>
          </div>
          ...
          {budget.budget_status !== "OK" && (
            <div
              className={`mt-3 p-2 rounded text-xs ${isDanger ? "bg-red-900/50 text-red-300" : "bg-yellow-900/50 text-yellow-300"}`}
            >
