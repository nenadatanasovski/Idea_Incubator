      ...
      if (ws.readyState !== WebSocket.OPEN) {
        ...
        try {
          const res = await fetch(
            `${API_BASE}/event-bus/events?since=${lastEventIdRef.current}&limit=50`,
          );
          if (res.ok) {
            ...
            if (data.events.length > 0) {
              setEvents((prev) => {
                const newEvents = data.events.filter(
                  (e: EventRecord) => !prev.some((p) => p.id === e.id),
                );
      ...
      <div className="p-4 h-[calc(100vh-4rem)] flex flex-col">
        ...
        <div className="flex gap-4 mb-4">
          ...
          <input
            ...
            value={filter.taskId}
            onChange={(e) =>
              setFilter((f) => ({ ...f, taskId: e.target.value }))
            }
            ...
            <tbody>
              {filteredEvents.map((event) => (
                ...
                >
                  ...
                  <td
                    className={`p-2 font-mono text-xs ${getTypeColor(event.type)}`}
                  >
                    <span className="mr-1">
...
function StatusCard({
  ...
  return (
    <div
      className={`p-3 rounded-lg border ${statusColors[status as keyof typeof statusColors] || statusColors.normal}`}
    >
      <div className="text-xs text-gray-400">{title}</div>
      <div className="text-xl font-bold">{value}</div>
      <div
        className={`text-xs ${status === "high" ? "text-red-400" : "text-green-400"}`}
      >
        {status}
      </div>
    </div>
