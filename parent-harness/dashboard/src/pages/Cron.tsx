...
const HARNESS_TASKS: ScheduledTask[] = [
  {
    ...
    description: 'Runs strategic planning cycle to analyze codebase and create improvement tasks',
  ...
  },
  {
    ...
    description: 'Monitors agent health, detects stuck agents, and triggers interventions',
...
export function Cron() {
  const [harnessTasks, setHarnessTasks] = useState<ScheduledTask[]>(HARNESS_TASKS)
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (configRes.ok) {
        ...
        setHarnessTasks(prev => prev.map(task => {
  ...
  async function toggleTask(taskId: string, enabled: boolean) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMessage({ type: 'success', text: `${task.name} ${enabled ? 'enabled' : 'disabled'}` })
      } else {
  ...
  async function triggerTask(taskId: string) {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:3333${endpoint}`, { method: 'POST' })
      if (res.ok) {
  ...
  if (loading) {
    return (
      <Layout>
        <div className="p-8 text-center text-gray-400">Loading scheduled tasks...</div>
      </Layout>
    )
  }
      ...
      <div className="p-6 max-w-6xl mx-auto">
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-700/50">
                  <th className="text-left p-4 text-sm font-medium text-gray-400">Task</th>
              ...
              <tbody>
                {harnessTasks.map((task) => (
                  <tr key={task.id} className="border-t border-gray-700/50">
                    ...
                    <td className="p-4 text-sm text-gray-300">{task.schedule}</td>
                    <td className="p-4 text-sm text-gray-400">{formatTime(task.lastRun)}</td>
                    <td className="p-4">
        ...
        {config && (
          <div className="bg-gray-800 rounded-lg p-4">
            <h2 className="text-lg font-semibold mb-4">📊 Current Schedule Settings</h2>
            ...
            <p className="text-xs text-gray-500 mt-4">
              Edit these values in the <a href="/config" className="text-blue-400 hover:underline">Config page</a>
            </p>
          </div>
        )}
      </div>
...
function ConfigCard({ label, value, icon }: { label: string; value: string | number; icon: string }) {
  return (
