...
export function Config() {
  ...
  async function saveConfig() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMessage({
          type: "error",
          text: err.details?.join(", ") || "Failed to save",
        });
      }
    } catch (err) {
  ...
  async function clearPlanCache() {
    try {
      const res = await fetch(`${API_BASE}/config/planning/clear-cache`, {
        method: "POST",
      });
      if (res.ok) {
  ...
  return (
    <Layout>
      <div className="p-6 max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">
              Manage limits, timeouts, budgets, and resilience settings
            </p>
          </div>
        ...
        {message && (
          <div
            className={`mb-4 p-3 rounded text-sm ${message.type === "success" ? "bg-green-900/50 text-green-300" : "bg-red-900/50 text-red-300"}`}
          >
            {message.text}
          </div>
        )}
        ...
        {stats && budget && (
          <div className="grid grid-cols-4 gap-4 mb-6">
            ...
            <StatCard
              ...
              color={
                budget.daily.percent_used > 80
                  ? "red"
                  : budget.daily.percent_used > 50
                    ? "yellow"
                    : "green"
              }
            />
            <StatCard
              ...
              color={
                budget.budget_status.includes("WARNING") ? "yellow" : "green"
              }
            />
          </div>
        )}
        ...
        {config && (
          <div className="grid grid-cols-3 gap-4">
            ...
            <ConfigSection
              title="🧠 Planning"
              description="Strategic planning agent settings"
            >
              ...
              <ConfigField label="Model">
                <select
                  ...
                  onChange={(e) =>
                    updateConfig("planning", "model", e.target.value)
                  }
                  className="w-full bg-gray-700 rounded px-3 py-2 text-sm"
                >
            ...
            <ConfigSection
              title="🤖 Agents"
              description="Build agent execution settings"
            >
              <ConfigField label="Default Model">
                <select
                  value={config.agents.model}
                  onChange={(e) =>
                    updateConfig("agents", "model", e.target.value)
                  }
                  className="w-full bg-gray-700 rounded px-3 py-2 text-sm"
                >
            ...
            <ConfigSection
              ...
              description="Order of models to try when rate-limited"
            >
              <FallbackChainEditor
                chain={
                  config.agents.model_fallback || ["opus", "sonnet", "haiku"]
                }
                onChange={(chain) =>
                  updateConfig("agents", "model_fallback", chain)
                }
              />
            </ConfigSection>
            ...
            <ConfigSection
              title="⚙️ Per-Type Limits"
              description="Max concurrent agents per type"
            >
              <PerTypeLimitsEditor
                limits={config.agents.max_per_type || {}}
                onChange={(limits) =>
                  updateConfig("agents", "max_per_type", limits)
                }
              />
            </ConfigSection>
            ...
            <ConfigSection
              title="💰 Budget"
              description="Token limits and cost controls"
            >
            ...
            <ConfigSection
              title="🔁 Retry"
              description="Failed task retry behavior"
            >
            ...
            <ConfigSection
              ...
              description="Auto-pause failing agent types"
            >
              <ConfigField label="Enabled">
                <Toggle
                  checked={config.circuit_breaker.enabled}
                  onChange={(v) =>
                    updateConfig("circuit_breaker", "enabled", v)
                  }
                />
              </ConfigField>
            ...
            <ConfigSection
              title="🔧 QA & Cleanup"
              description="Quality assurance and maintenance"
            >
  ...
  return (
    <div className="space-y-3">
      <div className="text-xs text-gray-400 mb-2">
        Drag order determines fallback priority
      </div>
    ...
    <div className="space-y-3">
      {Object.entries(limits).length === 0 ? (
        <div className="text-xs text-gray-500 italic">
          No per-type limits configured (using global max)
        </div>
      ) : (
        <div className="space-y-2">
          {Object.entries(limits).map(([type, limit]) => (
            <div key={type} className="flex items-center gap-2">
              <span className="text-sm text-gray-300 w-20 truncate capitalize">
                {type}
              </span>
              <input
                type="number"
                ...
                value={limit}
                onChange={(e) =>
                  updateLimit(type, parseInt(e.target.value) || 1)
                }
