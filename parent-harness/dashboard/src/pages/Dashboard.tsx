...
function mapAgentToCard(agent: Agent) {
  return {
    ...
    runningInstances:
      (agent as Agent & { running_instances?: number }).running_instances ?? 0,
  };
}
...
export function Dashboard() {
  ...
  const [stabilityHealth, setStabilityHealth] =
    useState<StabilityHealth | null>(null);
  ...
  const agentCards =
    agentsLoading || agentsError ? mockAgents : agents.map(mapAgentToCard);

  const taskCards =
    tasksLoading || tasksError ? mockTasks : tasks.map(mapTaskToCard);
  ...
  const eventItems =
    allEvents.length > 0 ? allEvents.map(mapEventToStream) : mockEvents;
      ...
      <div className="grid grid-cols-12 gap-4 h-[calc(100vh-10rem)]">
        ...
        <aside className="col-span-2 bg-gray-800 rounded-lg p-4 overflow-y-auto">
          ...
          <div className="space-y-2">
            ...
            {agentsError && (
              <div className="text-red-400 text-xs mb-2">Using mock data</div>
            )}
...
function SystemHealthPanel({
  ...
  const hasCriticalAlerts = alerts?.recentAlerts.some(
    (a) => a.severity === "critical",
  );
  const hasOpenCircuits =
    circuitBreakers &&
          ...
          {buildHealth && (
            <span
              className={`px-2 py-0.5 rounded text-xs font-medium ${
                buildHealth.status === "healthy"
                  ? "bg-green-900/50 text-green-300"
                  : buildHealth.status === "degraded"
        ...
        <div className="grid grid-cols-4 gap-4 mt-4">
          ...
          <div className="bg-gray-800 rounded-lg p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              {buildHealth && (
                <span
                  className={`w-3 h-3 rounded-full ${statusColors[buildHealth.status]}`}
                />
              )}
            </div>
            {buildHealth ? (
          ...
          <div className="bg-gray-800 rounded-lg p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              {stabilityHealth && (
                <span
                  className={`w-3 h-3 rounded-full ${statusColors[stabilityHealth.status]}`}
                />
              )}
            </div>
            {stabilityHealth ? (
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span
                    className={`font-medium capitalize ${
                      stabilityHealth.status === "stable"
                        ? "text-green-400"
                        : stabilityHealth.status === "unstable"
                          ? "text-yellow-400"
                          : "text-red-400"
                    }`}
                  >
                    {stabilityHealth.status}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-400">Crashes:</span>
                  <span className="text-white font-mono">
                    {stabilityHealth.crashCount}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-400">Uptime:</span>
                  <span className="text-white font-mono">
                    {formatUptime(stabilityHealth.uptime)}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                ...
                {Object.entries(circuitBreakers).map(([type, status]) => (
                  <div key={type} className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <span
                        className={`w-2 h-2 rounded-full ${circuitStateColors[status.state]}`}
                      />
                      <span
