...
export function Agents() {
  ...
  async function fetchAgentMessages(agent: Agent) {
    try {
      ...
      const botType = botTypeMap[agent.type] || botTypeMap[agent.id] || agent.type
      const res = await fetch(`${API_BASE}/telegram/messages?bot_type=${botType}&limit=20`)
      if (res.ok) {
    ...
    <Layout>
      <div className="grid grid-cols-12 gap-4 h-[calc(100vh-5rem)]">
        ...
        <div className="col-span-3 bg-gray-800 rounded-lg p-4 overflow-y-auto">
          <h2 className="text-lg font-semibold mb-4">🤖 Agent Fleet ({agents.length})</h2>
            ...
            {agents.map((agent) => (
              ...
              >
                ...
                <div className="mt-2 flex items-center gap-3 text-xs text-gray-400">
                  <span className={modelColors[agent.model] || 'text-gray-400'}>{agent.model}</span>
          ...
          {selectedAgent ? (
            <>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-400">{selectedAgent.metadata?.description || selectedAgent.type}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className={`px-3 py-1 rounded-full text-sm font-medium ${modelColors[selectedAgent.model] || 'text-gray-400'} bg-gray-900`}>
                      {selectedAgent.model}
                    </span>
                    <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-gray-900">
                      <span className={`w-2 h-2 rounded-full ${statusColors[selectedAgent.status]}`} />
                      <span className="text-sm">{statusLabels[selectedAgent.status]}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex border-b border-gray-700">
                {(['info', 'tools', 'telegram', 'activity'] as DetailTab[]).map((tab) => (
                  <button
                    key={tab}
              ...
              <div className="flex-1 overflow-y-auto p-6">
                ...
                {activeTab === 'telegram' && (
                  <TelegramTab agent={selectedAgent} botStatus={getBotStatus(selectedAgent)} />
                )}
                {activeTab === 'activity' && (
                  <ActivityTab messages={agentMessages} formatTime={formatTime} />
                )}
              </div>
            </>
          ) : (
...
function InfoTab({ agent, formatTime }: { agent: Agent; formatTime: (t: string | null) => string }) {
  return (
    <div className="space-y-6">
      {/* Stats Grid */}
      <div className="grid grid-cols-4 gap-4">
        <StatCard label="Tasks Completed" value={agent.tasks_completed} icon="✅" />
        <StatCard label="Tasks Failed" value={agent.tasks_failed} icon="❌" />
        <StatCard label="Running Instances" value={agent.running_instances} icon="🏃" />
        <StatCard label="Last Heartbeat" value={formatTime(agent.last_heartbeat)} icon="💓" />
      </div>
      ...
      {agent.metadata && (
        <>
          ...
          {/* Trigger Conditions */}
          {agent.metadata.triggerConditions && agent.metadata.triggerConditions.length > 0 && (
      ...
      {agent.current_task_id && (
        <Section title="🔄 Current Task">
          <div className="bg-blue-900/30 border border-blue-700 rounded-lg p-4">
            <span className="font-mono text-blue-300">{agent.current_task_id}</span>
          </div>
        </Section>
      )}
    </div>
  ...
  return (
    <div className="space-y-4">
      <p className="text-gray-400 mb-6">
        This agent has access to {tools.length} tool{tools.length !== 1 ? 's' : ''}.
      </p>
      
      <div className="grid gap-4">
        {tools.map((tool) => (
          <div key={tool} className="bg-gray-900 rounded-lg p-4 border border-gray-700">
...
function TelegramTab({ agent, botStatus }: { agent: Agent; botStatus?: BotStatus }) {
    ...
    <div className="space-y-6">
      ...
      {botStatus && (
        <div className={`rounded-lg p-4 border ${
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <span className={`w-3 h-3 rounded-full ${botStatus.webhookActive ? 'bg-green-500' : 'bg-red-500'}`} />
              <span className="font-semibold text-white">Webhook Status</span>
            </div>
            <span className={`text-sm ${botStatus.webhookActive ? 'text-green-400' : 'text-red-400'}`}>
      ...
      <Section title="📱 Configuration">
        <div className="bg-gray-900 rounded-lg divide-y divide-gray-700">
          ...
          <ConfigRow label="Bot Token Env" value={telegram.botEnvVar} type="env" />
          <ConfigRow label="Webhook Path" value={telegram.webhookPath || 'Not configured'} type="path" />
          {botStatus && (
            <>
              <ConfigRow label="Bot Username" value={botStatus.username} type="username" />
              <ConfigRow label="Webhook URL" value={botStatus.webhookUrl} type="url" />
            </>
          )}
        </div>
      </Section>
...
function AgentCommands({ agentType }: { agentType: string }) {
  ...
  if (commands.length === 0) {
    return <p className="text-gray-500 text-sm">No commands available for this agent type</p>
  }
    ...
    <div className="space-y-2">
      {commands.map((c) => (
        <div key={c.cmd} className="flex items-center gap-4">
          <code className="text-blue-400 font-mono bg-gray-800 px-2 py-1 rounded">{c.cmd}</code>
...
function ActivityTab({ messages, formatTime }: { messages: TelegramMessage[]; formatTime: (t: string) => string }) {
  ...
  return (
    <div className="space-y-3">
      <p className="text-gray-400 mb-4">Recent Telegram messages from this agent:</p>
          ...
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                msg.message_type === 'error' ? 'bg-red-900/50 text-red-300' :
                msg.message_type === 'success' ? 'bg-green-900/50 text-green-300' :
                msg.message_type === 'warning' ? 'bg-yellow-900/50 text-yellow-300' :
                'bg-gray-700 text-gray-300'
              }`}>
                {msg.message_type}
              </span>
              {msg.task_display_id && (
                <span className="text-xs font-mono text-blue-400">{msg.task_display_id}</span>
              )}
            </div>
...
function ConfigRow({ label, value, type }: { label: string; value: string; type: string }) {
  return (
    <div className="flex justify-between items-center p-3">
      <span className="text-gray-400 text-sm">{label}</span>
      <span className={`font-mono text-sm ${
        type === 'channel' ? 'text-blue-400' :
        type === 'env' ? 'text-yellow-400' :
        type === 'path' ? 'text-green-400' :
        type === 'url' ? 'text-purple-400' :
        type === 'username' ? 'text-cyan-400' :
        'text-gray-300'
      }`}>
        {type === 'url' && value.length > 50 ? value.slice(0, 50) + '...' : value}
      </span>
    </div>
  )
}
