      ...
      <Layout>
        <div className="p-4">
          <div className="bg-red-900/50 border border-red-500 rounded-lg p-4">
            <h2 className="text-red-400 font-bold">Error Loading Event System</h2>
      ...
      <div className="p-4 h-[calc(100vh-4rem)] overflow-auto">
        ...
        {!data ? (
          ...
          <>
            ...
            {selectedComponent && data.components[selectedComponent as keyof typeof data.components] && (
              <ComponentDetail
                component={data.components[selectedComponent as keyof typeof data.components]}
                formatTime={formatTime}
                formatInterval={formatInterval}
              />
            )}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <span className={`px-2 py-0.5 rounded text-xs font-medium ${statusColors[status] || 'bg-gray-600'}`}>
          {status}
        </span>
      </div>
      ...
      <div className="flex flex-wrap gap-2 text-xs">
        {component.metrics && Object.entries(component.metrics).slice(0, 4).map(([key, value]) => {
          if (typeof value === 'boolean') {
            return (
              <span key={key} className={`px-2 py-1 rounded ${value ? 'bg-green-900 text-green-300' : 'bg-red-900 text-red-300'}`}>
                {key}: {value ? '✓' : '✗'}
              </span>
            )
          }
          if (Array.isArray(value)) {
            return (
              <span key={key} className="px-2 py-1 bg-gray-700 rounded">
                {key}: {value.map(v => typeof v === 'number' ? v.toFixed(2) : v).join(', ')}
              </span>
            )
          }
          return (
            <span key={key} className="px-2 py-1 bg-gray-700 rounded">
              {key}: {typeof value === 'number' && key.includes('Usage') ? `${value}%` : value}
            </span>
          )
        })}
      </div>
      ...
      {component.scannerList && (
        <div className="mt-3 space-y-1">
          {component.scannerList.map((scanner) => (
            <div key={scanner.name} className="flex items-center justify-between text-xs">
              <span className={scanner.enabled ? 'text-green-400' : 'text-gray-500'}>
    ...
    <div className="bg-gray-900 rounded-lg p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {component.transitions && (
          <div>
            <h3 className="text-sm font-semibold text-gray-400 mb-2">Valid Transitions</h3>
                ...
                <div key={from} className="flex items-center gap-1">
                  ...
                  {toList.length > 0 ? (
                    <>
                      ...
                      <span className="text-green-400">{toList.join(' | ')}</span>
                    </>
                  ) : (
        ...
        {component.subscribedEvents && (
          <div>
            <h3 className="text-sm font-semibold text-gray-400 mb-2">Subscribes To</h3>
            <div className="flex flex-wrap gap-1">
              {component.subscribedEvents.map((event) => (
                <span key={event} className="px-1.5 py-0.5 bg-purple-900 text-purple-300 rounded text-xs">
                  {event}
                </span>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1">
              {component.emitsEvents.map((event) => (
                <span key={event} className="px-1.5 py-0.5 bg-cyan-900 text-cyan-300 rounded text-xs">
                  {event}
                </span>
              ))}
            </div>
        ...
        {component.thresholds && (
          <div>
            <h3 className="text-sm font-semibold text-gray-400 mb-2">Thresholds</h3>
            <div className="space-y-1">
        ...
        {component.scannerList && (
          <div className="md:col-span-2">
            <h3 className="text-sm font-semibold text-gray-400 mb-2">Scanners</h3>
            <div className="space-y-2">
              {component.scannerList.map((scanner) => (
                <div key={scanner.name} className="bg-gray-800 rounded p-2">
                  <div className="flex items-center justify-between">
                    <span className={`font-medium ${scanner.enabled ? 'text-green-400' : 'text-gray-500'}`}>
                      {scanner.enabled ? '●' : '○'} {scanner.name}
                    </span>
                    <span className="text-xs text-gray-500">
                  ...
                  <p className="text-xs text-gray-400 mt-1">{scanner.description}</p>
                  <div className="flex items-center justify-between mt-1 text-xs">
                    <span className="text-cyan-400">Emits: {scanner.emitsEvent}</span>
                    {scanner.lastRun && (
                      <span className="text-gray-500">Last: {formatTime(scanner.lastRun)}</span>
                    )}
                  </div>
                </div>
              ))}
        ...
        {/* Event Counts */}
        {component.eventCounts && Object.keys(component.eventCounts).length > 0 && (
          <div className="md:col-span-2">
            <h3 className="text-sm font-semibold text-gray-400 mb-2">Event Counts</h3>
            <div className="flex flex-wrap gap-1">
              {Object.entries(component.eventCounts)
                .sort((a, b) => b[1] - a[1])
                .slice(0, 15)
                .map(([event, count]) => (
                  <span key={event} className="px-1.5 py-0.5 bg-gray-700 rounded text-xs">
                    {event}: <span className="font-mono text-blue-400">{count}</span>
                  </span>
                ))}
            </div>
          </div>
              ...
              <tbody>
                {component.recentEvents.map((event) => (
                  <tr key={event.id} className="border-t border-gray-700">
                    <td className="p-2 text-gray-500 font-mono">{formatTime(event.timestamp)}</td>
                    <td className="p-2 text-cyan-400 font-mono">{event.type}</td>
                    <td className="p-2 text-gray-300 truncate max-w-md">{event.summary}</td>
                  </tr>
                ))}
              </tbody>
            </table>
