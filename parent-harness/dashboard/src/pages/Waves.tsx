...
export function Waves() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API_BASE}/waves`).then(r => r.json()).catch(() => []),
      fetch(`${API_BASE}/wave-runs`).then(r => r.json()).catch(() => []),
      ...
      setTasks(Array.isArray(tasksData) ? tasksData.filter((t: WaveTask) => t.wave_number) : [])
      setLoading(false)
    })
  }, [])
  ...
  const getWaveStats = () => {
    const waveNumbers = [...new Set(tasks.map(t => t.wave_number))].sort((a, b) => a - b)
  ...
  return (
    <div className="p-6">
      ...
      {waveRuns.length > 0 && (
        <div className="mb-8">
          <h2 className="text-lg font-semibold text-gray-300 mb-4">Active Runs</h2>
              ...
              <div key={run.id} className="bg-gray-800 rounded-lg p-4">
                ...
                <div className="mt-2 h-2 bg-gray-700 rounded-full overflow-hidden">
                  <div 
                    ...
                    style={{ width: `${(run.current_wave / run.total_waves) * 100}%` }}
                  />
                </div>
              </div>
            ))}
      ...
      {/* Wave Statistics */}
      <div className="mb-8">
        <h2 className="text-lg font-semibold text-gray-300 mb-4">Wave Statistics</h2>
        {waveStats.length === 0 ? (
            ...
            {waveStats.map(stat => (
              <div 
                ...
                className={`bg-gray-800 rounded-lg p-4 cursor-pointer border-2 transition-colors ${
                  selectedWave === stat.wave ? 'border-blue-500' : 'border-transparent hover:border-gray-600'
                }`}
                onClick={() => setSelectedWave(selectedWave === stat.wave ? null : stat.wave)}
              >
                <div className="flex items-center justify-between mb-3">
                  <span className="text-xl font-bold text-white">Wave {stat.wave}</span>
                  <span className="text-sm text-gray-400">{stat.total} tasks</span>
                </div>
              ...
              <tbody className="divide-y divide-gray-700">
                {getWaveTasks(selectedWave).map(task => (
                  <tr key={task.id} className="hover:bg-gray-750">
                    <td className="px-4 py-3 text-sm font-mono text-gray-400">{task.display_id}</td>
      ...
      {waveStats.length === 0 && waveRuns.length === 0 && (
        <div className="text-center py-12 text-gray-500">
          ...
          <p className="text-sm mt-2">Tasks with wave_number will appear here</p>
        </div>
      )}
    </div>
  )
