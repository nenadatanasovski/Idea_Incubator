...
const SCREENSHOT_DIR = '/home/ned-atanasovski/.openclaw/workspace/harness-screenshots';
...
async function runTests() {
  ...
  try {
    ...
    results.push({
      ...
      checks: {
        ...
        agentStatus: content.includes('Agent Status') || content.includes('AGENT STATUS'),
        eventStream: content.includes('Event Stream') || content.includes('EVENT STREAM'),
        taskQueue: content.includes('Task Queue') || content.includes('TASK QUEUE'),
      }
    });
  ...
  console.log('\nTC-4: Agents Page');
  try {
    await page.goto(`${DASHBOARD_URL}/agents`, { waitUntil: 'networkidle0', timeout: 15000 });
    await delay(3000); // Wait for API fetch
    await page.screenshot({ path: `${SCREENSHOT_DIR}/02-agents.png`, fullPage: true });
    
    const content = await page.content();
    const hasAgentFleet = content.includes('Agent Fleet');
    const hasBuildAgent = content.includes('Build Agent');
    const hasAgentList = content.includes('bg-gray-700/50') || content.includes('cursor-pointer');
    ...
    if (agentCards.length > 0) {
      ...
      await delay(1000);
      await page.screenshot({ path: `${SCREENSHOT_DIR}/03-agents-detail.png`, fullPage: true });
    }
    ...
    console.log(`  ${hasBuildAgent ? '✅' : '⚠️'} Agents page ${hasBuildAgent ? 'has agents' : 'empty'}`);
  } catch (err) {
  ...
  console.log('\nTC-5: Config Page');
  try {
    await page.goto(`${DASHBOARD_URL}/config`, { waitUntil: 'networkidle0', timeout: 15000 });
    await delay(3000); // Wait for API fetch
    await page.screenshot({ path: `${SCREENSHOT_DIR}/04-config.png`, fullPage: true });
    ...
    const checks = {
      ...
      clearCacheButton: content.includes('Clear Cache') || content.includes('Clear'),
      // New settings
      ...
      backoffMultiplier: content.includes('Backoff') || content.includes('Multiplier'),
      ...
      tokenLimit: content.includes('Token Limit') || content.includes('daily_token'),
    };
    ...
    results.push({
      test: 'TC-5',
      status: passedCount >= totalCount * 0.8 ? 'PASS' : passedCount >= totalCount * 0.5 ? 'PARTIAL' : 'FAIL',
      checks,
      summary: `${passedCount}/${totalCount} checks passed`
    });
    
    console.log(`  ${passedCount >= totalCount * 0.8 ? '✅' : '⚠️'} Config page: ${passedCount}/${totalCount} checks`);
  ...
  console.log('\nTC-13: Tasks Page');
  try {
    await page.goto(`${DASHBOARD_URL}/tasks`, { waitUntil: 'networkidle0', timeout: 15000 });
    await delay(2000);
  ...
  console.log('\nTC-14: Sessions Page');
  try {
    await page.goto(`${DASHBOARD_URL}/sessions`, { waitUntil: 'networkidle0', timeout: 15000 });
    await delay(2000);
    await page.screenshot({ path: `${SCREENSHOT_DIR}/06-sessions.png`, fullPage: true });
