...
async function runTests() {
  ...
  try {
    await page.goto(`${DASHBOARD_URL}/telegram`, { waitUntil: 'networkidle0', timeout: 15000 });
    await delay(2000);
    await page.screenshot({ path: `${SCREENSHOT_DIR}/telegram-01-page.png`, fullPage: true });
  ...
  try {
    const content = await page.content();
    const hasTelegramNav = content.includes('📱 Telegram') || content.includes('Telegram</a>') || content.includes('/telegram');
  ...
  try {
    const content = await page.content();
    const hasChannelSection = content.includes('Telegram Channels') || content.includes('No messages yet');
  ...
  try {
    ...
    await delay(2000);
    await page.screenshot({ path: `${SCREENSHOT_DIR}/telegram-02-dashboard.png`, fullPage: true });
    
    const content = await page.content();
    const hasBudgetIndicator = content.includes('💰') || content.includes('percent') || content.includes('Server On') || content.includes('%');
  ...
  try {
    ...
    await delay(2000);
    await page.screenshot({ path: `${SCREENSHOT_DIR}/telegram-03-config.png`, fullPage: true });
    
    const content = await page.content();
    const hasBudgetSection = content.includes('Budget') && content.includes('Token Limit');
  ...
  try {
    const response = await page.goto('http://localhost:3333/api/telegram/stats', { waitUntil: 'networkidle0' });
    ...
    console.log(`  ✅ API returned: ${data.total} messages, ${Object.keys(data.byBot || {}).length} bots`);
  } catch (err) {
  ...
  try {
    const response = await page.goto('http://localhost:3333/api/config/budget', { waitUntil: 'networkidle0' });
    const text = await response.text();
    const data = JSON.parse(text);
    ...
    console.log(`  ✅ Budget: ${data.daily?.percent_used?.toFixed(1)}% used, status: ${data.budget_status}`);
  } catch (err) {
