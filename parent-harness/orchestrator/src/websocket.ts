...
export const ws = {
  ...
  agentHeartbeat: (agentId: string) =>
    broadcast("agent:heartbeat", { agentId }),

  // Task events
  taskCreated: (task: unknown) => broadcast("task:created", task),
  taskUpdated: (task: unknown) => broadcast("task:updated", task),
  taskAssigned: (task: unknown, agentId: string) =>
    broadcast("task:assigned", { task, agentId }),
  taskCompleted: (task: unknown) => broadcast("task:completed", task),
  taskFailed: (task: unknown, error: string) =>
    broadcast("task:failed", { task, error }),
  ...
  iterationLogged: (iteration: unknown) =>
    broadcast("session:iteration", iteration),
