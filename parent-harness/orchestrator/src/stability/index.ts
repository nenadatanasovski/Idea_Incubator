...
const STABILITY_FILE = path.join(process.env.HOME || '/tmp', '.harness', 'stability.json');
const CRASH_LOG_FILE = path.join(process.env.HOME || '/tmp', '.harness', 'crash.log');
...
export function initStability(): void {
  ...
  if (fs.existsSync(STABILITY_FILE)) {
    try {
      const prevState = JSON.parse(fs.readFileSync(STABILITY_FILE, 'utf-8')) as StabilityState;
      ...
      if (prevState.pid !== process.pid) {
        ...
        console.log(`⚠️ Recovered from crash. Total crashes: ${state.crashCount}`);
...
function startSelfMonitor(): void {
  ...
  setInterval(() => {
    ...
    if (state.tickCount === lastTickCount && state.tickCount > 0) {
      ...
      console.warn(`⚠️ No new ticks since last check (tick #${state.tickCount})`);
    }
    lastTickCount = state.tickCount;
 ...
 */
export function crashProtectSync<T>(
  fn: () => T,
  context: string
): T | null {
  try {
