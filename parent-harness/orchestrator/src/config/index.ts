...
export interface HarnessConfig {
  ...
  rate_limit: {
    ...
    max_requests_per_minute: number; // Per-minute RPM limit (default: 35 = 70% of 50)
    max_tokens_per_minute: number; // Per-minute TPM limit (default: 28000 = 70% of 40K)
    max_concurrent: number; // Max concurrent spawns (default: 3)
  };
}
...
export function onConfigChange(
  listener: (config: HarnessConfig) => void,
): void {
 ...
 */
export function validateConfig(config: Partial<HarnessConfig>): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  if (config.planning?.interval_hours !== undefined) {
    if (
      config.planning.interval_hours < 1 ||
      config.planning.interval_hours > 168
    ) {
  ...
  if (config.cleanup?.retention_days !== undefined) {
    if (
      config.cleanup.retention_days < 1 ||
  ...
  if (config.agents?.max_output_tokens !== undefined) {
    if (
      config.agents.max_output_tokens < 1000 ||
      config.agents.max_output_tokens > 100000
    ) {
  ...
  if (config.budget?.p0_reserve_percent !== undefined) {
    if (
      config.budget.p0_reserve_percent < 0 ||
      config.budget.p0_reserve_percent > 50
    ) {
  ...
  if (config.circuit_breaker?.failure_threshold !== undefined) {
    if (
      ...
      config.circuit_breaker.failure_threshold > 20
    ) {
