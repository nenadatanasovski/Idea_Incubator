...
console.log('\n🔎 Validation Query Test:\n');
const validationResult = db.prepare(`
  SELECT
    COUNT(*) as total
  FROM test_cases
  WHERE id LIKE 'phase_1_task_%'
...
console.log(`   1. 8 test_cases created: ${cases.count === 8 ? '✓ PASS' : '✗ FAIL'}`);
console.log(`   2. Each has at least 1 step: ${allHaveSteps ? '✓ PASS' : '✗ FAIL'}`);
console.log(`   3. Key assertions defined: ${assertions.count >= 8 ? '✓ PASS' : '✗ FAIL'}`);
console.log(`   4. Validation query works: ${validationResult.total === 8 ? '✓ PASS' : '✗ FAIL'}`);
console.log(`   5. Phase 1 task tracking: ${cases.count === 8 && allHaveSteps ? '✓ PASS' : '✗ FAIL'}`);
