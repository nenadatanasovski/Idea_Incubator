...
export function verifySchemaAlignment(): void {
  ...
  for (const table of requiredTables) {
    if (!tableExists(table)) {
      throw new Error(`Schema verification failed: missing required table "${table}"`);
    }
  }
  ...
  for (const [table, column] of requiredColumns) {
    if (!columnExists(table, column)) {
      throw new Error(`Schema verification failed: missing column ${table}.${column}`);
    }
  }
}
...
export function runMaintenanceCheckpointAndBackup(reason: string = 'periodic'): string {
  verifyDatabaseIntegrity();
