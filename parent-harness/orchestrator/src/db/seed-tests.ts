...
export function seedTests(): void {
      ...
      cases: [
        {
          ...
          steps: [
            ...
            },
            {
              ...
              command: 'sqlite3 parent-harness/data/harness.db "SELECT COUNT(*) FROM agent_sessions"',
              expectedExitCode: 0,
            },
            {
              name: 'Check observability_events table',
              command: 'sqlite3 parent-harness/data/harness.db "SELECT COUNT(*) FROM observability_events"',
              expectedExitCode: 0,
            },
          ],
        },
          ...
          steps: [
            {
              ...
              command: 'sqlite3 parent-harness/data/harness.db "PRAGMA foreign_keys"',
              expectedExitCode: 0,
          ...
          steps: [
            {
              ...
              command: 'curl -s -X POST http://localhost:3333/api/agents -H "Content-Type: application/json" -d \'{"id":"test_seed_agent","name":"Seed Test Agent","type":"build_agent"}\'',
              expectedExitCode: 0,
          ...
          steps: [
            {
              ...
              command: 'curl -s -X PATCH http://localhost:3333/api/agents/test_seed_agent -H "Content-Type: application/json" -d \'{"status":"working"}\'',
              expectedExitCode: 0,
          ...
          steps: [
            {
              ...
              command: 'curl -s -X POST http://localhost:3333/api/tasks -H "Content-Type: application/json" -d \'{"display_id":"TEST-001","title":"Seed Test Task","category":"test","priority":"P2"}\'',
              expectedExitCode: 0,
          ...
          steps: [
            {
              ...
              command: 'curl -s -X POST http://localhost:3333/api/tasks/assign -H "Content-Type: application/json" -d \'{"taskDisplayId":"TEST-001","agentId":"test_seed_agent"}\'',
              expectedExitCode: 0,
            },
          ],
        },
  ...
  const suiteCount = getOne<{ count: number }>('SELECT COUNT(*) as count FROM test_suites')?.count || 0;
  const caseCount = getOne<{ count: number }>('SELECT COUNT(*) as count FROM test_cases')?.count || 0;
  const stepCount = getOne<{ count: number }>('SELECT COUNT(*) as count FROM test_steps')?.count || 0;
