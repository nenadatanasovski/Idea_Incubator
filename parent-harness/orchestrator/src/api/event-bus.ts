...
function extractSummary(type: HarnessEventName, data: unknown): string {
  ...
  switch (type) {
    ...
    case "task:ready_for_qa":
      ...
      return `${task?.display_id || "Unknown"}: ${task?.title || ""}`.slice(
        0,
        100,
      );
...
function extractIds(
  type: HarnessEventName,
  data: unknown,
): { taskId?: string; agentId?: string; sessionId?: string } {
  const d = data as Record<string, unknown>;
...
eventBusRouter.get("/events", (req, res) => {
  ...
  if (type) {
    filtered = filtered.filter(
      (e) => e.type === type || e.type.startsWith(type + ":"),
    );
  }
  if (taskId) {
  ...
  res.json({
    ...
    components: {
      ...
      eventBus: {
        ...
        status: "active",
        metrics: {
          totalEventsFired: Object.values(busStats.eventCounts).reduce(
            (a, b) => a + b,
            0,
          ),
          activeListeners: Object.values(busStats.listenerCounts).reduce(
            (a, b) => a + b,
            0,
          ),
      ...
      taskStateMachine: {
        ...
        emoji: "🔄",
        description:
          "Validates state transitions - pending → in_progress → verification → completed",
        status: "active",
        transitions: {
      ...
      qaService: {
        ...
        emoji: "✅",
        description:
          "Handles task verification - subscribes to task:ready_for_qa events",
      ...
      spawnService: {
        ...
        emoji: "🚀",
        description:
          "Handles agent spawning with backpressure - reacts to task:pending events",
        status: spawnStatus.enabled
          ? spawnStatus.canSpawn
            ? spawnStatus.processing
              ? "spawning"
              : "ready"
            : "paused"
          : "disabled",
        ...
        recentEvents: [
          ...getRecentEventsForComponent("session:started"),
          ...getRecentEventsForComponent("task:started"),
        ].slice(0, 10),
      },
      ...
      resourceMonitor: {
        ...
        emoji: "📊",
        description:
          "Monitors CPU/memory usage - emits backpressure events when thresholds crossed",
        status:
          resourceState.cpuStatus === "normal" &&
          resourceState.memoryStatus === "normal"
            ? "healthy"
            : "warning",
        metrics: {
        ...
        recentEvents: getRecentEventsForComponent("system:cpu").concat(
          getRecentEventsForComponent("system:memory"),
        ),
      },
      ...
      scanners: {
        ...
        emoji: "🔍",
        description:
          "Lightweight periodic checks that emit events when conditions met",
        status: "active",
        scannerList: [
          {
            name: "Pending Task Scanner",
            description:
              "Finds pending tasks with idle agents → emits task:pending",
          ...
          },
          {
            ...
            description:
              "Finds agents without heartbeat for 15+ min → emits agent:stuck",
          ...
          },
          {
            ...
            description:
              "Checks if planning cycle is due → emits schedule:planning_due",
