...
const WEBHOOK_SECRET =
  process.env.TELEGRAM_WEBHOOK_SECRET || "harness-webhook-secret-2026";
...
async function processUpdate(
  ...
  if (update.message?.text) {
    ...
    console.log(
      `📨 Received /${command} from ${botType} bot: ${args.join(" ")}`,
    );
    ...
    if (handler) {
      ...
      // Unknown command
      const availableCommands = botHandlers
        ? Array.from(botHandlers.keys())
            .map((c) => `/${c}`)
            .join(", ")
        : "none";
