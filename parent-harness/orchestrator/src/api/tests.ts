...
testsRouter.get('/suites', (_req, res) => {
  const suites = query<TestSuite>('SELECT * FROM test_suites ORDER BY phase ASC, name ASC');
  res.json(suites);
});
...
testsRouter.get('/suites/:id', (req, res) => {
  const suite = getOne<TestSuite>('SELECT * FROM test_suites WHERE id = ?', [req.params.id]);
  if (!suite) {
...
testsRouter.get('/runs/:id', (req, res) => {
  const testRun = getOne<TestRun>('SELECT * FROM test_runs WHERE id = ?', [req.params.id]);
  if (!testRun) {
...
testsRouter.patch('/runs/:id', (req, res) => {
  ...
  const { status, totalCases, passedCases, failedCases, skippedCases } = req.body;
  ...
  const updated = getOne<TestRun>('SELECT * FROM test_runs WHERE id = ?', [req.params.id]);
  res.json(updated);
});
