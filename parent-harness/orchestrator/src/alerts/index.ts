...
const ALERT_RULES: AlertRule[] = [
  {
    ...
    check: async () => {
      ...
      const workingAgents = allAgents.filter(a => 
        !['orchestrator', 'sia'].includes(a.type) && a.status !== 'idle'
      );
      const stuckAgents = allAgents.filter(a => a.status === 'stuck');
      
      if (stuckAgents.length > 0 && stuckAgents.length === workingAgents.length) {
        return {
          triggered: true,
  ...
  },
  {
    ...
    check: async () => {
      ...
      const completed = recentEvents.filter((e: ObservabilityEvent) => e.type === 'task:completed').length;
      const failed = recentEvents.filter((e: ObservabilityEvent) => e.type === 'task:failed').length;
      const total = completed + failed;
      
      if (total >= 5) { // Need at least 5 tasks to calculate
...
async function fireAlert(rule: AlertRule, message: string): Promise<void> {
  ...
  // Log to console
  console.log(`[ALERT] ${rule.severity.toUpperCase()}: ${rule.name} - ${message}`);
...
export function getAlertStatus(): {
  ...
  const recentAlerts = Array.from(lastAlertTimes.values())
    .filter(time => now - time < recentWindow)
    .length;
