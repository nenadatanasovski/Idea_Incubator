...
export function initBuildHealth(projectPath: string): void {
  ...
  if (fs.existsSync(STATE_FILE)) {
    try {
      ...
      console.log(
        `📊 Build health loaded: ${currentState.status} (${currentState.errorCount} errors)`,
      );
    } catch {
...
export async function checkBuildHealth(): Promise<BuildHealthState> {
  ...
  try {
    ...
    if (status !== currentState.status) {
      console.log(
        `🏗️ Build health: ${currentState.status} → ${status} (${errorCount} errors)`,
      );
      ...
      if (status === "critical") {
        events.systemError?.(
          "build-health",
          `Critical: ${errorCount} TypeScript errors`,
        );
 ...
 */
export function shouldAllowSpawn(
  taskCategory?: string,
  taskPriority?: string,
  ...
  if (currentState.status === "warning") {
    console.warn(
      `⚠️ Build has ${currentState.errorCount} errors - consider prioritizing fixes`,
    );
  }
