...
import type { FeatureSpec, LayerGenerator, GeneratorContext, LayerResult, ValidationResult } from '../feature-orchestrator.js';
...
const mockClaudeResponses = {
  multiFileSuccess: JSON.stringify({
    ...
    subtype: 'success',
    is_error: false,
    result: 'TASK_COMPLETE: Successfully modified 3 files with coordinated changes',
    total_cost_usd: 0.05,
    usage: {
      input_tokens: 1500,
      output_tokens: 800,
  ...
  multiFileRollback: JSON.stringify({
    ...
    subtype: 'error',
    is_error: true,
    result: 'TASK_FAILED: Compilation error in file 2, rolling back all changes',
    total_cost_usd: 0.03,
    usage: {
      input_tokens: 1200,
      output_tokens: 400
  ...
  frontendGeneration: JSON.stringify({
    ...
    subtype: 'success',
    is_error: false,
    result: 'TASK_COMPLETE: Generated React component with TypeScript types and Tailwind styling',
    total_cost_usd: 0.04,
    usage: {
      input_tokens: 1000,
      output_tokens: 1500
  ...
  backendGeneration: JSON.stringify({
    ...
    subtype: 'success',
    is_error: false,
    result: 'TASK_COMPLETE: Generated Express route handler with validation middleware',
    total_cost_usd: 0.04,
    usage: {
      input_tokens: 1100,
      output_tokens: 1300
  ...
  validationBlocked: JSON.stringify({
    ...
    subtype: 'error',
    is_error: true,
    result: 'TASK_FAILED: Validation failed - TypeScript errors detected, preventing commit',
    total_cost_usd: 0.01,
    usage: {
      input_tokens: 500,
      output_tokens: 200
...
describe('Build Agent Integration Tests (VIBE-P13-010)', () => {
  ...
  describe('Frontend Component Generation', () => {
    it('should generate React component from spec', () => {
      ...
      uiGen.generate({ components: [componentSpec] }, context).then(result => {
        expect(result.status).toBe('completed');
  ...
  describe('Backend Endpoint Generation', () => {
    ...
    it('should include validation middleware', () => {
      const apiSpec = {
        ...
        pathParams: {
          id: { type: 'string' as const, required: true, description: 'Test ID' }
        },
        requestBody: {
          typeName: 'TestRequest',
          fields: {
            name: { type: 'string', required: true, validation: [{ type: 'minLength' as const, value: 3 }] }
          }
        },
        responseBody: {
          typeName: 'TestResponse',
    ...
    it('should generate TypeScript types for request/response', () => {
      ...
      expect(generated.typeDefinitions).toContain('export interface CreateTestRequest');
      expect(generated.typeDefinitions).toContain('export interface TestResponse');
  ...
  describe('Database Migration Generation', () => {
    it('should generate SQL migration from spec', async () => {
      ...
      const result = await dbGen.generate({ tables: [], migrations: [] }, context);
    ...
    it('should include rollback migration', async () => {
      ...
      const result = await dbGen.generate(
        ...
        {
          runId: 'test-run-5',
          featureSpec: { id: 'test-5', name: 'Test', description: 'Test', layers: {} },
          previousLayers: new Map(),
          codebaseRoot: '/tmp/test'
        }
      );
  ...
  describe('Full Feature Orchestration', () => {
    it('should orchestrate all three layers successfully', async () => {
      ...
      const totalFiles = Array.from(result.layerResults.values())
