...
async function runCheck(
  ...
  try {
    ...
    return {
      ...
      error: (error.stderr || error.message || "Unclassified failure").slice(
        0,
        2500,
      ),
    };
...
export async function verifyTask(taskId: string): Promise<QAResult> {
  ...
  const passed = criticalChecks.every(
    (c) => c.passed || c.output?.includes("No "),
  );
  ...
  try {
    ...
    if (fixTask) {
      ...
      events.taskAssigned(
        fixTask.id,
        "system",
        `Fix task created for ${originalTask.display_id}`,
      );
    }
    ...
    if (
      ...
      console.warn(
        `⚠️ Fix task already exists for ${originalTask.display_id}, skipping`,
      );
      return undefined;
    }
    console.error(
      `❌ Failed to create fix task for ${originalTask.display_id}:`,
      errorMsg,
    );
    return undefined;
  }
...
export async function runQACycle(): Promise<QAResult[]> {
  ...
  const pendingTasks = tasks.getTasks({
    status: "pending_verification" as tasks.Task["status"],
  });
