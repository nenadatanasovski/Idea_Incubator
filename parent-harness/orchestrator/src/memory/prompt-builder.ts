...
import { calculateRelevance, generateTaskSignature } from '../introspection/relevance.js';
 ...
 */
export function buildIntrospectionContext(agentId: string, task?: Task): string {
  ...
  if (scored.length > 0) {
    ...
    for (const { session, relevance } of scored) {
      ...
      sections.push(`- **Session ${session.id.substring(0, 8)}** (relevance: ${pct}%, status: ${session.status})`);
      if (summary) {
