...
import {
  checkOpenClawHealth,
  buildAgentPrompt,
} from "../../src/spawner/openclaw-spawner.js";
  ...
  test("QA can run verification on a task", async () => {
    try {
      ...
      const task = tasks.createTask({
        ...
        pass_criteria: JSON.stringify(["Tests pass", "Build succeeds"]),
      });
      createdTaskIds.push(task.id);
  ...
  test("OpenClaw gateway health check", async () => {
    try {
      ...
      recordResult(
        ...
        healthy,
        healthy ? undefined : "Gateway not running or not reachable",
      );
    } catch (e) {
