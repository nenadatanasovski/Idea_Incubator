...
const VALID_PHASES = ['database', 'types', 'services', 'api', 'tests', 'exports'];
 ...
 */
function validateDependencyGraph(tasks: Array<Record<string, any>>): { valid: boolean; error?: string } {
  ...
  for (const task of tasks) {
    if (!visit(task.id, [])) {
      return { valid: false, error: `Circular or invalid dependency involving ${task.id}` };
    }
  }
 ...
 */
function calculateStructuralSimilarity(generated: string, reference: string): number {
  const genSections = REQUIRED_SPEC_SECTIONS.filter(s => generated.includes(s));
  const refSections = REQUIRED_SPEC_SECTIONS.filter(s => reference.includes(s));
  ...
  describe('Test 1: Simple Feature (simple-counter)', () => {
    ...
    it('should have all required fields in each task', () => {
      for (const task of tasks) {
        ...
        expect(missing, `Task ${task.id} missing: ${missing.join(', ')}`).toHaveLength(0);
      }
    });
    ...
    it('should have actionable file paths in all tasks', () => {
      ...
      for (const ref of references) {
        ...
        for (const task of tasks) {
          ...
          if (task.file && !task.file.includes('/') && !task.file.includes('.')) {
  ...
  describe('Test 6: Structural Similarity', () => {
    it('should achieve >= 80% structural similarity for generated specs', () => {
      ...
      for (const ref of references) {
        ...
        const missingSections = hasRequiredSections(content, REQUIRED_SPEC_SECTIONS);
        expect(
          missingSections,
