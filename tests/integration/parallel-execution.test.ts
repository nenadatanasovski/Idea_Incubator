...
describe('Parallel Execution', () => {
  ...
  it('should aggregate token usage from parallel calls', async () => {
    ...
    const responses = await Promise.all([
      mockClient.messages.create({ model: 'test', max_tokens: 100, messages: [] }),
      mockClient.messages.create({ model: 'test', max_tokens: 100, messages: [] }),
      mockClient.messages.create({ model: 'test', max_tokens: 100, messages: [] })
    ]);

    const totalInput = responses.reduce((sum, r) => sum + r.usage.input_tokens, 0);
    const totalOutput = responses.reduce((sum, r) => sum + r.usage.output_tokens, 0);
  ...
  it('should handle mixed success/failure in parallel', async () => {
    ...
    // Mock one to fail
    mockClient.messages.create
      .mockResolvedValueOnce({ content: [{ type: 'text', text: 'success' }], usage: { input_tokens: 100, output_tokens: 200 } })
