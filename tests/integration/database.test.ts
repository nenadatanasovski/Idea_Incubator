...
describe('SQLite Database', () => {
  ...
  it('should validate idea_type enum', () => {
    ...
    expect(() => {
      db.run('INSERT INTO ideas (id, slug, title, idea_type) VALUES (?, ?, ?, ?)', [
  ...
  it('should set default lifecycle_stage to SPARK', () => {
    db.run('INSERT INTO ideas (id, slug, title) VALUES (?, ?, ?)', [
      'idea-1', 'test', 'Test Idea'
    ]);

    const result = db.exec('SELECT lifecycle_stage FROM ideas WHERE id = ?', ['idea-1']);
  ...
  it('should query ideas by lifecycle stage', () => {
    db.run('INSERT INTO ideas (id, slug, title, lifecycle_stage) VALUES (?, ?, ?, ?)', [
    ...
    const result = db.exec('SELECT * FROM ideas WHERE lifecycle_stage = ?', ['SPARK']);
