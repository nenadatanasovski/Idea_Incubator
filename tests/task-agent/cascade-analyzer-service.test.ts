...
async function createTestTask(title?: string): Promise<string> {
  ...
  await run(
    ...
    [taskId, `${TEST_PREFIX}${taskId.slice(0, 8)}`, title || `${TEST_PREFIX}Test Task`]
  );
  await saveDb();
  return taskId;
}
...
// Create dependency
async function createDependency(taskA: string, taskB: string, relType: string = 'depends_on'): Promise<void> {
  await run(
...
describe('CascadeAnalyzerService', () => {
  ...
  describe('analyze', () => {
    it('should detect cascade effects on status change', async () => {
      ...
      const analysis = await cascadeAnalyzerService.analyze(taskA, 'status_change', {
        newStatus: 'completed'
      });
  ...
  describe('priority_change trigger', () => {
    it('should detect priority recalculation effects', async () => {
      ...
      const analysis = await cascadeAnalyzerService.analyze(taskA, 'priority_change', {
        oldPriority: 'P3',
        newPriority: 'P1'
      });
      ...
      expect(analysis.effects.some(e => e.effectType === 'recalculate_priority')).toBe(true);
    });
  });
  ...
  describe('impact_change trigger', () => {
    it('should detect parallelism invalidation effects', async () => {
      ...
      const analysis = await cascadeAnalyzerService.analyze(taskA, 'impact_change', {
      ...
      expect(analysis.effects.some(e => e.effectType === 'invalidate_analysis')).toBe(true);
    });
  });
  ...
  describe('requirement_change trigger', () => {
    it('should notify dependent tasks of requirement changes', async () => {
      ...
      const analysis = await cascadeAnalyzerService.analyze(taskA, 'requirement_change', {
        changedFields: ['description', 'acceptance_criteria']
      });

      expect(analysis.trigger).toBe('requirement_change');
      expect(analysis.effects.some(e => e.effectType === 'notify_dependents')).toBe(true);
    });
  });
});
