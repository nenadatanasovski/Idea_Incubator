...
async function addFileImpact(taskId: string, filePath: string, operation: string = 'UPDATE'): Promise<void> {
  await run(
...
async function cleanupTestData(): Promise<void> {
  await run(`DELETE FROM task_file_impacts WHERE task_id IN (SELECT id FROM tasks WHERE display_id LIKE '${TEST_PREFIX}%')`);
...
describe('AtomicityValidator', () => {
  ...
  describe('validate', () => {
    ...
    it('should flag task with many file impacts', async () => {
      ...
      expect(result.violations.some(v => v.rule === 'single_file')).toBe(true);
    });
    ...
    it('should flag tasks with multiple concerns in title', async () => {
      ...
      expect(result.violations.some(v => v.rule === 'single_concern')).toBe(true);
    });
  });
  ...
  describe('rules', () => {
    ...
    it('should check time_bound rule based on effort', async () => {
      ...
      expect(smallResult.violations.some(v => v.rule === 'time_bound')).toBe(false);
      expect(xlargeResult.violations.some(v => v.rule === 'time_bound')).toBe(true);
    });
  });
