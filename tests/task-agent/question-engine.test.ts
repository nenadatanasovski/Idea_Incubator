...
describe('QuestionEngine', () => {
  ...
  describe('question categories', () => {
    it('should generate requirements questions', async () => {
      ...
      const questions = await questionEngine.generateQuestions(taskId);
      const requirementsQuestions = questions.filter(q => q.category === 'requirements');
    ...
    it('should generate acceptance questions', async () => {
      ...
      const questions = await questionEngine.generateQuestions(taskId);
      const acceptanceQuestions = questions.filter(q => q.category === 'acceptance');
    ...
    it('should generate testing questions', async () => {
      ...
      const questions = await questionEngine.generateQuestions(taskId);
      const testingQuestions = questions.filter(q => q.category === 'testing');
  ...
  describe('answerQuestion', () => {
    it('should record answer for a question', async () => {
      ...
      await questionEngine.answerQuestion(taskId, question.id, 'This is my answer');

      const updatedQuestions = await questionEngine.getQuestions(taskId);
      const answeredQuestion = updatedQuestions.find(q => q.id === question.id);
  ...
  describe('skipQuestion', () => {
    it('should remove skipped question', async () => {
      ...
      const questions = await questionEngine.generateQuestions(taskId);
      const optionalQuestion = questions.find(q => q.importance !== 'required');

      if (optionalQuestion) {
        await questionEngine.skipQuestion(taskId, optionalQuestion.id);

        const updatedQuestions = await questionEngine.getQuestions(taskId);
        const skippedQuestion = updatedQuestions.find(q => q.id === optionalQuestion.id);
  ...
  describe('areRequiredQuestionsAnswered', () => {
    it('should return false when required questions unanswered', async () => {
      ...
      const allAnswered = await questionEngine.areRequiredQuestionsAnswered(taskId);
