...
async function cleanupTestData(): Promise<void> {
  await run(`DELETE FROM task_state_history WHERE task_id IN (SELECT id FROM tasks WHERE display_id LIKE '${TEST_PREFIX}%')`);
...
describe('TaskStateHistoryService', () => {
  ...
  describe('getTransitionCount', () => {
    it('should count transitions for a task', async () => {
      ...
      const count = await taskStateHistoryService.getTransitionCount(testTaskId);
  ...
  describe('getTimeInStatus', () => {
    it('should calculate time spent in a status', async () => {
      ...
      const timeInPending = await taskStateHistoryService.getTimeInStatus(testTaskId, 'pending');
  ...
  describe('hasBeenInStatus', () => {
    it('should return true if task has been in status', async () => {
      ...
      const hasBeen = await taskStateHistoryService.hasBeenInStatus(testTaskId, 'pending');
    ...
    it('should return false if task has never been in status', async () => {
      ...
      const hasBeen = await taskStateHistoryService.hasBeenInStatus(testTaskId, 'failed');
