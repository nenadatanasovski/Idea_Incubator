...
async function cleanupTestData(): Promise<void> {
  await run(`DELETE FROM task_impacts WHERE task_id IN (SELECT id FROM tasks WHERE display_id LIKE '${TEST_PREFIX}%')`);
...
describe('TaskImpactService', () => {
  ...
  describe('getImpacts', () => {
    ...
    it('should filter impacts by type', async () => {
      ...
      const fileImpacts = await taskImpactService.getImpactsByType(testTaskId, 'file');
      const apiImpacts = await taskImpactService.getImpactsByType(testTaskId, 'api');
