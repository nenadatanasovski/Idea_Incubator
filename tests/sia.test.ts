...
describe("SIA", () => {
  describe("Extraction Rules", () => {
    ...
    it("should match sql-js API errors", () => {
      const rule = EXTRACTION_RULES.find(
        (r) => r.name === "sql-js-async-getdb",
      );
      expect(rule).toBeDefined();
      expect(
        rule?.errorPattern.test("prepare is not a function on Promise"),
      ).toBe(true);
    });

    it("should match express return type errors", () => {
      const rule = EXTRACTION_RULES.find(
        (r) => r.name === "express-return-type",
      );
      expect(rule).toBeDefined();
      expect(rule?.errorPattern.test("error TS7030: Not all code paths")).toBe(
        true,
      );
    });
  });
  ...
  describe("inferFilePattern", () => {
    ...
    it("should infer route pattern", () => {
      expect(inferFilePattern("server/routes/users.ts")).toBe(
        "server/routes/*.ts",
      );
    });
  ...
  describe("matchesFilePattern", () => {
    ...
    it("should match directory patterns", () => {
      expect(
        matchesFilePattern("server/routes/api.ts", "server/routes/*"),
      ).toBe(true);
    ...
    it("should match specific directory + extension", () => {
      ...
      expect(
        matchesFilePattern("server/routes/api.js", "server/routes/*.ts"),
      ).toBe(false);
    });
  });
  ...
  describe("Execution Analyzer", () => {
    ...
    describe("identifyFailures", () => {
      it("should identify failed tasks with error messages", () => {
        ...
        expect(failures[0].errorMessage).toBe(
          "DATETIME not supported in SQLite",
        );
  ...
  describe("CLAUDE.md Updater", () => {
    const createEntry = (
      overrides: Partial<import("../types/sia").KnowledgeEntry> = {},
    ): import("../types/sia").KnowledgeEntry => ({
      id: "entry-1",
      type: "gotcha",
