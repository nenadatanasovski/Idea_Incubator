...
const mockProfileContext: ProfileContext = {
  ...
  passionContext: 'Interests: plants, technology. Strong motivation from personal experience.',
  skillsContext: 'Technical Skills: React, Python, ML. Experience: 5 years in mobile dev. Gaps: Marketing.',
  networkContext: 'Industry Connections: Tech startup ecosystem. Community: Plant hobbyist forums. Network: 500+ LinkedIn.',
  lifeStageContext: 'Status: Employed. Hours Available: 20/week. Runway: 12 months. Tolerance: Medium.'
};
...
describe('formatProfileForCategory', () => {
  describe('when profile is null', () => {
    ...
    it('should work for all categories', () => {
      const categories = ['problem', 'solution', 'feasibility', 'market', 'risk', 'fit'] as const;
  ...
  describe('feasibility category', () => {
    it('should include skills context', () => {
      const result = formatProfileForCategory(mockProfileContext, 'feasibility');
  ...
  describe('field extraction', () => {
    ...
    it('should handle missing fields gracefully', () => {
      ...
      for (const category of categories) {
        expect(() => formatProfileForCategory(minimalProfile, category)).not.toThrow();
      }
    });
  });
});
