...
describe('classifyQuestionToId', () => {
  describe('Problem category questions', () => {
    ...
    it('should classify pain severity questions to P2_PAIN', () => {
      ...
      expect(classifyQuestionToId('How painful is this problem for users?')).toBe('P2_PAIN');
      expect(classifyQuestionToId('What is the severity of the pain?')).toBe('P2_PAIN');
    });
    ...
    it('should classify validation questions to P4_EVIDENCE', () => {
      // Test the clearest case
      expect(classifyQuestionToId('What evidence do you have to support this?')).toBe('P4_EVIDENCE');
    });
  });
  ...
  describe('Solution category questions', () => {
    ...
    it('should classify differentiation questions to S3_DIFF', () => {
      // Test the clearest case
      expect(classifyQuestionToId('What makes this different from existing products?')).toBe('S3_DIFF');
    });
  });
  ...
  describe('Market category questions', () => {
    ...
    it('should classify competitor questions to M3_COMPETITORS', () => {
      ...
      expect(classifyQuestionToId('Who are your competitors?')).toBe('M3_COMPETITORS');
      expect(classifyQuestionToId('What is the competitive landscape?')).toBe('M3_COMPETITORS');
    });
...
describe('classifyQuestion', () => {
  it('should return detailed classification result', () => {
    const result = classifyQuestion('What is the core problem you are solving?');

    expect(result.originalQuestion).toBe('What is the core problem you are solving?');
