...
class MockCommunicationHub extends EventEmitter {
  ...
  async askQuestion(question: { id: string; [key: string]: unknown }) {
    ...
    return { success: true, messageId: 123, channel: 'telegram' as const, deliveredAt: new Date() };
  }

  async haltAgent(agentId: string, _agentType: string, _reason: string, _details: string) {
    this.haltedAgents.add(agentId);
    return { id: 'halt-1', agentId, agentType: _agentType, reason: _reason, details: _details, haltedAt: new Date() };
  }
}
...
async function testMON001() {
  ...
  const { MonitoringAgent } = await import('../../server/monitoring/monitoring-agent');

  // Test 1: Can instantiate MonitoringAgent
  const agent = new MonitoringAgent(mockDb);
  results.push({ test: 'MON-001.1: MonitoringAgent instantiation', passed: true });
  ...
  results.push({ test: 'MON-001.2: Agent registration', passed: agentRegistered });
  console.log(agentRegistered ? '  ✅ MON-001.2: Agent registration' : '  ❌ MON-001.2: Agent registration');
  ...
  console.log(statusUpdated ? '  ✅ MON-001.3: Status update' : '  ❌ MON-001.3: Status update');
  ...
  console.log(hasMetrics ? '  ✅ MON-001.4: System metrics' : '  ❌ MON-001.4: System metrics');
}
...
async function testMON002() {
  ...
  const { MonitoringAgent } = await import('../../server/monitoring/monitoring-agent');
  const { integrateMonitoringWithHub } = await import('../../server/monitoring/hub-integration');
  ...
  hub.emit('agent:ready', { agentId: 'hub-agent-1', session: { agentType: 'build', sessionId: 'sess-1' } });
  await new Promise(r => setTimeout(r, 10));
  const agentState = agent.getAgentState('hub-agent-1');
  const agentReadyHandled = agentState !== undefined && agentState.agentType === 'build';
  results.push({ test: 'MON-002.2: Agent ready event', passed: agentReadyHandled });
  console.log(agentReadyHandled ? '  ✅ MON-002.2: Agent ready event handling' : '  ❌ MON-002.2: Agent ready event handling');

  // Test 3: Agent blocked event is handled
  hub.emit('agent:blocked', { agentId: 'hub-agent-1', agentType: 'build', reason: 'waiting', questionIds: ['q-1'] });
  await new Promise(r => setTimeout(r, 10));
  const blockedState = agent.getAgentState('hub-agent-1');
  const blockedHandled = blockedState?.status === 'blocked';
  results.push({ test: 'MON-002.3: Agent blocked event', passed: blockedHandled });
  console.log(blockedHandled ? '  ✅ MON-002.3: Agent blocked event handling' : '  ❌ MON-002.3: Agent blocked event handling');
}
...
async function testMON005() {
  ...
  const issue = agent.detectIssue('timeout', 'high', 'Test timeout issue', { test: true });
  ...
  console.log(issueDetected ? '  ✅ MON-005.1: Issue detection' : '  ❌ MON-005.1: Issue detection');

  // Test 2: Issue has correct properties
  const hasProperties = issue.severity === 'high' && issue.description === 'Test timeout issue';
  results.push({ test: 'MON-005.2: Issue properties', passed: hasProperties });
  console.log(hasProperties ? '  ✅ MON-005.2: Issue properties' : '  ❌ MON-005.2: Issue properties');

  // Test 3: Issue emits event
  let eventReceived = false;
  agent.on('issue:detected', () => { eventReceived = true; });
  agent.detectIssue('error', 'critical', 'Critical error', {});
  const eventEmitted = eventReceived;
  results.push({ test: 'MON-005.3: Issue event emission', passed: eventEmitted });
  console.log(eventEmitted ? '  ✅ MON-005.3: Issue event emission' : '  ❌ MON-005.3: Issue event emission');
}
...
async function testMON006() {
  ...
  const { ResponseEscalator, ResponseLevel } = await import('../../server/monitoring/response-escalator');

  const hub = new MockCommunicationHub();
  const escalator = new ResponseEscalator(hub as any);

  results.push({ test: 'MON-006.1: ResponseEscalator instantiation', passed: true });
  ...
  results.push({ test: 'MON-006.2: LOG level response', passed: logLevelCorrect });
  console.log(logLevelCorrect ? '  ✅ MON-006.2: LOG level response' : '  ❌ MON-006.2: LOG level response');
  ...
  results.push({ test: 'MON-006.3: ALERT level response', passed: alertLevelCorrect });
  console.log(alertLevelCorrect ? '  ✅ MON-006.3: ALERT level response' : '  ❌ MON-006.3: ALERT level response');
  ...
  results.push({ test: 'MON-006.4: HALT level response', passed: haltLevelCorrect });
  console.log(haltLevelCorrect ? '  ✅ MON-006.4: HALT level response' : '  ❌ MON-006.4: HALT level response');

  // Notifications sent
  const notificationsSent = hub.notifications.size > 0;
  results.push({ test: 'MON-006.5: Notifications sent', passed: notificationsSent });
  console.log(notificationsSent ? '  ✅ MON-006.5: Notifications sent' : '  ❌ MON-006.5: Notifications sent');

  // Agent halted
  const agentHalted = hub.haltedAgents.has('test-agent-halt');
  results.push({ test: 'MON-006.6: Agent halt triggered', passed: agentHalted });
  console.log(agentHalted ? '  ✅ MON-006.6: Agent halt triggered' : '  ❌ MON-006.6: Agent halt triggered');
...
async function testMON009() {
  ...
  const { ResponseEscalator } = await import('../../server/monitoring/response-escalator');
  ...
  console.log(hasQuestions ? '  ✅ MON-009.1: Questions created' : '  ❌ MON-009.1: Questions created');
  ...
  results.push({ test: 'MON-009.2: ALERT question type', passed: hasAlertType });
  console.log(hasAlertType ? '  ✅ MON-009.2: ALERT question type' : '  ❌ MON-009.2: ALERT question type');

  results.push({ test: 'MON-009.3: APPROVAL question type', passed: hasApprovalType });
  console.log(hasApprovalType ? '  ✅ MON-009.3: APPROVAL question type' : '  ❌ MON-009.3: APPROVAL question type');
