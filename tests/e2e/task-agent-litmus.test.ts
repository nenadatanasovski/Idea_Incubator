...
async function cleanupTestData(): Promise<void> {
  await run(`DELETE FROM tasks WHERE display_id LIKE '${CONFIG.TEST_PREFIX}%' OR title LIKE '${CONFIG.TEST_PREFIX}%'`);
  await run(`DELETE FROM task_lists_v2 WHERE name LIKE '${CONFIG.TEST_PREFIX}%'`);
  await saveDb();
}
...
describe('Task Agent Litmus Tests', () => {
  ...
  describe('Flow 1: Task Creation → Evaluation Queue → Task List → Parallel Execution', () => {
    ...
    it('should show queue statistics', async () => {
      ...
      console.log(`[LITMUS] Queue stats: ${stats.totalQueued} queued, ${stats.staleCount} stale`);
    });
  ...
  describe('Flow 2: Multiple Tasks → Parallel Execution Waves', () => {
    ...
    it('should create Wave 2 when dependency added', async () => {
      ...
      await circularDependencyPrevention.safeAddDependency(tasks[2].id, tasks[0].id);
      ...
      for (const wave of waves) {
        console.log(`[LITMUS]   Wave ${wave.waveNumber}: ${wave.taskCount} tasks`);
      }
    });
  });
  ...
  describe('Flow 3: Circular Dependency Prevention', () => {
    ...
    it('should detect cycle when trying to add A depends on C', async () => {
      const cycleCheck = await circularDependencyPrevention.wouldCreateCycle(taskA.id, taskC.id);
    ...
    it('should prevent cycle-creating dependency', async () => {
      const addResult = await circularDependencyPrevention.safeAddDependency(taskA.id, taskC.id);
      ...
      console.log(`[LITMUS] Cycle prevented: ${addResult.cycleDetected?.hasCycle ? 'YES' : 'NO'}`);
    });
  });
  ...
  describe('Flow 4: File Conflict Detection', () => {
    ...
    it('should provide file conflict details', async () => {
      const details = await fileConflictDetector.getConflictDetails(task1.id, task2.id);
      ...
      console.log(`[LITMUS]   - File conflicts: ${details.fileConflicts.length}`);
      console.log(`[LITMUS]   - Conflicting file: ${details.fileConflicts[0].filePath}`);
    });
  });
  ...
  describe('Flow 5: Failure Isolation', () => {
    ...
    beforeEach(async () => {
      ...
      await run(
        `INSERT INTO task_lists_v2 (id, name, project_id, status) VALUES (?, ?, ?, ?)`,
        [taskListId, `${CONFIG.TEST_PREFIX}Failure Isolation Test`, null, 'draft']
      );
