...
async function runTest(): Promise<void> {
  ...
  if (summaryTokens > 500) {
    throw new Error(
      `FAIL: readme.summary exceeds 500 tokens (${summaryTokens})`,
    );
  }
  ...
  if (!coreDocs.readme.fullPath.endsWith("README.md")) {
    throw new Error(
      `FAIL: fullPath does not end with README.md: ${coreDocs.readme.fullPath}`,
    );
  }
...
`;
  ...
  const largeDocs = await buildCoreDocsLayer(
    "test-large-content",
    "large-idea",
  );
  ...
  if (largeTotalTokens > 1000) {
    throw new Error(
      `FAIL: Large content exceeds 1000 tokens (${largeTotalTokens})`,
    );
  }
  ...
  if (largeDocs.development.recentQA.length > 5) {
    throw new Error(
      `FAIL: More than 5 Q&A entries (${largeDocs.development.recentQA.length})`,
    );
  }
  ...
  if (largeDocs.development.gaps.length > 0) {
    console.log(
      `  Gaps: ${largeDocs.development.gaps.slice(0, 3).join(", ")}${largeDocs.development.gaps.length > 3 ? "..." : ""}`,
    );
  }
