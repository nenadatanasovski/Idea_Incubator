...
async function runTests() {
  ...
  for (const bot of BOTS_TO_TEST) {
    ...
    if (result.success) {
      console.log(
        `  ✅ ${bot.type.padEnd(12)} @${result.username} (ID: ${result.botId})`,
      );
    } else {
  ...
  if (testChatId) {
    ...
    for (const bot of workingBots) {
      ...
      if (sendResult.success) {
        console.log(
          `  ✅ ${bot.botType.padEnd(12)} Message sent (ID: ${sendResult.messageId})`,
        );
      } else {
  ...
  for (const bot of results.filter((r) => r.success)) {
    ...
    try {
      ...
      if (data.ok) {
        ...
        if (updates.length > 0) {
          console.log(
            `  📬 ${bot.botType.padEnd(12)} ${updates.length} pending update(s)`,
          );
          ...
          if (firstUpdate.message) {
            ...
            console.log(
              `     └─ From: ${msg.from?.username || msg.from?.id} | Chat: ${msg.chat.id}`,
            );
            console.log(
            ...
            if (!testChatId) {
              console.log(
                `     └─ 💡 Add TELEGRAM_TEST_CHAT_ID=${msg.chat.id} to .env`,
              );
            }
          }
        } else {
