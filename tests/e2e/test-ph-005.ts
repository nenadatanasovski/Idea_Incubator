...
import { classifyAllDocuments, DocumentClassification } from '../../agents/ideation/document-classifier.js';
...
async function setupTestData(): Promise<void> {
  ...
  fs.writeFileSync(path.join(ideaFolder, 'target-users.md'), '# Target Users\n');
  fs.writeFileSync(path.join(ideaFolder, 'research', 'market.md'), '# Market Research\n');
  fs.writeFileSync(path.join(ideaFolder, 'research', 'competitive.md'), '# Competitive Analysis\n');
...
async function runTest(): Promise<void> {
  ...
  try {
    ...
    const classifications = await classifyAllDocuments(TEST_USER, TEST_IDEA, 'RESEARCH');
    console.log('1. Returns array:', Array.isArray(classifications) ? 'PASS' : 'FAIL');

    // Test 2: All items have correct structure
    const hasCorrectStructure = classifications.every(c =>
    ...
    const expectedPaths = ['README.md', 'development.md', 'target-users.md', 'research/market.md', 'research/competitive.md', 'planning/brief.md'];
    ...
    const hasReasons = classifications.every(c => c.reason && c.reason.length > 0);
    ...
    let sorted = true;
    const priorityMap: Record<string, number> = { required: 2, recommended: 1, optional: 0 };
    ...
    classifications.forEach(c => {
      console.log(`  ${c.classification.padEnd(12)} ${c.path.padEnd(30)} ${c.reason}`);
    });

    // Overall result
    const allPassed = hasCorrectStructure && allIncluded && hasReasons && sorted;
    console.log('\n' + (allPassed ? 'TEST-PH-005: ALL PASS CRITERIA MET' : 'TEST-PH-005: SOME CRITERIA FAILED'));
