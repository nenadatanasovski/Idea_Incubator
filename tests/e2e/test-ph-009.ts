...
async function runTest(): Promise<void> {
  ...
  if (typeof check.currentPhase !== 'string' || !VALID_STAGES.includes(check.currentPhase)) {
    console.log(`FAIL: currentPhase '${check.currentPhase}' is not a valid LifecycleStage`);
    passed = false;
  } else {
    console.log(`PASS: currentPhase is a valid LifecycleStage (${check.currentPhase})`);
  }

  if (typeof check.targetPhase !== 'string' || !VALID_STAGES.includes(check.targetPhase)) {
    console.log(`FAIL: targetPhase '${check.targetPhase}' is not a valid LifecycleStage`);
    passed = false;
  } else {
    console.log(`PASS: targetPhase is a valid LifecycleStage (${check.targetPhase})`);
  }
  ...
  if (typeof check.completionPercent !== 'number') {
    ...
    console.log(`FAIL: completionPercent ${check.completionPercent} is out of range [0-100]`);
    passed = false;
  } else {
    console.log(`PASS: completionPercent is valid (${check.completionPercent}%)`);
  }
  ...
  if (!Array.isArray(check.missingRequired)) {
    ...
    console.log(`PASS: missingRequired is an array with ${check.missingRequired.length} items`);
  }
  ...
  if (!Array.isArray(check.warnings)) {
    ...
    console.log(`PASS: warnings is an array with ${check.warnings.length} items`);
  }
  ...
  if (check.missingRequired.length > 0 && check.canTransition === true) {
    console.log('FAIL: canTransition should be false when missingRequired is not empty');
    passed = false;
  } else if (check.missingRequired.length === 0 && check.canTransition === false) {
    // This could happen for invalid transition, check warnings
    if (!check.warnings.some(w => w.includes('Cannot transition'))) {
      console.log('WARN: canTransition is false but missingRequired is empty and no transition warning');
    } else {
      console.log('PASS: canTransition correctly reflects required docs or transition validity');
    }
  } else {
  ...
  const nonExistentCheck = await canTransitionTo('test-user', 'non-existent-idea-xyz', 'CLARIFY');
  console.log(`   Result: ${JSON.stringify(nonExistentCheck, null, 2)}`);

  if (typeof nonExistentCheck.canTransition !== 'boolean') {
    console.log('FAIL: Non-existent idea check should still return boolean canTransition');
    passed = false;
  } else {
    console.log('PASS: Non-existent idea returns valid TransitionCheck structure');
  }
