...
async function cleanupTestData(): Promise<void> {
  try {
    ...
    await run(`DELETE FROM task_impacts WHERE task_id IN (SELECT id FROM tasks WHERE title LIKE '${TEST_PREFIX}%')`);
    await run(`DELETE FROM task_appendices WHERE task_id IN (SELECT id FROM tasks WHERE title LIKE '${TEST_PREFIX}%')`);
    await run(`DELETE FROM task_versions WHERE task_id IN (SELECT id FROM tasks WHERE title LIKE '${TEST_PREFIX}%')`);
    await run(`DELETE FROM task_state_history WHERE task_id IN (SELECT id FROM tasks WHERE title LIKE '${TEST_PREFIX}%')`);
    await run(`DELETE FROM task_test_results WHERE task_id IN (SELECT id FROM tasks WHERE title LIKE '${TEST_PREFIX}%')`);
    await run(`DELETE FROM task_relationships WHERE source_task_id IN (SELECT id FROM tasks WHERE title LIKE '${TEST_PREFIX}%')`);
    await run(`DELETE FROM task_relationships WHERE target_task_id IN (SELECT id FROM tasks WHERE title LIKE '${TEST_PREFIX}%')`);
    ...
    await run(`DELETE FROM task_list_items WHERE task_id IN (SELECT id FROM tasks WHERE title LIKE '${TEST_PREFIX}%')`);
...
describe('8.1 PRD → Task Extraction → Execution', () => {
  ...
  it('should complete full PRD to execution workflow', async () => {
    ...
    const prd = await prdService.create({
      ...
      functionalDescription: 'Implement secure user authentication with login/logout',
      successCriteria: JSON.stringify([
        { description: 'Users can log in with email/password', metric: 'success_rate', target: '99%', verified: false },
        { description: 'Session persists across page reloads', metric: 'session_duration', target: '24h', verified: false },
      ]),
      constraints: JSON.stringify(['Must use JWT tokens', 'Must support OAuth2']),
      outOfScope: JSON.stringify(['Social login', 'Two-factor authentication']),
      status: 'draft',
    }, 'test-user');
...
describe('8.2 Impact Conflict Detection', () => {
  ...
  it('should detect UPDATE-UPDATE conflict on same file', async () => {
    ...
    // Check conflict
    const canParallel = await fileConflictDetector.canRunParallel(task1Id, task2Id);
    expect(canParallel).toBe(false);

    const details = await fileConflictDetector.getConflictDetails(task1Id, task2Id);
  ...
  it('should allow parallel execution for non-conflicting files', async () => {
    ...
    const canParallel = await fileConflictDetector.canRunParallel(task1Id, task3Id);
    expect(canParallel).toBe(true);
  });
  ...
  it('should get all conflicting tasks for a given task', async () => {
    ...
    const conflictingTasks = await fileConflictDetector.getConflictingTasks(task1Id);
...
describe('8.3 Cascade Propagation', () => {
  ...
  it('should identify tasks with overlapping file impacts', async () => {
    ...
    const tasksWithOverlap = await cascadeAnalyzerService.findTasksWithOverlappingImpacts(
      'file',
      'types/user.ts'
    );
  ...
  it('should propagate status changes through dependency chain', async () => {
    ...
    await taskStateHistoryService.recordStateChange(task1Id, 'pending', 'failed', 'test', 'Simulated failure');

    // Check if dependent tasks should be blocked
    const task2Row = await getOne(`SELECT status FROM tasks WHERE id = ?`, [task2Id]) as any;
    const task3Row = await getOne(`SELECT status FROM tasks WHERE id = ?`, [task3Id]) as any;
  ...
  it('should track cascade effects for auto-approve lists', async () => {
    ...
    const taskList = await getOne(`SELECT auto_approve_reviews FROM task_lists_v2 WHERE id = ?`, [taskListId]) as any;
    ...
    const canAutoApprove = await cascadeExecutorService.canAutoApprove(taskListId);
    expect(canAutoApprove).toBe(true);
  });

  it('should generate cascade report for task edit', async () => {
    // Simulate editing task1 description
    const originalTask = await getOne(`SELECT * FROM tasks WHERE id = ?`, [task1Id]) as any;
...
describe('8.4 Versioning and Rollback', () => {
  ...
  it('should support rollback to previous version', async () => {
    ...
    const currentTask = await getOne(`SELECT title, description FROM tasks WHERE id = ?`, [taskId]) as any;
    ...
    const rolledBackTask = await getOne(`SELECT title, description FROM tasks WHERE id = ?`, [taskId]) as any;
  ...
  it('should record state transitions in history', async () => {
    ...
    await taskStateHistoryService.recordStateChange(taskId, 'pending', 'in_progress', 'agent-1', 'Started work');
    await taskStateHistoryService.recordStateChange(taskId, 'in_progress', 'validating', 'agent-1', 'Code complete');
    await taskStateHistoryService.recordStateChange(taskId, 'validating', 'failed', 'validator', 'Tests failed');
    await taskStateHistoryService.recordStateChange(taskId, 'failed', 'in_progress', 'agent-1', 'Retry attempt');
    await taskStateHistoryService.recordStateChange(taskId, 'in_progress', 'validating', 'agent-1', 'Fixed issue');
    await taskStateHistoryService.recordStateChange(taskId, 'validating', 'completed', 'validator', 'All tests passed');
...
describe('Integration: Full Task System Workflow', () => {
  ...
  it('should complete full workflow from PRD to versioned task execution', async () => {
    ...
    // 5. Check conflicts
    const canParallel = await fileConflictDetector.canRunParallel(task1.id, task2.id);
    ...
    // 8. Get coverage
    const coverage = await prdCoverageService.calculateCoverage(prd.id);
    console.log(`  [8/8] Coverage calculated: ${Math.round(coverage.coveragePercent)}%`);
