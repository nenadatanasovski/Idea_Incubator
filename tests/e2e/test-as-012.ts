...
async function runTest() {
  ...
  try {
    ...
    if (fs.existsSync(newPath)) {
      ...
      const expectedFiles = ['README.md', 'development.md', 'research/market.md'];
      let filesFound = 0;
      ...
      if (filesFound === expectedFiles.length) {
        ...
        console.log(`✗ Some files missing (found ${filesFound}/${expectedFiles.length})`);
        allPassed = false;
      }
    } else {
    ...
    if (fs.existsSync(readmePath)) {
      ...
      if (metadata.ideaSlug === newSlug) {
        ...
        console.log(`✗ Frontmatter ideaSlug not updated (found: ${metadata.ideaSlug || metadata.id})`);
        allPassed = false;
      }
    }
    ...
    try {
      ...
      const dbTestOldPath = path.resolve(usersRoot, testUserSlug, 'ideas', testOldSlug);
      const dbTestNewPath = path.resolve(usersRoot, testUserSlug, 'ideas', testNewSlug);
      ...
      await run(`DELETE FROM ideation_artifacts WHERE user_slug = ?`, [testUserSlug]);
      await run(`DELETE FROM ideation_sessions WHERE user_slug = ?`, [testUserSlug]);
      await saveDb();
      ...
      await run(
        ...
        [testArtifactId, testSessionId, testUserSlug, testOldSlug, 'markdown', 'Test Artifact', 'Test content']
      );
      await saveDb();
      ...
      const dbTestPath = path.resolve(usersRoot, testUserSlug, 'ideas', testNewSlug);
