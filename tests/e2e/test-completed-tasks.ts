...
function log(taskId: string, description: string, passed: boolean, details: string) {
...
async function validateCOM() {
  ...
  try {
    ...
    log('COM-002', 'Bot registry and token management', hasClass && hasGetInstance,
      hasClass && hasGetInstance ? 'BotRegistry class and getBotRegistry function' : 'Missing exports');
  } catch (e) { log('COM-002', 'Bot registry', false, String(e)); }

  // COM-003: Chat linker
  try {
    const chatLinker = await import('../../server/communication/chat-linker');
    const hasClass = typeof chatLinker.ChatLinker === 'function';
    log('COM-003', 'Chat ID linking', hasClass, hasClass ? 'ChatLinker class' : 'Missing ChatLinker');
  } catch (e) { log('COM-003', 'Chat linker', false, String(e)); }
  ...
  try {
    ...
    log('COM-004', 'Telegram message sender', hasClass, hasClass ? 'TelegramSender class' : 'Missing');
  } catch (e) { log('COM-004', 'Telegram sender', false, String(e)); }

  // COM-005: Telegram receiver
  try {
    const receiver = await import('../../server/communication/telegram-receiver');
    const hasClass = typeof receiver.TelegramReceiver === 'function';
    log('COM-005', 'Telegram multi-bot receiver', hasClass, hasClass ? 'TelegramReceiver class' : 'Missing');
  } catch (e) { log('COM-005', 'Telegram receiver', false, String(e)); }

  // COM-006: _Question delivery
  try {
    const delivery = await import('../../server/communication/question-delivery');
    ...
    log('COM-006', '_Question delivery with inline buttons', hasClass,
      hasClass ? '_QuestionDelivery class' : 'Missing');
  } catch (e) { log('COM-006', '_Question delivery', false, String(e)); }

  // COM-007: Answer processor
  try {
    const processor = await import('../../server/communication/answer-processor');
    const hasClass = typeof processor.AnswerProcessor === 'function';
    log('COM-007', 'Answer processing and routing', hasClass,
      hasClass ? 'AnswerProcessor class' : 'Missing');
  } catch (e) { log('COM-007', 'Answer processor', false, String(e)); }
  ...
  try {
    ...
    log('COM-008', 'Gmail SMTP email sender', hasClass, hasClass ? 'EmailSender class' : 'Missing');
  } catch (e) { log('COM-008', 'Email sender', false, String(e)); }
  ...
  try {
    ...
    log('COM-009', 'Email question delivery (fallback)', hasClass,
      hasClass ? 'EmailChecker class' : 'Missing');
  } catch (e) { log('COM-009', 'Email checker', false, String(e)); }

  // COM-010: Notification dispatcher
  try {
    const dispatcher = await import('../../server/communication/notification-dispatcher');
    const hasClass = typeof dispatcher.NotificationDispatcher === 'function';
    log('COM-010', 'Notification dispatcher', hasClass,
      hasClass ? 'NotificationDispatcher class' : 'Missing');
  } catch (e) { log('COM-010', 'Notification dispatcher', false, String(e)); }
  ...
  try {
    ...
    log('COM-011', '_Question-gated execution (halt)', hasClass,
      hasClass ? 'ExecutionGate class' : 'Missing');
  } catch (e) { log('COM-011', 'Execution gate', false, String(e)); }
  ...
  try {
    ...
    log('COM-012', '/summary command handler', hasMethods,
      hasMethods ? 'TelegramReceiver handles commands' : 'Missing command handler');
  } catch (e) { log('COM-012', 'Summary command', false, String(e)); }

  // COM-013: Message templates
  try {
    const templates = await import('../../server/communication/message-templates');
    const hasClass = typeof templates.MessageTemplates === 'function';
    log('COM-013', 'Message templates', hasClass, hasClass ? 'MessageTemplates class' : 'Missing');
  } catch (e) { log('COM-013', 'Message templates', false, String(e)); }

  // COM-014: Agent handshake
  try {
    const handshake = await import('../../server/communication/agent-handshake');
    const hasClass = typeof handshake.AgentHandshake === 'function';
    log('COM-014', 'Agent communication handshake', hasClass,
      hasClass ? 'AgentHandshake class' : 'Missing');
  } catch (e) { log('COM-014', 'Agent handshake', false, String(e)); }
  ...
  try {
    ...
    log('COM-015', 'Communication Hub (single entry point)', hasClass && hasGetInstance,
      hasClass && hasGetInstance ? 'CommunicationHub singleton' : 'Missing');
  } catch (e) { log('COM-015', 'Communication Hub', false, String(e)); }
  ...
  log('COM-016', 'Database migrations', migrationsExist >= 2,
...
async function validateMON() {
  ...
  try {
    ...
    log('MON-001', 'Core architecture', hasClass && hasMethods,
      hasClass && hasMethods ? 'MonitoringAgent with all methods' : 'Missing methods');
  } catch (e) { log('MON-001', 'Core architecture', false, String(e)); }
  ...
  try {
    ...
    log('MON-002', 'Event bus listener', hasIntegrate && hasCreate,
      hasIntegrate && hasCreate ? 'Integration functions' : 'Missing functions');
  } catch (e) { log('MON-002', 'Event bus listener', false, String(e)); }
  ...
  try {
    ...
    log('MON-005', 'Detection engine', hasDetect && hasGetIssues,
      hasDetect && hasGetIssues ? 'detectIssue and getDetectedIssues' : 'Missing detection methods');
  } catch (e) { log('MON-005', 'Detection engine', false, String(e)); }
  ...
  try {
    ...
    log('MON-006', 'Response escalator', hasClass && hasLevels && hasMethods,
      hasClass && hasLevels && hasMethods ? 'ResponseEscalator with levels' : 'Missing');
  } catch (e) { log('MON-006', 'Response escalator', false, String(e)); }
  ...
  try {
    ...
    log('MON-008', 'Heartbeat emitter', hasStart && hasStop,
      hasStart && hasStop ? 'start/stop control heartbeat intervals' : 'Missing lifecycle');
  } catch (e) { log('MON-008', 'Heartbeat emitter', false, String(e)); }
  ...
  try {
    ...
    log('MON-009', '_Question integration (ALERT/ESCALATION/APPROVAL)',
      hasAlert && hasEscalate && hasHalt,
      hasAlert && hasEscalate && hasHalt ? 'All question types supported' : 'Missing types');
  } catch (e) { log('MON-009', '_Question integration', false, String(e)); }
}
...
async function validateWSK() {
  ...
  try {
    ...
    log('WSK-001', 'WebSocket server setup', hasInit, hasInit ? 'initWebSocket function' : 'Missing');
    ...
    log('WSK-002', 'Client connection management', hasRooms && hasActiveRooms,
    ...
    log('WSK-003', 'Event broadcasting', hasBroadcast && hasSessionBroadcast && hasAgentBroadcast,
    ...
    log('WSK-005', 'Connection heartbeat', hasMonitorClients,
...
async function validateQUE() {
  ...
  try {
    ...
    log('QUE-001', '_Question queue database schema', hasClass,
      hasClass ? 'AnswerProcessor manages queue' : 'Missing');
    log('QUE-002', 'Queue priority management', hasRegister,
      hasRegister ? 'register_Question with priority' : 'Missing');
    log('QUE-003', 'Answer processing and agent resume', hasProcessButton && hasProcessText,
      hasProcessButton && hasProcessText ? 'processButtonAnswer + processTextAnswer' : 'Missing');
  } catch (e) {
...
async function validateNTF() {
  ...
  try {
    ...
    log('NTF-001', 'Telegram bot setup', typeof sender.TelegramSender === 'function',
      'TelegramSender class');
    log('NTF-002', 'Telegram question delivery', typeof delivery._QuestionDelivery === 'function',
      '_QuestionDelivery class');
    log('NTF-003', 'Telegram inline answers', typeof receiver.TelegramReceiver === 'function',
...
async function validateFND() {
  ...
  log('FND-002', 'Complete pending migrations (021-024)', found === migrations.length,
