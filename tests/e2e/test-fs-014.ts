...
async function main(): Promise<void> {
  ...
  await test('addRelationship inserts row into idea_relationships table', async () => {
    ...
    const rows = await query<{ id: string }>(`SELECT id FROM idea_relationships WHERE id = ?`, [id]);
    if (rows.length !== 1) {
  ...
  await test('getRelationships returns all relationships for an idea', async () => {
    ...
    const relationships = await getRelationships('test-user1', 'test-feature-idea');

    if (relationships.length < 2) {
      throw new Error(`Expected at least 2 relationships, got ${relationships.length}`);
    }

    // Verify relationships have the expected structure
    const hasParent = relationships.some(r => r.relationshipType === 'parent');
    const hasIntegration = relationships.some(r => r.relationshipType === 'integrates_with');
  ...
  await test('getChildren returns ideas where this idea is parent', async () => {
    ...
    if (children[0].slug !== 'test-feature-idea') {
      throw new Error(`Expected child slug 'test-feature-idea', got '${children[0].slug}'`);
    }

    if (children[0].userSlug !== 'test-user1') {
      throw new Error(`Expected child userSlug 'test-user1', got '${children[0].userSlug}'`);
    }
  });
  ...
  await test('getParent returns parent idea if exists', async () => {
    ...
    if (parent.slug !== 'test-parent-app') {
      throw new Error(`Expected parent slug 'test-parent-app', got '${parent.slug}'`);
    }

    if (parent.userSlug !== 'test-user1') {
      throw new Error(`Expected parent userSlug 'test-user1', got '${parent.userSlug}'`);
    }
  });
  ...
  await test('Relationships correctly distinguish internal vs external', async () => {
    ...
    const rows = await query<{ to_external: string | null }>(`SELECT to_external FROM idea_relationships WHERE id = ?`, [externalId]);
    ...
    if (rows[0].to_external !== 'Shopify') {
      throw new Error(`Expected to_external 'Shopify', got '${rows[0].to_external}'`);
    }
  });
  ...
  await test('Metadata stored as JSON', async () => {
    ...
    const rows = await query<{ metadata: string }>(`SELECT metadata FROM idea_relationships WHERE id = ?`, [id]);
    ...
    if (!Array.isArray(parsedMetadata.tags) || parsedMetadata.tags.length !== 2) {
    ...
    const relationships = await getRelationships('test-user1', 'test-idea-with-metadata');
    const collabRel = relationships.find(r => r.relationshipType === 'collaboration');

    if (!collabRel) {
      throw new Error('Collaboration relationship not found via getRelationships');
    }

    if (collabRel.metadata.description !== 'A complex relationship') {
      throw new Error('Metadata not correctly parsed in getRelationships result');
    }
  });
  ...
  await test('removeRelationship removes the relationship', async () => {
    ...
    // Verify it exists
    let rows = await query<{ id: string }>(`SELECT id FROM idea_relationships WHERE id = ?`, [id]);
    if (rows.length !== 1) {
    ...
    // Verify it's gone
    rows = await query<{ id: string }>(`SELECT id FROM idea_relationships WHERE id = ?`, [id]);
    if (rows.length !== 0) {
