...
async function runTest() {
  ...
  if (loaded !== null) {
    ...
    if (loaded.title === "Load Test") {
      ...
      console.log(
        `✗ Title mismatch: expected 'Load Test', got '${loaded.title}'`,
      );
      failed++;
    }
    ...
    if (loaded.sessionId === "session-test-123") {
      ...
      console.log(
        `✗ SessionId mismatch: expected 'session-test-123', got '${loaded.sessionId}'`,
      );
      failed++;
    }
  ...
  console.log("\nTest 3: Returns null for non-existent file");
  const nonExistent = await loadArtifact(
    testUserSlug,
    testIdeaSlug,
    "does-not-exist.md",
  );
  ...
  const noFrontmatter = await loadArtifact(
    testUserSlug,
    testIdeaSlug,
    "no-frontmatter.md",
  );
  if (noFrontmatter !== null) {
    ...
    if (noFrontmatter.type === "markdown") {
      ...
      console.log(
        `✗ Expected default type 'markdown', got '${noFrontmatter.type}'`,
      );
      failed++;
    }
  ...
  console.log("\nTest 6: File path normalized (handles leading slash)");
  const withLeadingSlash = await loadArtifact(
    testUserSlug,
    testIdeaSlug,
    "/load-test.md",
  );
  if (
    ...
    withLeadingSlash.filePath === "load-test.md"
  ) {
  ...
  console.log("\nTest 7: File path normalized (adds .md extension)");
  const withoutExtension = await loadArtifact(
    testUserSlug,
    testIdeaSlug,
    "load-test",
  );
  if (
    ...
    withoutExtension.filePath === "load-test.md"
  ) {
    ...
    console.log(
      `✗ Extension not normalized: got '${withoutExtension.filePath}'`,
    );
    failed++;
  }
