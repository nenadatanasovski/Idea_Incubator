...
import { updateRecentlyUpdated, loadPriorityRules } from '../../agents/ideation/priority-manager.js';
import { getIdeaFolderPath, createIdeaFolder } from '../../utils/folder-structure.js';
...
async function runTest() {
  ...
  const count = priority2.recently_updated.filter((f: string) => f === 'research/market.md').length;
  if (count !== 1) {
  ...
  if (priority3.recently_updated.length > 10) {
    console.log('FAIL: List exceeds 10 items, length: ' + priority3.recently_updated.length);
    process.exit(1);
  }
  ...
  fs.rmSync(path.join(process.cwd(), 'users', userSlug), { recursive: true, force: true });
