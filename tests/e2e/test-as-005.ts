...
async function runTests(): Promise<void> {
  ...
  try {
    ...
    if (fs.existsSync(filePath)) {
      console.log(
        `✓ PASS: Creates file at correct path (${artifact.filePath})`,
      );
      passed++;
    } else {
  ...
  try {
    ...
    if (missingFields.length === 0) {
      console.log(
        "✓ PASS: File has valid frontmatter with: id, title, type, sessionId, createdAt, updatedAt",
      );
      passed++;
    } else {
      throw new Error(
        `Missing fields in frontmatter: ${missingFields.join(", ")}`,
      );
    }
  } catch (error) {
  ...
  try {
    ...
    if (fs.existsSync(indexPath)) {
      ...
      if (artifactCount > 0) {
        console.log(
          "✓ PASS: .metadata/index.json updated with new artifact entry",
        );
        passed++;
      } else {
  ...
  try {
    ...
    const missingFields = requiredFields.filter(
      (f) => artifact[f as keyof typeof artifact] === undefined,
    );
  ...
  try {
    ...
    if (
      ...
      console.log(
        "✓ PASS: Idempotent: calling twice with same path updates existing file",
      );
      passed++;
    } else {
      throw new Error(
        `Expected same ID (${artifact1.id} vs ${artifact2.id}) and updated content`,
      );
    }
  } catch (error) {
