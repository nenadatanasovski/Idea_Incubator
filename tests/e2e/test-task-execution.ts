...
async function executeSingleTask(
  ...
  return fetchJson(`${API_BASE}/executor/execute-one`, {
    // Uses 'execute-one' not 'execute-single'
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
async function testLoadTaskList(): Promise<boolean> {
  ...
  try {
    const taskList = await loadTaskList(
      "docs/bootstrap/SPEC-IMPLEMENTATION-GAPS.md",
    );
...
async function testExecutorStatus(): Promise<boolean> {
  ...
  try {
    ...
    console.log(
      `Total: ${status.totalTasks}, Completed: ${status.completedTasks}, Pending: ${status.pendingTasks}`,
    );
