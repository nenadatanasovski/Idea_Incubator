...
async function runTest() {
  ...
  try {
    ...
    results.push({
      ...
      passed: !draftExists,
      message: draftExists
        ? `Draft folder still exists at ${draftPath}`
        : "Draft folder successfully removed",
    });
    ...
    results.push({
      ...
      message: newFolderExists
        ? `Idea folder created at ${newPath}`
        : `Idea folder NOT found at ${newPath}`,
    });
    ...
    results.push({
      ...
      message: testFileInNewLocation
        ? "test.md preserved"
        : "test.md NOT found in new location",
    });
    ...
    const allSubdirsExist = subdirs.every((dir) =>
      fs.existsSync(path.join(newPath, dir)),
    );
    results.push({
      criterion: "All subdirectories created",
      passed: allSubdirsExist,
      message: allSubdirsExist
        ? "All subdirectories exist"
        : "Some subdirectories missing",
    });
    ...
    const sessionUpdated =
      sessions.length > 0 && sessions[0].idea_slug === ideaSlug;
    results.push({
      criterion: "Database ideation_sessions updated",
      passed: sessionUpdated,
      message: sessionUpdated
        ? `idea_slug changed to ${ideaSlug}`
        : `idea_slug is ${sessions[0]?.idea_slug || "NULL"}`,
    });
    ...
    results.push({
      ...
      message: pathCorrect
        ? "Path correct"
        : `Expected ${expectedPath}, got ${newPath}`,
    });
