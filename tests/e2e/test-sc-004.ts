 ...
 */
async function createTestSessionViaAPI(userSlug: string): Promise<{ sessionId: string; draftId: string }> {
...
async function runTests(): Promise<TestResult> {
  ...
  try {
    ...
    console.log('\n--- Test 1: Endpoint exists ---');
    const response1 = await fetch(`${API_BASE}/session/${sessionId}/name-idea`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ title: 'My New Idea', ideaType: 'business' })
    });
    result.criteria['endpoint_exists'] = response1.status !== 404;
    console.log('Endpoint exists:', result.criteria['endpoint_exists'], 'Status:', response1.status);

    if (!result.criteria['endpoint_exists']) {
      result.errors.push('Endpoint POST /api/ideation/session/:sessionId/name-idea does not exist');
      return result;
    }
    ...
    const newIdeaPath = path.join(process.cwd(), 'users', userSlug, 'ideas', expectedSlug);
    const oldDraftPath = path.join(process.cwd(), 'users', userSlug, 'ideas', draft.draftId);

    result.criteria['renames_draft_folder'] = fs.existsSync(newIdeaPath) && !fs.existsSync(oldDraftPath);
    console.log('Renames draft folder:', result.criteria['renames_draft_folder']);
    ...
    result.criteria['updates_session_db'] = sessionRow?.idea_slug === expectedSlug;
    console.log('Updates session idea_slug:', result.criteria['updates_session_db'], 'Value:', sessionRow?.idea_slug);

    // Test 5: Returns updated session with new ideaSlug
    result.criteria['returns_updated_session'] =
      response1.status === 200 &&
      data1.session?.ideaSlug === expectedSlug;
    console.log('Returns updated session:', result.criteria['returns_updated_session']);

    // Test 6: Returns 400 if session not linked to draft
    console.log('\n--- Test 6: Returns 400 for non-draft session ---');
    const nonDraftSessionId = await createTestSession(userSlug + '-nodraft', 'regular-idea');
    const response6 = await fetch(`${API_BASE}/session/${nonDraftSessionId}/name-idea`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ title: 'Test', ideaType: 'business' })
    });
    result.criteria['returns_400_for_non_draft'] = response6.status === 400;
    console.log('Returns 400 for non-draft:', result.criteria['returns_400_for_non_draft'], 'Status:', response6.status);
    ...
    const response7 = await fetch(`${API_BASE}/session/${sessionId2}/name-idea`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    console.log('Returns 409 for duplicate:', result.criteria['returns_409_for_duplicate'], 'Status:', response7.status);
    ...
    const response8 = await fetch(`${API_BASE}/session/${sessionId3}/name-idea`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ title: 'Test Idea! @#$% Special (Chars) 123', ideaType: 'business' })
    });
    ...
    console.log('Handles special characters:', result.criteria['handles_special_chars']);
    ...
    await run(
      `INSERT INTO ideation_artifacts (id, session_id, user_slug, idea_slug, type, title, content, file_path, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [artifactId, sessionId4, userSlug, draft4.draftId, 'markdown', 'Test Artifact', 'Test content', null, new Date().toISOString(), new Date().toISOString()]
    );
    await saveDb();

    const response9 = await fetch(`${API_BASE}/session/${sessionId4}/name-idea`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ title: 'Artifact Test Idea', ideaType: 'business' })
    });
    ...
    result.criteria['updates_artifacts_db'] = artifactRow?.idea_slug === 'artifact-test-idea';
    console.log('Updates artifacts idea_slug:', result.criteria['updates_artifacts_db'], 'Value:', artifactRow?.idea_slug);
...
runTests().then(result => {
  ...
  console.log('\n' + (result.passed ? 'TEST PASSED: TEST-SC-004' : 'TEST FAILED: TEST-SC-004'));
