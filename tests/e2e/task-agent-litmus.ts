...
function checkpoint(message: string): Promise<boolean> {
  return new Promise((resolve) => {
    ...
    rl.question('Press ENTER when verified (or type "skip" to skip): ', (answer) => {
...
async function testFlow1(db: Database.Database, userId: string, projectId: string): Promise<void> {
  logSection('TEST FLOW 1: Task Creation → Validation → Approval → Execution → Completion');
  ...
  await checkpoint('Telegram: Verify suggestion message received with [Execute Now] button');

  log('Simulating task completion...');
  db.prepare('UPDATE tasks SET status = ? WHERE id = ?').run('completed', taskId);
  db.prepare('UPDATE task_lists SET status = ?, completed_tasks = 1 WHERE id = ?').run('completed', taskListId);
...
async function testFlow2(db: Database.Database, userId: string, projectId: string): Promise<void> {
  ...
  db.prepare('UPDATE tasks SET status = ? WHERE id = ?').run('cancelled', duplicateTaskId);
...
async function testFlow4(db: Database.Database, userId: string, projectId: string): Promise<void> {
  ...
  db.prepare('UPDATE tasks SET status = ? WHERE id = ?').run('pending', taskAId);
  ...
  db.prepare('UPDATE tasks SET status = ? WHERE id = ?').run('blocked', taskBId);
  ...
  db.prepare('UPDATE tasks SET status = ? WHERE id = ?').run('blocked', taskCId);

  await checkpoint('Verify dependency chain: A → B → C (B and C blocked)');

  log('Step 4.4: Completing Task A...');
  db.prepare('UPDATE tasks SET status = ? WHERE id = ?').run('completed', taskAId);

  log('Step 4.5: Unblocking Task B...');
  db.prepare('UPDATE tasks SET status = ? WHERE id = ?').run('pending', taskBId);

  await checkpoint('Telegram: Verify unblock notification for Task B');

  log('Step 4.6: Completing Task B...');
  db.prepare('UPDATE tasks SET status = ? WHERE id = ?').run('completed', taskBId);
  db.prepare('UPDATE tasks SET status = ? WHERE id = ?').run('pending', taskCId);
