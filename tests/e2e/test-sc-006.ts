...
import { buildProgressLayer, estimateTokens } from '../../agents/ideation/idea-context-builder.js';
...
async function runTest(): Promise<boolean> {
  ...
  const ideaPath = path.join(process.cwd(), 'users', testUserSlug, 'ideas', testIdeaSlug);
  ...
  try {
    ...
    if (typeof progress.completionPercent !== 'number') {
      console.log(`❌ completionPercent is not a number: ${typeof progress.completionPercent}`);
      return false;
    }
    if (progress.completionPercent < 0 || progress.completionPercent > 100) {
      console.log(`❌ completionPercent out of range: ${progress.completionPercent}`);
      return false;
    }
    ...
    console.log(`✓ documentsComplete is array with ${progress.documentsComplete.length} items`);
    if (progress.documentsComplete.length > 0) {
      console.log(`  Sample: ${progress.documentsComplete.slice(0, 3).join(', ')}`);
    }
    ...
    console.log(`✓ documentsMissing is array with ${progress.documentsMissing.length} items`);
    if (progress.documentsMissing.length > 0) {
      console.log(`  Sample: ${progress.documentsMissing.slice(0, 3).join(', ')}`);
    }
    ...
    console.log(`✓ lastActivity is Date: ${progress.lastActivity.toISOString()}`);
    ...
    if (tokenCount >= 300) {
      console.log(`⚠ Token count exceeds target: ${tokenCount} tokens (target: < 300)`);
    ...
    console.log(JSON.stringify(progress, (key, value) => {
