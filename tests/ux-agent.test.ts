...
import {
  summarizeIssues,
  meetsThreshold,
} from "../agents/ux/accessibility-checker.js";
...
describe("UX Agent", () => {
  describe("Journey Definitions", () => {
    ...
    it("should get all journeys", () => {
      ...
      expect(allJourneys.length).toBeGreaterThanOrEqual(
        STANDARD_JOURNEYS.length,
      );
    });
  ...
  describe("MCP Bridge", () => {
    ...
    it("mock bridge should not throw on navigate", async () => {
      const bridge = createMockBridge();
      await expect(
        bridge.navigate("http://example.com"),
