...
describe("VaguenessDetector", () => {
  describe("detectVagueness", () => {
    ...
    test("PASS: Detects non-committal language", () => {
      ...
      expect(result.isVague).toBe(true);
      expect(result.reasons.some((r) => r.includes("Non-committal"))).toBe(
        true,
      );
    });

    test("PASS: Detects deflecting language", () => {
      const result = detectVagueness(
        "I haven't thought about that, good question",
      );
      expect(result.isVague).toBe(true);
    ...
    test("PASS: Detects unclear language", () => {
      const result = detectVagueness(
        "You know, like, basically stuff and things",
      );
      expect(result.isVague).toBe(true);
    ...
    test("PASS: Recognizes clear, substantive response", () => {
      const result = detectVagueness(
        "I have 10 years of experience in software development at fintech companies. My main expertise is in payment systems.",
      );
    ...
    test("PASS: No follow-up for clear response", () => {
      const result = detectVagueness(
        "I want to build a B2B SaaS product for the healthcare industry.",
      );
    ...
    test("PASS: Score is between 0 and 1", () => {
      const result = detectVagueness(
        "maybe possibly perhaps could be might be I think I guess sort of kind of",
      );
  ...
  describe("isSimpleConfirmation", () => {
    ...
    test("PASS: Rejects longer messages", () => {
      expect(isSimpleConfirmation("Yes, I agree with that assessment")).toBe(
        false,
      );
    });
  ...
  describe("classifyMessage", () => {
    ...
    test("PASS: Classifies substantive message", () => {
      const result = classifyMessage(
        "I have extensive experience in software development and want to build a B2B SaaS product for the healthcare industry targeting small clinics.",
      );
