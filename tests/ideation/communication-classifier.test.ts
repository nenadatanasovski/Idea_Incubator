...
describe('CommunicationStyleClassifier', () => {
  describe('classifyCommunicationStyle', () => {
    ...
    test('PASS: Ignores assistant messages', () => {
      const messages = [
        { role: 'assistant', content: 'This is a long assistant message with many words that should not affect the classification at all because it contains over fifty words and would normally trigger verbose scoring.' },
        { role: 'user', content: 'Yes.' },
        { role: 'assistant', content: 'Another verbose response from the agent with many many more words to try to influence the scoring algorithm incorrectly.' },
    ...
    test('PASS: Evidence array is populated', () => {
      const messages = [
        { role: 'user', content: 'A short message.' },
      ];
