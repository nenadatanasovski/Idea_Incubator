...
import {
  MemoryManager,
} from '../../agents/ideation/memory-manager.js';
import { createDefaultSelfDiscoveryState, createDefaultMarketDiscoveryState, createDefaultNarrowingState } from '../../utils/ideation-defaults.js';
...
describe('MemoryManager', () => {
  ...
  describe('upsert', () => {
    test('PASS: Creates new memory file if not exists', async () => {
      ...
      const mockDb = {
        ...
        exec: vi.fn()
          ...
          .mockReturnValueOnce([{ // Get created file
            columns: ['id', 'session_id', 'file_type', 'content', 'version', 'created_at', 'updated_at'],
            values: [['file-123', 'session-123', 'self_discovery', '# Self-Discovery', 1, '2024-01-01T00:00:00.000Z', '2024-01-01T00:00:00.000Z']],
          }]),
      };
      vi.mocked(getDb).mockReturnValue(mockDb as unknown as ReturnType<typeof getDb>);

      const file = await memoryManager.upsert('session-123', 'self_discovery', '# Self-Discovery');
    ...
    test('PASS: Updates existing memory file', async () => {
      ...
      const mockDb = {
        run: vi.fn(),
        exec: vi.fn()
          .mockReturnValueOnce([{ // Existing file found
            columns: ['id', 'version'],
            values: [['file-123', 1]],
          }])
          .mockReturnValueOnce([{ // Get updated file
            columns: ['id', 'session_id', 'file_type', 'content', 'version', 'created_at', 'updated_at'],
            values: [['file-123', 'session-123', 'self_discovery', '# Updated', 2, '2024-01-01T00:00:00.000Z', '2024-01-01T00:01:00.000Z']],
          }]),
      };
      vi.mocked(getDb).mockReturnValue(mockDb as unknown as ReturnType<typeof getDb>);

      const file = await memoryManager.upsert('session-123', 'self_discovery', '# Updated');
    ...
    test('PASS: Increments version on update', async () => {
      ...
      const mockDb = {
        ...
        exec: vi.fn()
          ...
          .mockReturnValueOnce([{
            columns: ['id', 'session_id', 'file_type', 'content', 'version', 'created_at', 'updated_at'],
            values: [['file-123', 'session-123', 'self_discovery', '# Updated', 3, '2024-01-01T00:00:00.000Z', '2024-01-01T00:01:00.000Z']],
          }]),
      };
      vi.mocked(getDb).mockReturnValue(mockDb as unknown as ReturnType<typeof getDb>);
  ...
  describe('get', () => {
    ...
    test('PASS: Returns memory file for valid ID', async () => {
      ...
      const mockDb = {
        ...
        exec: vi.fn().mockReturnValue([{
          columns: ['id', 'session_id', 'file_type', 'content', 'version', 'created_at', 'updated_at'],
          values: [['file-123', 'session-123', 'self_discovery', '# Content', 1, '2024-01-01T00:00:00.000Z', '2024-01-01T00:00:00.000Z']],
        }]),
      };
      vi.mocked(getDb).mockReturnValue(mockDb as unknown as ReturnType<typeof getDb>);
  ...
  describe('getByType', () => {
    test('PASS: Returns null for non-existent type', async () => {
      ...
      const file = await memoryManager.getByType('session-123', 'self_discovery');
    ...
    test('PASS: Returns all files for session', async () => {
      ...
      const mockDb = {
        ...
        exec: vi.fn().mockReturnValue([{
          columns: ['id', 'session_id', 'file_type', 'content', 'version', 'created_at', 'updated_at'],
          values: [
            ['file-1', 'session-123', 'self_discovery', '# Self', 1, '2024-01-01T00:00:00.000Z', '2024-01-01T00:00:00.000Z'],
            ['file-2', 'session-123', 'market_discovery', '# Market', 1, '2024-01-01T00:00:00.000Z', '2024-01-01T00:00:00.000Z'],
          ],
        }]),
      };
      vi.mocked(getDb).mockReturnValue(mockDb as unknown as ReturnType<typeof getDb>);
    ...
    test('PASS: Creates handoff notes', async () => {
      ...
      const mockDb = {
        ...
        exec: vi.fn()
          .mockReturnValue([{
            columns: ['id', 'session_id', 'file_type', 'content', 'version', 'created_at', 'updated_at'],
            values: [['file-123', 'session-123', 'handoff_notes', '# Handoff', 1, '2024-01-01T00:00:00.000Z', '2024-01-01T00:00:00.000Z']],
          }]),
      };
      vi.mocked(getDb).mockReturnValue(mockDb as unknown as ReturnType<typeof getDb>);

      await memoryManager.createHandoffSummary('session-123', 'We discussed coworking spaces');
