...
describe('Ideation Integration: Full Session Flow', () => {
  describe('Happy Path: Discovery Mode', () => {
    it('completes a full session from start to capture', () => {
      ...
      state = ideationReducer(state, {
        ...
        payload: { sessionId: 'session-abc', greeting: 'Let\'s discover your next idea!' },
      });
      ...
      state = ideationReducer(state, {
        ...
        payload: {
          message: {
            ...
            role: 'assistant',
            content: 'Let\'s discover your next idea! What are you passionate about?',
            buttons: [
              { id: 'btn-tech', label: 'Technology', value: 'tech', style: 'primary' },
              { id: 'btn-health', label: 'Health', value: 'health', style: 'secondary' },
            ],
            form: null,
      ...
      expect(state.conversation.streamingContent).toBe('That\'s exciting! AI is transforming many industries.');
      ...
      state = ideationReducer(state, {
        ...
        payload: {
          message: {
            ...
            sessionId: 'session-abc',
            role: 'assistant',
            content: 'That\'s exciting! AI is transforming many industries. What problems have you noticed?',
            buttons: null,
            form: null,
            createdAt: new Date().toISOString(),
          },
      ...
      expect(state.candidate.candidate?.title).toBe('AI-Powered Workflow Automation');
  ...
  describe('Happy Path: Have Idea Mode', () => {
    it('validates and captures an existing idea', () => {
      ...
      state = ideationReducer(state, {
        type: 'SESSION_CREATED',
        payload: { sessionId: 'session-def', greeting: 'Tell me about your idea!' },
      });
      ...
      state = ideationReducer(state, {
        ...
        payload: {
          message: {
            ...
            role: 'user',
            content: 'I want to build a meal planning app that uses AI to suggest recipes based on what\'s in your fridge',
            buttons: null,
            form: null,
            createdAt: new Date().toISOString(),
          },
...
describe('Ideation Integration: Token Handoff', () => {
  it('handles token limit approach and handoff', () => {
    let state: IdeationStore = {
      ...
      tokens: {
        usage: { total: 50000, limit: 100000, percentUsed: 50, shouldHandoff: false },
    ...
    state = ideationReducer(state, {
      type: 'TOKEN_UPDATE',
      payload: {
        usage: { total: 70000, limit: 100000, percentUsed: 70, shouldHandoff: false },
      },
    });
    ...
    state = ideationReducer(state, {
      type: 'TOKEN_UPDATE',
      payload: {
        usage: { total: 80000, limit: 100000, percentUsed: 80, shouldHandoff: true },
      },
    });
    ...
    state = ideationReducer(state, {
      type: 'TOKEN_UPDATE',
      payload: {
        usage: { total: 5000, limit: 100000, percentUsed: 5, shouldHandoff: false },
      },
    });
        ...
        messages: [
          {
            ...
            buttons: [
              { id: 'btn-saas', label: 'SaaS', value: 'saas', style: 'primary' },
              { id: 'btn-marketplace', label: 'Marketplace', value: 'marketplace', style: 'secondary' },
              { id: 'btn-service', label: 'Service', value: 'service', style: 'outline' },
            ],
            form: null,
            createdAt: new Date().toISOString(),
          },
...
describe('Ideation Integration: Session Lifecycle', () => {
  ...
  it('handles multiple handoffs in long session', () => {
    let state: IdeationStore = {
      ...
      tokens: {
        usage: { total: 0, limit: 100000, percentUsed: 0, shouldHandoff: false },
        handoffPending: false,
        handoffCount: 0,
      },
    };
...
describe('Ideation Integration: Message Threading', () => {
  it('maintains correct message order through multiple exchanges', () => {
    ...
    const messages = [
      ...
      { role: 'assistant' as const, content: 'Interesting! Can you give an example?' },
      { role: 'user' as const, content: 'Manual data entry that wastes time' },
      { role: 'assistant' as const, content: 'That\'s a great pain point to explore!' },
    ];
    ...
    expect(state.conversation.messages[6].content).toBe('That\'s a great pain point to explore!');
  });
});
