...
describe('question-generator', () => {
  ...
  const createMockRequirements = (overrides: Partial<AnalyzedRequirements> = {}): AnalyzedRequirements => ({
  ...
  describe('question types', () => {
    ...
    it('should support CLARIFYING type', () => {
      ...
      const hasClarifying = result.questions.some(q => q.type === 'CLARIFYING');
    ...
    it('should support CONFIRMING type', () => {
      ...
      const hasConfirming = result.questions.some(q => q.type === 'CONFIRMING');
    ...
    it('should support PREFERENCE type', () => {
      ...
      const hasPreference = result.questions.some(q => q.type === 'PREFERENCE');
  ...
  describe('question structure', () => {
    ...
    it('should provide defaults for non-blocking questions', () => {
      ...
      const clarifyingQuestions = result.questions.filter(q => q.type === 'CLARIFYING');
    ...
    it('should provide options for preference questions', () => {
      ...
      const preferenceQuestions = result.questions.filter(q => q.type === 'PREFERENCE');
  ...
  describe('ambiguity detection', () => {
    it('should detect TBD/TODO placeholders', () => {
      ...
      const hasTbdQuestion = result.questions.some(q =>
  ...
  describe('missing information detection', () => {
    ...
    it('should flag missing MVP scope', () => {
      ...
      const hasScopeQuestion = result.questions.some(q =>
    ...
    it('should ask about database when storage mentioned', () => {
      ...
      const hasDbQuestion = result.questions.some(q =>
  ...
  describe('blocking count', () => {
    it('should count blocking questions correctly', () => {
      ...
      const actualBlocking = result.questions.filter(q => q.type === 'BLOCKING').length;
  ...
  describe('canProceedWithDefaults', () => {
    it('should be true when all questions have defaults', () => {
      ...
      if (result.canProceedWithDefaults) {
        const blockingWithoutDefaults = result.questions.filter(q =>
          q.type === 'BLOCKING' && !q.default
        );
  ...
  describe('strict mode', () => {
    it('should generate more questions in strict mode', () => {
      ...
      expect(strictResult.questions.length).toBeGreaterThanOrEqual(normalResult.questions.length);
    });
  });
});
