...
`;
      ...
      expect(result.errors.some(e => e.includes('Functional Requirements'))).toBe(true);
    });
...
`;
  ...
  describe('loadTemplate', () => {
    it('should load spec template from file', async () => {
      ...
      const templatePath = path.join(process.cwd(), 'templates', 'unified', 'build', 'spec.md');
    ...
    it('should load tasks template from file', async () => {
      const renderer = new TemplateRenderer();
      const templatePath = path.join(process.cwd(), 'templates', 'unified', 'build', 'tasks.md');
    ...
    it('should throw for missing template', async () => {
      const renderer = new TemplateRenderer();
      await expect(renderer.loadTemplate('/nonexistent/path.md'))
