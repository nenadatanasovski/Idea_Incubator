...
import { ContextLoader, LoadedContext, Gotcha as _Gotcha } from '../../agents/specification/context-loader.js';
...
describe('context-loader', () => {
  ...
  describe('loadGotchas', () => {
    ...
    it('should include SQLite timestamp gotcha', async () => {
      const gotchas = await loader.loadGotchas();
      const sqliteGotcha = gotchas.find(g => g.content.includes('TEXT') && g.content.includes('timestamp'));
  ...
  describe('getRelevantGotchas', () => {
    it('should filter gotchas by file pattern', () => {
      const sqlGotchas = loader.getRelevantGotchas('migrations/001.sql', 'CREATE');
    ...
    it('should filter gotchas by action type', () => {
      ...
      const updateGotchas = loader.getRelevantGotchas('database/db.ts', 'UPDATE');
  ...
  describe('load', () => {
    ...
    it('should estimate tokens correctly', async () => {
      ...
      const totalLength = context.claude.length +
