...
describe('gotcha-injector', () => {
  ...
  describe('inject', () => {
    ...
    it('should inject SQL gotchas for SQL files', () => {
      ...
      const hasParamGotcha = injectedTask.gotchas.some(g =>
    ...
    it('should process multiple tasks', () => {
      const tasks = [
        ...
        createMockTask({ id: 'T-002', file: 'server/routes/api.ts', phase: 'api' }),
  ...
  describe('action type matching', () => {
    ...
    it('should match BOTH gotchas to any action', () => {
      const createTask = createMockTask({ action: 'CREATE', file: 'database/db.ts' });
      const updateTask = createMockTask({ action: 'UPDATE', file: 'database/db.ts' });
