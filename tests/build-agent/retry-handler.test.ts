...
describe("retry-handler", () => {
  ...
  describe("withRetryForErrors", () => {
    ...
    it("should retry for specified error types", async () => {
      ...
      const result = await handler.withRetryForErrors(operation, [
        CustomError,
      ] as Array<new (...args: any[]) => Error>);
  ...
  describe("static error checks", () => {
    describe("isNetworkError", () => {
      it("should detect network errors", () => {
        expect(RetryHandler.isNetworkError(new Error("network error"))).toBe(
          true,
        );
        expect(RetryHandler.isNetworkError(new Error("ECONNREFUSED"))).toBe(
          true,
        );
        expect(RetryHandler.isNetworkError(new Error("socket hang up"))).toBe(
          true,
        );
      ...
      it("should return false for non-network errors", () => {
        expect(RetryHandler.isNetworkError(new Error("random error"))).toBe(
          false,
        );
    ...
    describe("isRateLimitError", () => {
      it("should detect rate limit errors", () => {
        expect(
          RetryHandler.isRateLimitError(new Error("rate limit exceeded")),
        ).toBe(true);
        expect(
          RetryHandler.isRateLimitError(new Error("too many requests")),
        ).toBe(true);
        expect(RetryHandler.isRateLimitError(new Error("HTTP 429"))).toBe(true);
      });

      it("should return false for non-rate-limit errors", () => {
        expect(RetryHandler.isRateLimitError(new Error("server error"))).toBe(
          false,
        );
      });
    });
    ...
    describe("isTemporaryError", () => {
      it("should detect both network and rate limit errors", () => {
        expect(RetryHandler.isTemporaryError(new Error("network error"))).toBe(
          true,
        );
        expect(RetryHandler.isTemporaryError(new Error("rate limit"))).toBe(
          true,
        );
        expect(
          RetryHandler.isTemporaryError(new Error("permanent error")),
        ).toBe(false);
      });
    });
  });
  ...
  describe("withRetry helper", () => {
    it("should succeed and return value", async () => {
      const result = await withRetry(() => Promise.resolve("value"), {
        maxRetries: 2,
        baseDelay: 10,
      });
