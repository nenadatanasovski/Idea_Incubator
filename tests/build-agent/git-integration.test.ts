...
describe('git-integration', () => {
  ...
  const mockExecSuccess = (stdout: string = '', stderr: string = '') => {
    mockExec.mockImplementation((cmd: string, opts: unknown, callback?: (err: Error | null, result: { stdout: string; stderr: string }) => void) => {
    ...
    it('should commit file successfully', async () => {
      mockExec.mockImplementation((cmd: string, opts: unknown, callback?: (err: Error | null, result: { stdout: string; stderr: string }) => void) => {
        ...
        if (cmd.includes('rev-parse --is-inside-work-tree')) {
          ...
          cb?.(null, { stdout: '[main abc1234] build(auto): complete T-001', stderr: '' });
        } else {
          cb?.(null, { stdout: '', stderr: '' });
        }
      });
  ...
  describe('commitMultiple', () => {
    it('should commit multiple files', async () => {
      mockExec.mockImplementation((cmd: string, opts: unknown, callback?: (err: Error | null, result: { stdout: string; stderr: string }) => void) => {
        ...
        if (cmd.includes('rev-parse --is-inside-work-tree')) {
          cb?.(null, { stdout: 'true', stderr: '' });
        } else if (cmd.includes('diff --cached --name-only') && !cmd.includes('"')) {
      ...
      const result = await git.commitMultiple('T-001', ['file1.ts', 'file2.ts']);
  ...
  describe('formatCommitMessage', () => {
    ...
    it('should format with custom description', () => {
      const message = git.formatCommitMessage('T-001', 'file.ts', 'add new feature');
