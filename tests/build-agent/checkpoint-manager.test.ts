...
describe('checkpoint-manager', () => {
  ...
  describe('constructor', () => {
    ...
    it('should accept custom project root', () => {
      const customManager = new CheckpointManager({ projectRoot: '/custom/root' });
  ...
  describe('save', () => {
    ...
    it('should save multiple checkpoints', () => {
      manager.save('build-001', 'T-001', createMockState({ currentTaskIndex: 1 }));
      manager.save('build-001', 'T-002', createMockState({ currentTaskIndex: 2 }));
      manager.save('build-001', 'T-003', createMockState({ currentTaskIndex: 3 }));
  ...
  describe('load', () => {
    ...
    it('should preserve state data', () => {
      ...
      expect(loadedState!.context).toEqual({ custom: 'data', nested: { value: 123 } });
    });
  });

  describe('loadById', () => {
    it('should load specific checkpoint', () => {
      const id1 = manager.save('build-001', 'T-001', createMockState({ currentTaskIndex: 1 }));
      manager.save('build-001', 'T-002', createMockState({ currentTaskIndex: 2 }));
  ...
  describe('cleanup', () => {
    it('should keep only last N checkpoints', () => {
      for (let i = 1; i <= 10; i++) {
        manager.save('build-001', `T-${String(i).padStart(3, '0')}`, createMockState({ currentTaskIndex: i }));
      }
