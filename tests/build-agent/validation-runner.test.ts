...
describe('validation-runner', () => {
  ...
  describe('checkExpected', () => {
    ...
    it('should match all tests pass', () => {
      const result = runner.checkExpected('Tests completed', 'all tests pass', 0);
      expect(result).toBe(true);
    });

    it('should match exact output', () => {
      const result = runner.checkExpected('expected output here', 'expected output', 0);
      expect(result).toBe(true);
    });
  ...
  describe('edge cases', () => {
    ...
    it('should handle special characters in command', async () => {
      const result = await runner.run('echo "hello & goodbye"', 'hello & goodbye');
