...
import { getLevelConfig, LEVEL_CONFIGS } from '../agents/validation/level-configs.js';
...
describe('Validation Agent', () => {
  ...
  describe('Result Aggregator', () => {
    ...
    const createConfig = (name: string, required: boolean): ValidatorConfig => ({
      name,
      command: '',
      args: [],
      required,
    ...
    it('should fail when required validator fails', () => {
      const results = [
        createResult('typescript', false, 1000),
      ];
      const configs = [
        createConfig('typescript', true),
      ];
  ...
  describe('Coverage Analyzer', () => {
    ...
    describe('Level Configs with Coverage', () => {
      it('THOROUGH level should include coverage validator', () => {
        const config = getLevelConfig('THOROUGH');
        const coverageValidator = config.validators.find(v => v.name === 'coverage');
