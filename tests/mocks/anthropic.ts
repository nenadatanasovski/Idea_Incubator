...
export function mockEvaluationResponse(scores: Record<string, number>): MockMessage {
  return {
    content: [{
      type: 'text',
      text: JSON.stringify({
...
export function mockArbiterResponse(verdict: 'EVALUATOR' | 'RED_TEAM' | 'DRAW'): MockMessage {
  return {
    content: [{
      type: 'text',
      text: JSON.stringify({
...
export function mockChallengeResponse(persona: string, challenge: string): MockMessage {
  return {
    content: [{
      type: 'text',
      text: JSON.stringify({
...
export function mockDevelopmentResponse(): MockMessage {
  return {
    content: [{
      ...
      text: JSON.stringify({
        questions: [
          { category: 'user', question: 'Who is the target user?', priority: 'critical' },
          { category: 'problem', question: 'What problem does this solve?', priority: 'critical' },
          { category: 'solution', question: 'How is this different from alternatives?', priority: 'important' }
        ],
...
export function mockSynthesisResponse(): MockMessage {
  return {
    content: [{
      type: 'text',
      text: JSON.stringify({
        executiveSummary: 'This is a promising idea with strong problem-solution fit.',
        ...
        recommendationReasoning: 'The idea shows promise but needs more market validation.'
      })
    }],
