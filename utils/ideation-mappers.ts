...
export function mapMessageRowToMessage(row: IdeationMessageRow): IdeationMessage {
  return {
    ...
    webSearchResults: row.web_search_results ? JSON.parse(row.web_search_results) as WebSearchResult[] : null,
    tokenCount: row.token_count,
...
export function mapMessageToRow(message: Partial<IdeationMessage>): Partial<IdeationMessageRow> {
  ...
  if (message.webSearchResults !== undefined) row.web_search_results = message.webSearchResults ? JSON.stringify(message.webSearchResults) : null;
...
export function mapMemoryToRow(memory: Partial<MemoryFile>): Partial<MemoryFileRow> {
  ...
  if (memory.createdAt !== undefined) row.created_at = memory.createdAt.toISOString();
  if (memory.updatedAt !== undefined) row.updated_at = memory.updatedAt.toISOString();
