...
async function main() {
  ...
  try {
    ...
    await session.run('MATCH (b:Block) WHERE b.id STARTS WITH "test-perf-" DELETE b');
    ...
    const blocks = Array.from({ length: 100 }, (_, i) => ({
      id: `test-perf-${i}`,
      type: ['CONTEXT', 'KNOWLEDGE', 'GOAL', 'TASK', 'DECISION', 'QUESTION', 'OBSERVATION', 'ARTIFACT', 'ARCHIVE'][i % 9],
      title: `Test Block ${i}`,
    ...
    console.log(`  ✅ Inserted 100 blocks in ${insertTime.toFixed(2)}ms (${(insertTime / 100).toFixed(2)}ms/block)\n`);
    ...
    const queries = [
      ...
      },
      {
        ...
        query: 'CALL db.index.fulltext.queryNodes("block_content_search", "testing") YIELD node RETURN node LIMIT 10',
      },
      {
        name: 'Complex filter (type + status + confidence)',
        query: 'MATCH (b:Block) WHERE b.type = "KNOWLEDGE" AND b.status = "active" AND b.confidence > 0.5 RETURN b LIMIT 10',
      },
    ];
    ...
    for (const { name, query } of queries) {
      ...
      console.log(`  ${passed ? '✅' : '❌'} ${name}: ${avgTime.toFixed(2)}ms (avg of 5) ${passed ? '' : `> ${THRESHOLD_MS}ms threshold`}`);
    }
