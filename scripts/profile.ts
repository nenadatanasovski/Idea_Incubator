...
function rowToProfile(row: ProfileRow): UserProfile {
  return {
    ...
    industryConnections: safeParseObjectArray<IndustryConnection>(
      row.industry_connections,
    ),
...
export async function updateProfile(
  ...
  if (updates.primaryGoals !== undefined)
    data.primary_goals = JSON.stringify(updates.primaryGoals);
  if (updates.successDefinition !== undefined)
    data.success_definition = updates.successDefinition;
  if (updates.interests !== undefined)
    data.interests = JSON.stringify(updates.interests);
  if (updates.motivations !== undefined) data.motivations = updates.motivations;
  if (updates.domainConnection !== undefined)
    data.domain_connection = updates.domainConnection;
  if (updates.technicalSkills !== undefined)
    data.technical_skills = JSON.stringify(updates.technicalSkills);
  if (updates.professionalExperience !== undefined)
    data.professional_experience = updates.professionalExperience;
  if (updates.domainExpertise !== undefined)
    data.domain_expertise = JSON.stringify(updates.domainExpertise);
  if (updates.knownGaps !== undefined) data.known_gaps = updates.knownGaps;
  if (updates.industryConnections !== undefined)
    data.industry_connections = JSON.stringify(updates.industryConnections);
  if (updates.professionalNetwork !== undefined)
    data.professional_network = updates.professionalNetwork;
  if (updates.communityAccess !== undefined)
    data.community_access = JSON.stringify(updates.communityAccess);
  if (updates.partnershipPotential !== undefined)
    data.partnership_potential = updates.partnershipPotential;
  if (updates.employmentStatus !== undefined)
    data.employment_status = updates.employmentStatus;
  if (updates.weeklyHoursAvailable !== undefined)
    data.weekly_hours_available = updates.weeklyHoursAvailable;
  if (updates.financialRunwayMonths !== undefined)
    data.financial_runway_months = updates.financialRunwayMonths;
  if (updates.riskTolerance !== undefined)
    data.risk_tolerance = updates.riskTolerance;
  if (updates.otherCommitments !== undefined)
    data.other_commitments = updates.otherCommitments;
...
export async function linkProfileToIdea(
  ...
  if (existing) {
    ...
    await update(
      "idea_profiles",
      {
        goals_override: overrides?.goalsOverride
          ? JSON.stringify(overrides.goalsOverride)
          : null,
        passion_notes: overrides?.passionNotes ?? null,
        relevant_skills: overrides?.relevantSkills
          ? JSON.stringify(overrides.relevantSkills)
          : null,
        relevant_network: overrides?.relevantNetwork ?? null,
        time_commitment: overrides?.timeCommitment ?? null,
        linked_at: now,
      },
      "idea_id = ? AND profile_id = ?",
      [ideaId, profileId],
    );
  } else {
    // Create new link
...
export async function getIdeaProfile(ideaId: string): Promise<{
  ...
  return {
    profile,
    overrides: {
      goalsOverride: link.goals_override
        ? JSON.parse(link.goals_override)
        : undefined,
      passionNotes: link.passion_notes ?? undefined,
      relevantSkills: link.relevant_skills
        ? JSON.parse(link.relevant_skills)
        : undefined,
...
function formatNetworkContext(
  ...
  if (industryConnections.length > 0) {
    const formatted = industryConnections
      .map(
        (ic) =>
 ...
 */
export async function getEvaluationProfileContext(
  ideaId: string,
...
export async function captureProfileInteractive(): Promise<UserProfile> {
  ...
  try {
    ...
    console.log(
      "This profile will be used to evaluate Personal Fit (FT1-FT5) criteria.\n",
    );
    ...
    const successDefinition = await prompt(
      rl,
      "What does success look like for you? (optional): ",
    );

    // FT2: Passion
    console.log("\n--- Passion & Motivation (FT2) ---");
    const interestsInput = await prompt(
      rl,
      "Areas of interest (comma-separated, optional): ",
    );
    ...
    const skillsInput = await prompt(
      rl,
      "Technical skills (comma-separated, optional): ",
    );
    const technicalSkills = skillsInput
      ? skillsInput
          .split(",")
          .map((s) => s.trim())
          .filter(Boolean)
    ...
    const professionalExperience = await prompt(
      rl,
      "Professional experience summary (optional): ",
    );

    const expertiseInput = await prompt(
      rl,
      "Domain expertise areas (comma-separated, optional): ",
    );
    const domainExpertise = expertiseInput
      ? expertiseInput
          .split(",")
          .map((s) => s.trim())
          .filter(Boolean)
    ...
    const knownGaps = await prompt(
      rl,
      "What skill gaps do you have? (optional): ",
    );

    // FT4: Network
    console.log("\n--- Network & Connections (FT4) ---");
    const professionalNetwork = await prompt(
      rl,
      "Describe your professional network (optional): ",
    );

    const communitiesInput = await prompt(
      rl,
      "Communities you have access to (comma-separated, optional): ",
    );
    const communityAccess = communitiesInput
      ? communitiesInput
          .split(",")
          .map((s) => s.trim())
          .filter(Boolean)
    ...
    const hoursInput = await prompt(
      rl,
      "Hours per week available for side projects (optional): ",
    );
    const weeklyHoursAvailable = hoursInput
      ? parseInt(hoursInput, 10)
      : undefined;

    const runwayInput = await prompt(
      rl,
      "Financial runway in months (optional): ",
    );
    const financialRunwayMonths = runwayInput
      ? parseInt(runwayInput, 10)
      : undefined;
    ...
    const otherCommitments = await prompt(
      rl,
      "Other commitments (family, projects, etc., optional): ",
    );
    ...
    switch (command) {
      ...
      case "list": {
        ...
        if (profiles.length === 0) {
          console.log(
            'No profiles found. Run "npm run profile create" to create one.',
          );
        } else {
          console.log("\n=== User Profiles ===\n");
          for (const p of profiles) {
            console.log(
              `- ${p.name} (${p.slug}): ${p.primaryGoals.join(", ")}`,
            );
          }
        }
        break;
      ...
      default:
        ...
        console.log(
          "  create              Create a new user profile interactively",
        );
