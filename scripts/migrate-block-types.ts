...
async function migrateBlockTypes(dryRun: boolean, verbose: boolean): Promise<MigrationStats> {
  ...
  console.log(`Junction table: ${junctionMigrated} migrated, ${junctionRemoved} removed`);
...
function printStats(stats: MigrationStats): void {
  ...
  for (const [type, count] of sortedOld) {
    const newType = TYPE_MAPPING[type] || (TYPES_TO_REMOVE.includes(type) ? 'REMOVED' : 'UNKNOWN');
...
async function validateMigration(): Promise<boolean> {
  ...
  const oldTypes = Object.keys(TYPE_MAPPING).filter(t => !VALID_NEW_TYPES.includes(t));
  ...
  for (const oldType of oldTypes) {
    ...
    if (remaining[0]?.count > 0) {
      console.error(`❌ Found ${remaining[0].count} blocks still with type: ${oldType}`);
      return false;
    }
  }
