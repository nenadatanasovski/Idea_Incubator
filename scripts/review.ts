...
program
  ...
  .action(async (slug, options) => {
    try {
      ...
      console.log("\n--- Review Mode ---");
      console.log(
        "For each criterion, press Enter to accept AI score, or type a new score (1-10).",
      );
      ...
      for (const score of scores) {
        ...
        console.log(
          `  AI Score: ${score.final_score}/10 (${(score.confidence * 100).toFixed(0)}% confidence)`,
        );
 ...
 */
async function recalculateOverallScore(
  sessionId: string,
  _ideaId: string,
): Promise<void> {
  ...
  // Update session
  await run("UPDATE evaluation_sessions SET overall_score = ? WHERE id = ?", [
    overall,
    sessionId,
  ]);
  ...
  logInfo(
    `Recommendation: ${interpretation.recommendation} - ${interpretation.description}`,
  );
}
...
program
  ...
  .action(async (slug1, slug2) => {
    try {
      ...
      console.log(
        `|-----------|${"-".repeat(idea1.title.length + 2)}|${"-".repeat(idea2.title.length + 2)}|------|`,
      );

      for (const score1 of idea1.scores) {
        const score2 = idea2.scores.find(
          (s) => s.criterion_name === score1.criterion_name,
        );
        const s2 = score2?.final_score ?? "-";
        const diff =
          typeof s2 === "number" ? (score1.final_score - s2).toFixed(1) : "-";
        const diffStr =
        ...
        console.log(
          `| ${score1.criterion_name} | ${score1.final_score} | ${s2} | ${diffStr} |`,
        );
      }
      ...
      for (const category of CATEGORIES) {
        ...
        const avg1 =
          cat1Scores.reduce((sum, s) => sum + s.final_score, 0) /
          cat1Scores.length;
        const avg2 =
          cat2Scores.reduce((sum, s) => sum + s.final_score, 0) /
          cat2Scores.length;

        console.log(
          `${category}: ${avg1.toFixed(1)} vs ${avg2.toFixed(1)} (${avg1 > avg2 ? slug1 : slug2} leads)`,
        );
      }
