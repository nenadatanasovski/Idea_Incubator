...
async function verify(): Promise<void> {
  ...
  try {
    const { stdout } = await execAsync(
      "npx vitest run tests/spec-agent/acceptance.test.ts",
      {
        timeout: 60000,
      },
    );
  ...
  for (const refPath of REFERENCE_SPECS) {
    ...
    if (!specExists || !tasksExists || !briefExists) {
      refSpecsValid = false;
      console.log(
        `  ✗ ${refPath}: missing ${!specExists ? "spec.md " : ""}${!tasksExists ? "tasks.md " : ""}${!briefExists ? "brief.md" : ""}`,
      );
    } else {
      console.log(`  ✓ ${refPath}`);
    }
  ...
  results.push({
    ...
    message: refSpecsValid
      ? "All reference specs present"
      : "Missing reference specs",
  });
  ...
  for (const agentPath of AGENT_SPECS) {
    ...
    if (!briefExists) {
      ...
      console.log(
        `  ✗ ${agentPath}: missing ${!specExists ? "spec.md " : ""}${!tasksExists ? "tasks.md" : ""}`,
      );
    } else {
  ...
  results.push({
    ...
    message: agentSpecsValid
      ? "All agent specs present"
      : "Missing agent specs",
  });
  ...
  if (fs.existsSync(specAgentSpec) && fs.existsSync(specAgentTasks)) {
    ...
    const hasArchitecture =
      ...
      specContent.includes("System Context");
    const hasRequirements =
      specContent.includes("Functional Requirements") ||
      specContent.includes("FR-");
    const hasTasks =
      tasksContent.includes("T-001") || tasksContent.includes("T-");
