...
program.name("idea-cli").description("Idea Incubator CLI").version("0.1.0");
...
program
  ...
  .action(async (options) => {
    try {
      ...
      let sql =
        "SELECT slug, title, lifecycle_stage, idea_type FROM ideas WHERE 1=1";
      const params: string[] = [];
      ...
      ideas.forEach((idea) => {
        ...
        console.log(
          `    Stage: ${idea.lifecycle_stage} | Type: ${idea.idea_type || "unset"}`,
        );
        console.log("");
      });
  ...
  .action(async (slug) => {
    try {
      ...
      if (scores.length > 0) {
        ...
        scores.forEach((s) => {
          console.log(
            `  ${s.category}: ${s.avg_score?.toFixed(1) || "-"} (${(s.avg_confidence * 100)?.toFixed(0) || "-"}%)`,
          );
        });
      }
...
program
  ...
  .action(async (slug, options) => {
    try {
      ...
      }>("SELECT id, slug, current_version FROM ideas WHERE slug = ?", [slug]);
...
program
  ...
  .option(
    "-a, --action <action>",
    "What to do with parent: keep, pause, abandon",
    "keep",
  )
  .action(async (slug, options) => {
    try {
