...
export async function captureIdea(): Promise<string> {
  ...
  try {
    ...
    const finalContent = await Promise.race([
      contentPromise,
      new Promise<string>((resolve) => setTimeout(() => resolve(content.trim()), 100))
    ]);
    ...
    console.log('3. Develop the idea: Ask Claude to "develop" or "flesh out" the idea');
 ...
 */
export async function captureFromArgs(title: string, type: IdeaType, summary?: string): Promise<string> {
  const slug = titleToSlug(title);
  const id = uuidv4();
