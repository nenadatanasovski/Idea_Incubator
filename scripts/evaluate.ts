...
import { determineEvaluationFlow, estimateEvaluationCost, formatCostEstimate } from '../agents/orchestrator.js';
import { CATEGORIES, interpretScore } from '../agents/config.js';
import { runFullDebate, formatDebateResults, type FullDebateResult } from '../agents/debate.js';
...
import { conductPreEvaluationResearch, shouldSkipResearch, type ResearchResult, type CreatorLocation } from '../agents/research.js';
...
program
  ...
  .action(async (slug, options) => {
    ...
    try {
      ...
      if (flowState.hasExistingEvaluation && !flowState.isStale && !options.force) {
        logWarning(`Idea was already evaluated on ${flowState.lastEvaluatedAt}`);
      ...
      if (flowState.isStale) {
        logWarning('Idea has been modified since last evaluation. Re-evaluating...');
      }
      ...
      if (costEstimate.total > budget) {
        logWarning(`Estimated cost ($${costEstimate.total.toFixed(2)}) exceeds budget ($${budget.toFixed(2)})`);
        if (!options.force) {
      ...
      const ideaContent = fs.readFileSync(readmePath, 'utf-8');
      ...
      if (profileContext) {
        ...
        logWarning('No user profile linked - Personal Fit scores will have low confidence');
        logInfo('Link a profile with: npm run profile link ' + slug + ' <profile-slug>');
      }
      ...
      if (structuredContext && structuredContext.coverage.overall > 0) {
        ...
        logInfo('No structured answers available - evaluation will rely on idea content');
      ...
      if (!shouldSkipResearch()) {
        ...
        if (profileContext?.profile?.country) {
          ...
          logInfo(`Creator location: ${creatorLocation.city ? creatorLocation.city + ', ' : ''}${creatorLocation.country}`);
        }

        try {
          const userClaims = await extractClaimsFromContent(ideaContent, costTracker);
          logInfo(`Extracted claims: domain="${userClaims.domain}", ${userClaims.competitors.length} competitors, tech: ${userClaims.technology.join(', ')}`);

          research = await conductPreEvaluationResearch(ideaContent, userClaims, costTracker, creatorLocation);
          ...
          if (research.geographicAnalysis) {
            ...
            if (geo.localMarket?.marketSize.tam) {
              logInfo(`Local market (${geo.creatorLocation?.country || 'Unknown'}): TAM ${geo.localMarket.marketSize.tam}`);
            }
      ...
      console.log(`- Tokens: ${costReport.inputTokens.toLocaleString()} input, ${costReport.outputTokens.toLocaleString()} output`);
      ...
      if (ideaData.lifecycle_stage === 'SPARK') {
        await run(
          'UPDATE ideas SET lifecycle_stage = ? WHERE id = ?',
          ['EVALUATE', ideaData.id]
        );
      ...
      try {
        ...
        await errorBroadcaster.error((error as Error).message || 'Evaluation failed');
      } catch {
...
async function getStructuredContext(ideaId: string): Promise<StructuredEvaluationContext | null> {
  try {
    ...
    const questionIdMapping: Record<string, (answer: string) => void> = {
      ...
      'P1_CORE': (a) => { structuredAnswers.problem = structuredAnswers.problem || {}; structuredAnswers.problem.core_problem = a; },
      'P1_SCOPE': (a) => { structuredAnswers.problem = structuredAnswers.problem || {}; structuredAnswers.problem.problem_scope = a; },
      'P1_WHEN': (a) => { structuredAnswers.problem = structuredAnswers.problem || {}; structuredAnswers.problem.problem_triggers = a; },

      // Problem category - P2 (Problem Severity)
      'P2_PAIN': (a) => { structuredAnswers.problem = structuredAnswers.problem || {}; structuredAnswers.problem.pain_severity = a; },
      'P2_COST': (a) => { structuredAnswers.problem = structuredAnswers.problem || {}; structuredAnswers.problem.pain_cost = a; },
      'P2_FREQUENCY': (a) => { structuredAnswers.problem = structuredAnswers.problem || {}; structuredAnswers.problem.pain_frequency = a; },
      'P2_ALTERNATIVES': (a) => { structuredAnswers.problem = structuredAnswers.problem || {}; structuredAnswers.problem.current_workarounds = a; },

      // Problem category - P3 (Target User)
      'P3_WHO': (a) => { structuredAnswers.problem = structuredAnswers.problem || {}; structuredAnswers.problem.target_user = a; },
      'P3_SEGMENT': (a) => { structuredAnswers.problem = structuredAnswers.problem || {}; structuredAnswers.problem.user_segment = a; },
      'P3_SIZE': (a) => { structuredAnswers.problem = structuredAnswers.problem || {}; structuredAnswers.problem.user_size = a; },
      'P3_ACCESS': (a) => { structuredAnswers.problem = structuredAnswers.problem || {}; structuredAnswers.problem.user_access = a; },

      // Problem category - P4 (Problem Validation)
      'P4_EVIDENCE': (a) => { structuredAnswers.problem = structuredAnswers.problem || {}; structuredAnswers.problem.validation = a; },
      'P4_CONVERSATIONS': (a) => { structuredAnswers.problem = structuredAnswers.problem || {}; structuredAnswers.problem.user_conversations = a; },
      'P4_WILLINGNESS': (a) => { structuredAnswers.problem = structuredAnswers.problem || {}; structuredAnswers.problem.willingness_to_pay = a; },

      // Problem category - P5 (Problem Uniqueness)
      'P5_EXISTING': (a) => { structuredAnswers.problem = structuredAnswers.problem || {}; structuredAnswers.problem.existing_solutions = a; },
      'P5_GAP': (a) => { structuredAnswers.problem = structuredAnswers.problem || {}; structuredAnswers.problem.solution_gaps = a; },
      'P5_ANGLE': (a) => { structuredAnswers.problem = structuredAnswers.problem || {}; structuredAnswers.problem.unique_angle = a; },

      // Solution category - S1 (Solution Clarity)
      'S1_WHAT': (a) => { structuredAnswers.solution = structuredAnswers.solution || {}; structuredAnswers.solution.description = a; },
      'S1_VALUE_PROP': (a) => { structuredAnswers.solution = structuredAnswers.solution || {}; structuredAnswers.solution.value_proposition = a; },
      'S1_HOW': (a) => { structuredAnswers.solution = structuredAnswers.solution || {}; structuredAnswers.solution.user_flow = a; },

      // Solution category - S2 (Solution Feasibility)
      'S2_TECH': (a) => { structuredAnswers.solution = structuredAnswers.solution || {}; structuredAnswers.solution.technology = a; },
      'S2_PROVEN': (a) => { structuredAnswers.solution = structuredAnswers.solution || {}; structuredAnswers.solution.proven_approach = a; },
      'S2_HARD': (a) => { structuredAnswers.solution = structuredAnswers.solution || {}; structuredAnswers.solution.hard_parts = a; },

      // Solution category - S3 (Solution Uniqueness)
      'S3_DIFF': (a) => { structuredAnswers.solution = structuredAnswers.solution || {}; structuredAnswers.solution.differentiation = a; },
      'S3_WHY_BETTER': (a) => { structuredAnswers.solution = structuredAnswers.solution || {}; structuredAnswers.solution.why_better = a; },
      'S3_SECRET': (a) => { structuredAnswers.solution = structuredAnswers.solution || {}; structuredAnswers.solution.secret_insight = a; },

      // Solution category - S4 (Solution Scalability)
      'S4_SCALE': (a) => { structuredAnswers.solution = structuredAnswers.solution || {}; structuredAnswers.solution.scale_bottlenecks = a; },
      'S4_MARGINAL': (a) => { structuredAnswers.solution = structuredAnswers.solution || {}; structuredAnswers.solution.marginal_costs = a; },

      // Solution category - S5 (Solution Defensibility)
      'S5_MOAT': (a) => { structuredAnswers.solution = structuredAnswers.solution || {}; structuredAnswers.solution.moat = a; },
      'S5_PROTECTION': (a) => { structuredAnswers.solution = structuredAnswers.solution || {}; structuredAnswers.solution.ip_protection = a; },
      'S5_NETWORK': (a) => { structuredAnswers.solution = structuredAnswers.solution || {}; structuredAnswers.solution.network_effects = a; },

      // Feasibility category - F1 (Technical Complexity)
      'F1_MVP': (a) => { structuredAnswers.feasibility = structuredAnswers.feasibility || {}; structuredAnswers.feasibility.mvp = a; },
      'F1_COMPONENTS': (a) => { structuredAnswers.feasibility = structuredAnswers.feasibility || {}; structuredAnswers.feasibility.components = a; },
      'F1_UNKNOWNS': (a) => { structuredAnswers.feasibility = structuredAnswers.feasibility || {}; structuredAnswers.feasibility.unknowns = a; },

      // Feasibility category - F2 (Resource Requirements)
      'F2_COST': (a) => { structuredAnswers.feasibility = structuredAnswers.feasibility || {}; structuredAnswers.feasibility.cost_estimate = a; },
      'F2_TEAM': (a) => { structuredAnswers.feasibility = structuredAnswers.feasibility || {}; structuredAnswers.feasibility.team_requirements = a; },
      'F2_TOOLS': (a) => { structuredAnswers.feasibility = structuredAnswers.feasibility || {}; structuredAnswers.feasibility.tools_needed = a; },

      // Feasibility category - F3 (Skill Availability)
      'F3_GAP': (a) => { structuredAnswers.feasibility = structuredAnswers.feasibility || {}; structuredAnswers.feasibility.skill_gaps = a; },
      'F3_ACQUIRE': (a) => { structuredAnswers.feasibility = structuredAnswers.feasibility || {}; structuredAnswers.feasibility.skill_acquisition = a; },

      // Feasibility category - F4 (Time to Value)
      'F4_FIRST_VALUE': (a) => { structuredAnswers.feasibility = structuredAnswers.feasibility || {}; structuredAnswers.feasibility.time_to_feedback = a; },
      'F4_FIRST_REVENUE': (a) => { structuredAnswers.feasibility = structuredAnswers.feasibility || {}; structuredAnswers.feasibility.time_to_revenue = a; },

      // Feasibility category - F5 (Dependency Risk)
      'F5_DEPS': (a) => { structuredAnswers.feasibility = structuredAnswers.feasibility || {}; structuredAnswers.feasibility.dependencies = a; },
      'F5_CONTROL': (a) => { structuredAnswers.feasibility = structuredAnswers.feasibility || {}; structuredAnswers.feasibility.dependency_control = a; },

      // Market category - M1 (Market Size)
      'M1_TAM': (a) => { structuredAnswers.market = structuredAnswers.market || {}; structuredAnswers.market.tam = a; },
      'M1_SAM': (a) => { structuredAnswers.market = structuredAnswers.market || {}; structuredAnswers.market.sam = a; },
      'M1_SOM': (a) => { structuredAnswers.market = structuredAnswers.market || {}; structuredAnswers.market.som = a; },

      // Market category - M2 (Market Growth)
      'M2_TREND': (a) => { structuredAnswers.market = structuredAnswers.market || {}; structuredAnswers.market.trends = a; },
      'M2_DRIVERS': (a) => { structuredAnswers.market = structuredAnswers.market || {}; structuredAnswers.market.trend_drivers = a; },

      // Market category - M3 (Competition)
      'M3_COMPETITORS': (a) => { structuredAnswers.market = structuredAnswers.market || {}; structuredAnswers.market.competitors = a; },
      'M3_LANDSCAPE': (a) => { structuredAnswers.market = structuredAnswers.market || {}; structuredAnswers.market.competitive_landscape = a; },
      'M3_COMP_WEAKNESS': (a) => { structuredAnswers.market = structuredAnswers.market || {}; structuredAnswers.market.competitor_weaknesses = a; },

      // Market category - M4 (Entry Barriers)
      'M4_BARRIERS': (a) => { structuredAnswers.market = structuredAnswers.market || {}; structuredAnswers.market.barriers = a; },
      'M4_OVERCOME': (a) => { structuredAnswers.market = structuredAnswers.market || {}; structuredAnswers.market.barrier_strategy = a; },

      // Market category - M5 (Timing)
      'M5_WHY_NOW': (a) => { structuredAnswers.market = structuredAnswers.market || {}; structuredAnswers.market.timing = a; },
      'M5_CATALYST': (a) => { structuredAnswers.market = structuredAnswers.market || {}; structuredAnswers.market.timing_catalyst = a; },

      // Risk category
      'R_BIGGEST': (a) => { structuredAnswers.risk = structuredAnswers.risk || {}; structuredAnswers.risk.biggest_risk = a; },
      'R_MITIGATION': (a) => { structuredAnswers.risk = structuredAnswers.risk || {}; structuredAnswers.risk.mitigation = a; },
      'R_EXECUTION': (a) => { structuredAnswers.risk = structuredAnswers.risk || {}; structuredAnswers.risk.execution_risk = a; },
      'R_MARKET': (a) => { structuredAnswers.risk = structuredAnswers.risk || {}; structuredAnswers.risk.market_risk = a; },
      'R_TECHNICAL': (a) => { structuredAnswers.risk = structuredAnswers.risk || {}; structuredAnswers.risk.technical_risk = a; },
      'R_FINANCIAL': (a) => { structuredAnswers.risk = structuredAnswers.risk || {}; structuredAnswers.risk.financial_risk = a; },
      'R_REGULATORY': (a) => { structuredAnswers.risk = structuredAnswers.risk || {}; structuredAnswers.risk.regulatory_risk = a; },
      'R_KILL': (a) => { structuredAnswers.risk = structuredAnswers.risk || {}; structuredAnswers.risk.kill_conditions = a; },
      'R_PREMORTEM': (a) => { structuredAnswers.risk = structuredAnswers.risk || {}; structuredAnswers.risk.premortem = a; },

      // Business model category
      'BM_MODEL': (a) => { structuredAnswers.business_model = structuredAnswers.business_model || {}; structuredAnswers.business_model.revenue_model = a; },
      'BM_PRICE': (a) => { structuredAnswers.business_model = structuredAnswers.business_model || {}; structuredAnswers.business_model.pricing = a; },
      'BM_CAC': (a) => { structuredAnswers.business_model = structuredAnswers.business_model || {}; structuredAnswers.business_model.cac = a; },
      'BM_LTV': (a) => { structuredAnswers.business_model = structuredAnswers.business_model || {}; structuredAnswers.business_model.ltv = a; },
      'BM_GTM': (a) => { structuredAnswers.business_model = structuredAnswers.business_model || {}; structuredAnswers.business_model.gtm = a; },
      'BM_REVENUE': (a) => { structuredAnswers.business_model = structuredAnswers.business_model || {}; structuredAnswers.business_model.revenue_projection = a; },

      // Fit category - FT1 (Personal Fit / Goal Alignment)
      'FT1_GOALS': (a) => { structuredAnswers.fit = structuredAnswers.fit || {}; structuredAnswers.fit.goal_alignment = a; },
      'FT1_WHY_THIS': (a) => { structuredAnswers.fit = structuredAnswers.fit || {}; structuredAnswers.fit.why_this_idea = a; },
      'FT1_SUCCESS': (a) => { structuredAnswers.fit = structuredAnswers.fit || {}; structuredAnswers.fit.success_definition = a; },

      // Fit category - FT2 (Passion Alignment)
      'FT2_PASSION': (a) => { structuredAnswers.fit = structuredAnswers.fit || {}; structuredAnswers.fit.passion = a; },
      'FT2_EXPERIENCE': (a) => { structuredAnswers.fit = structuredAnswers.fit || {}; structuredAnswers.fit.personal_experience = a; },
      'FT2_LONG_TERM': (a) => { structuredAnswers.fit = structuredAnswers.fit || {}; structuredAnswers.fit.long_term_interest = a; },

      // Fit category - FT3 (Skill Match)
      'FT3_SKILLS': (a) => { structuredAnswers.fit = structuredAnswers.fit || {}; structuredAnswers.fit.skills = a; },
      'FT3_UNIQUE': (a) => { structuredAnswers.fit = structuredAnswers.fit || {}; structuredAnswers.fit.unique_advantage = a; },
      'FT3_LEARN': (a) => { structuredAnswers.fit = structuredAnswers.fit || {}; structuredAnswers.fit.skills_to_learn = a; },

      // Fit category - FT4 (Network Leverage)
      'FT4_NETWORK': (a) => { structuredAnswers.fit = structuredAnswers.fit || {}; structuredAnswers.fit.network = a; },
      'FT4_ACCESS': (a) => { structuredAnswers.fit = structuredAnswers.fit || {}; structuredAnswers.fit.customer_access = a; },
      'FT4_COMMUNITY': (a) => { structuredAnswers.fit = structuredAnswers.fit || {}; structuredAnswers.fit.community = a; },

      // Fit category - FT5 (Life Stage Fit)
      'FT5_TIMING': (a) => { structuredAnswers.fit = structuredAnswers.fit || {}; structuredAnswers.fit.life_timing = a; },
      'FT5_CAPACITY': (a) => { structuredAnswers.fit = structuredAnswers.fit || {}; structuredAnswers.fit.time_capacity = a; },
      'FT5_RUNWAY': (a) => { structuredAnswers.fit = structuredAnswers.fit || {}; structuredAnswers.fit.financial_runway = a; },
      'FT5_RISK': (a) => { structuredAnswers.fit = structuredAnswers.fit || {}; structuredAnswers.fit.risk_tolerance = a; },
    };
...
async function saveEvaluationResults(
  ...
  await run(
    ...
    [
      ...
      (result.tokensUsed.input * 15 + result.tokensUsed.output * 75) / 1_000_000
    ]
  );
...
async function saveDebateResults(
  ...
  for (const debate of debateResult.debates) {
    ...
    for (let roundIdx = 0; roundIdx < debate.rounds.length; roundIdx++) {
      ...
      for (let challengeIdx = 0; challengeIdx < round.verdicts.length; challengeIdx++) {
    ...
    for (const challenge of debate.challenges) {
      await run(
        ...
        [
          ...
          challenge.persona,
          challenge.challenge,
          ['CRITICAL', 'MAJOR', 'MINOR', 'ADDRESSED'].includes(challenge.severity) ? challenge.severity : 'MAJOR',
          new Date().toISOString()
        ]
      );
    }
...
async function saveFinalSynthesis(
  ...
  for (const debate of debateResult.debates) {
    if (debate.finalScore >= 7) {
      keyStrengths.push(`${debate.criterion.name}: Strong (${debate.finalScore}/10)`);
    } else if (debate.finalScore <= 4) {
      keyWeaknesses.push(`${debate.criterion.name}: Weak (${debate.finalScore}/10)`);
    }

    // Add insights as assumptions/questions
    for (const insight of debate.summary.keyInsights) {
      if (insight.toLowerCase().includes('assume') || insight.toLowerCase().includes('if ')) {
  ...
  const survivalRate = totalChallenges > 0 ? survivedChallenges / totalChallenges : 0;
  ...
  const executiveSummary = `Evaluation completed with overall score of ${finalScore.toFixed(1)}/10. ` +
...
async function generateEvaluationFile(
  ...
  for (const category of CATEGORIES) {
    ...
    if (catEvals.length > 0) {
      const avgScore = catEvals.reduce((sum, e) => sum + e.score, 0) / catEvals.length;
      const avgConf = catEvals.reduce((sum, e) => sum + e.confidence, 0) / catEvals.length;
      lines.push(`| ${category.charAt(0).toUpperCase() + category.slice(1)} | ${avgScore.toFixed(1)}/10 | ${(avgConf * 100).toFixed(0)}% |`);
    }
  }
  ...
  if (debateResult) {
    ...
    lines.push(`- **Initial Score:** ${debateResult.overallOriginalScore.toFixed(1)}/10`);
    lines.push(`- **Final Score:** ${debateResult.overallFinalScore.toFixed(1)}/10`);
    lines.push(`- **Score Change:** ${(debateResult.overallFinalScore - debateResult.overallOriginalScore) >= 0 ? '+' : ''}${(debateResult.overallFinalScore - debateResult.overallOriginalScore).toFixed(1)}`, '');
    ...
    if (significantChanges.length > 0) {
      ...
      for (const debate of significantChanges) {
        ...
        lines.push(`- **${debate.criterion.name}:** ${debate.originalScore} → ${debate.finalScore} (${emoji}${change.toFixed(1)})`);
      }
      lines.push('');
    }
  ...
  if (research && research.searchesPerformed > 0) {
    ...
    if (research.marketSize.verified) {
      ...
      lines.push(`**Verified Market Size:** ${research.marketSize.verified}`, '');
...
async function showExistingEvaluation(ideaId: string): Promise<void> {
  ...
  for (const score of scores) {
    ...
    console.log(`  ${score.criterion_name}: ${score.final_score}/10 (${confidence}% confidence)`);
  }

  // Calculate and show overall
  const categoryAvgs = CATEGORIES.map(cat => {
    const catScores = scores.filter(s => s.category === cat);
    return catScores.reduce((sum, s) => sum + s.final_score, 0) / catScores.length;
  });

  const config = getConfig();
  const weights = Object.values(config.categoryWeights);
  const overall = categoryAvgs.reduce((sum, avg, i) => sum + avg * weights[i], 0);
