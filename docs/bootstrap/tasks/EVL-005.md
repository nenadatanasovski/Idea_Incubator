...
async function scoreFitCriteria(
  ...
  if (!profile) {
    return {
      ...
      confidence: 0.2,
      rationale:
        "No user profile available. Create a profile for accurate Fit scoring.",
      evidence: [],
    };
  }
  ...
  switch (criterion) {
    case "FT1":
      return scoreGoalAlignment(ideaContext, profile);
    case "FT2":
      return scorePassionMatch(ideaContext, profile);
    // ... etc
  }
}
```
