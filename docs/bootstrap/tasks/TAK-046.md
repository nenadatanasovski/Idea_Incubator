...
export class CrossProjectDeps {
  ...
  async createCrossProjectLink(
    ...
    const sourceTask = db.prepare('SELECT project_id FROM tasks WHERE id = ?').get(sourceTaskId) as { project_id: string };
    const targetTask = db.prepare('SELECT project_id FROM tasks WHERE id = ?').get(targetTaskId) as { project_id: string };
  ...
  private estimateDelay(taskId: string): number {
    ...
    const task = db.prepare('SELECT estimated_effort FROM tasks WHERE id = ?').get(taskId) as any;
  ...
  async generateDependencyGraph(projectIds?: string[]): Promise<DependencyGraph> {
    // Get projects
    ...
    const projects = db.prepare(projectQuery).all(projectIds || []) as { project_id: string }[];
  ...
  private async calculateCriticalPath(links: CrossProjectLink[]): Promise<string[]> {
