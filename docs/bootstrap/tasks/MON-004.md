...
export class StateReconciler {
  reconcile(
    ...
    const divergenceCount = agents.reduce(
      (sum, a) => sum + a.divergences.length, 0
    );
    const overallConfidence = agents.reduce(
  ...
  private reconcileAgent(
    ...
    const agreement = divergences.length === 0 ? 'full' :
                     divergences.every(d => d.severity === 'info') ? 'partial' :
                     'divergent';
  ...
  private diagnoseDivergence(eb: AgentState, ui: UIAgentState): Divergence['possibleCause'] {
