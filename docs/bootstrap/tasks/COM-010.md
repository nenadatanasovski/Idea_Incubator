...
export class NotificationDispatcher extends EventEmitter {
  ...
  async dispatch(question: Question): Promise<DispatchResult> {
    ...
    // Step 1: Try Telegram
    const telegramResult = await this.telegramDelivery.deliverQuestion(
      question as any,
    );
    ...
    if (telegramResult.success) {
      ...
      console.warn(
        `Telegram delivery failed for ${question.id}: ${telegramResult.error}`,
      );
      ...
      if (emailResult.success) {
        ...
        console.error(
          `Email delivery also failed for ${question.id}: ${emailResult.error}`,
        );
      }
    }
  ...
  private async handleDeliveryFailure(
    ...
    switch (config.fallbackBehavior) {
      case "HARD_HALT":
        console.error(
          `HARD HALT: Question ${question.id} could not be delivered. Agent will block.`,
        );
      ...
      case "TIMEOUT":
        console.warn(
          `Delivery failed for ${question.id}. Will timeout to default.`,
        );
      ...
      case "LOG_ONLY":
        console.log(
          `Alert ${question.id} could not be delivered. Logged only.`,
        );
        break;
    }
  }
  ...
  private setupRetryNotifications(question: Question, interval: number): void {
    const timer = setInterval(async () => {
      ...
      console.error(
        `Retry failed for ${question.id}. Will try again in ${interval / 60000} minutes.`,
      );
    }, interval);
