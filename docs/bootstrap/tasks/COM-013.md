...
const TEMPLATES: Record<QuestionType, Template> = {
  ...
  BLOCKING: {
    ...
    standard: '{{emoji}} *{{label}}*\n\n{{content}}\n\n{{options}}\n\n⏸️ _Agent blocked: {{agentId}}_',
    detailed: '{{emoji}} *{{label}}*\n\n{{content}}\n\n{{options}}\n\n*Context:*\n{{context}}\n\n⏸️ _Agent blocked: {{agentId}}_\n💡 _Default on timeout: {{default}}_',
  ...
  DECISION: {
    ...
    concise: '{{emoji}} {{content}}',
    standard: '{{emoji}} *{{label}}*\n\n{{content}}\n\n{{options}}\n\n_From: {{agentId}}_',
    detailed: '{{emoji}} *{{label}}*\n\n{{content}}\n\n{{options}}\n\n*Trade-offs:*\n{{context}}\n\n_From: {{agentId}} | Timeout: {{timeout}} → {{default}}_',
    urgent: '{{emoji}}{{emoji}} *DECISION REQUIRED* {{emoji}}{{emoji}}\n\n{{content}}\n\n{{options}}',
  },
  ...
  APPROVAL: {
    ...
    standard: '{{emoji}} *{{label}}*\n\n{{content}}\n\n{{options}}\n\n⛔ _Requires your approval_\n_From: {{agentId}}_',
    detailed: '{{emoji}} *{{label}}*\n\n{{content}}\n\n*Rationale:*\n{{rationale}}\n\n*Evidence:*\n```\n{{evidence}}\n```\n\n{{options}}\n\n⛔ _This action requires your explicit approval_\n⏸️ _Session halted: {{agentId}}_',
...
export class MessageTemplates {
  format(data: TemplateData, level: FormatLevel = 'standard'): string {
    ...
    // Format context
    if (data.context) {
      message = message.replace(/{{context}}/g, this.formatContext(data.context));
      message = message.replace(/{{rationale}}/g, data.context.rationale || '');
      message = message.replace(/{{evidence}}/g, this.formatEvidence(data.context.evidence));
      message = message.replace(/{{history}}/g, this.formatHistory(data.context.history));
    } else {
...
const message = messageTemplates.format({
  ...
  options: [
    ...
    { label: 'No, let it retry', description: 'Agent continues with backoff' },
