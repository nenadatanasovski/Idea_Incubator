...
export class CommunicationHub extends EventEmitter {
  ...
  // State
  private registeredAgents: Map<string, { type: AgentType; ready: boolean }> = new Map();
  ...
  static getInstance(db?: Database, config?: Partial<HubConfig>): CommunicationHub {
  ...
  async initialize(): Promise<void> {
    ...
    this.handshake = new AgentHandshake(this.db, this.botRegistry, this.dispatcher);
  ...
  private setupEventHandlers(): void {
    ...
    this.answerProcessor.on('answer:received', async (data) => {
      ...
      await this.executionGate.onQuestionAnswered(questionId, agentId, undefined, answer);
  ...
  async register(
    ...
    if (result.success) {
      this.registeredAgents.set(agentId, { type: agentType, ready: result.state === 'READY' });
  ...
  async askQuestion(
    ...
    await this.db.query(`
      ...
      VALUES (?, ?, ?, ?, 'pending', ?)
    `, [questionId, agentId, options.type, options.content, new Date().toISOString()]);
    ...
    if (this.isBlockingType(options.type)) {
      ...
      if (clearance.cleared) {
        ...
        return {
          ...
          error: clearance.reason === 'timeout' ? 'Question timed out' : 'Cancelled',
        };
      }
    }
  ...
  async waitForAnswer(
    ...
    const result = await this.executionGate.waitForClearance(agentId, undefined, timeoutMs);
