...
| **Blocks**     | All agent tasks (AGT-_, SPC-_, BLD-_, VAL-_, SIA-\*) |
| **Priority**   | P1                                                   |
| **Owner**      | Build Agent                                          |
...
┌─────────────────────────────────────────────────────────────────┐
│  1. REGISTER                                                     │
│     Agent calls communicationHub.register(agentId, agentType)   │
│     → Assigned to correct bot (@vibe_spec_bot, etc.)            │
│     → Added to active agents registry                            │
└──────────────────────────┬──────────────────────────────────────┘
                           │
                           ▼
┌─────────────────────────────────────────────────────────────────┐
│  2. HELLO MESSAGE                                                │
│     Agent sends: "🟢 Agent spec-agent-123 is online"            │
│     → Delivered via assigned bot                                 │
│     → Includes agent type, session ID, capabilities             │
└──────────────────────────┬──────────────────────────────────────┘
                           │
                           ▼
┌─────────────────────────────────────────────────────────────────┐
│  3. DELIVERY CONFIRMATION                                        │
│     System confirms message was delivered to Telegram/Email      │
│     → If fails, retry with backoff                               │
│     → If all retries fail, agent enters DEGRADED mode           │
└──────────────────────────┬──────────────────────────────────────┘
                           │
                           ▼
┌─────────────────────────────────────────────────────────────────┐
│  4. USER ACKNOWLEDGMENT (optional, configurable)                 │
│     For critical agents (Monitoring, Orchestrator):              │
│     → Wait for user to tap [Acknowledged] button                 │
│     → Or auto-acknowledge after configurable timeout             │
│     For worker agents:                                           │
│     → Auto-acknowledge after delivery confirmation               │
└──────────────────────────┬──────────────────────────────────────┘
                           │
                           ▼
┌─────────────────────────────────────────────────────────────────┐
...
│     Agent status: READY                                          │
│     Can now send questions and receive answers                   │
...
1. **Registration**:
   - `register(agentId, agentType, sessionId?, capabilities?)` → RegistrationResult
...
5. **Handshake State Machine**:
   - States: REGISTERING → HELLO_SENT → AWAITING_ACK → READY | DEGRADED
   - Timeouts per state
...
export class AgentHandshake extends EventEmitter {
  ...
  private async sendHello(
    ...
    for (let attempt = 1; attempt <= config.helloRetries; attempt++) {
      try {
        ...
        if (result.delivered) {
          console.log(
            `[Handshake] Hello sent for ${agent.agentId} (attempt ${attempt})`,
          );
          return { success: true };
        }
      } catch (error) {
        console.error(
          `[Handshake] Hello failed for ${agent.agentId} (attempt ${attempt}):`,
          error,
        );
      }
  ...
  private formatHelloMessage(agent: ActiveAgent): string {
    const emoji = agent.agentType === "MONITOR" ? "🟢" : "👋";
    ...
    setTimeout(async () => {
      if (!agent.ackReceived) {
        if (config.autoAckOnTimeout) {
          console.log(
            `[Handshake] Auto-acknowledging ${agent.agentId} after timeout`,
          );
          agent.state = "READY";
          agent.ackReceived = true;
          agent.ackReceivedAt = new Date();
          ...
          console.log(
            `[Handshake] ${agent.agentId} still awaiting acknowledgment`,
          );
          this.emit("agent:awaiting_ack", {
            agentId: agent.agentId,
            agentType: agent.agentType,
          });
          // Retry hello
  ...
  async acknowledgeAgent(agentId: string): Promise<boolean> {
    ...
    if (agent.state !== "AWAITING_ACK") {
      console.log(
        `[Handshake] Agent ${agentId} not awaiting ack (state: ${agent.state})`,
      );
      return false;
    }
