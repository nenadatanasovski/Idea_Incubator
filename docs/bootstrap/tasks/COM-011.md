...
export class ExecutionGate extends EventEmitter {
  ...
  private waitingAgents: Map<
    string,
    { resolve: (result: ClearanceResult) => void }
  > = new Map();
  ...
  async canProceed(agentId: string, sessionId?: string): Promise<boolean> {
    ...
    const blockingQuestions = await this.getBlockingQuestions(
      agentId,
      sessionId,
    );
...
class SpecAgent {
  ...
  async doWork() {
    ...
    const canProceed = await this.executionGate.canProceed(
      this.agentId,
      this.sessionId,
    );
