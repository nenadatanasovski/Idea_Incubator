...
export class SummaryHandler {
  ...
  private async buildSummary(timeWindow: { hours: number; label: string }): Promise<Summary> {
    const windowStart = new Date(Date.now() - timeWindow.hours * 60 * 60 * 1000);
  ...
  private formatSummary(summary: Summary): string {
    ...
    if (summary.issuesDetected > 0) {
      msg += `\n⚠️ *Issues Detected:* ${summary.issuesDetected}\n`;
  ...
  private async getPendingQuestions(): Promise<{
    ...
    return {
      ...
      oldest: {
        id: oldest.id,
        age: ageMinutes > 60 ? `${Math.floor(ageMinutes / 60)}h` : `${ageMinutes}m`,
  ...
  private async getRecentIssues(since: Date): Promise<{
    ...
    return {
      count: results.length,
      highlights: results.slice(0, 5).map((r: any) =>
        `${r.type}: ${r.description.substring(0, 50)}`
      ),
    };
  }
}
...
• Validation agent blocked on coverage question
