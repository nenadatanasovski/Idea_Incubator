...
export class AnswerProcessor extends EventEmitter {
  ...
  private pendingOtherReplies: Map<string, { questionId: string; chatId: string; expiresAt: Date }> = new Map();
  ...
  async processButtonAnswer(callback: {
    ...
    await this.updateMessageWithAnswer(botType as any, chatId, messageId, action);
  ...
  async processTextAnswer(message: ReceivedMessage): Promise<ProcessResult> {
    ...
    if (!question || question.status !== 'pending') {
      return { success: false, questionId, error: 'Question no longer pending' };
    }
    ...
    await this.sender.sendMessage({
      agentType: botType,
      text: `✅ Answer recorded: "${text.substring(0, 50)}${text.length > 50 ? '...' : ''}"`,
    });
...
processor.on('answer:received', (data: {
  questionId: string;
  answer: Answer;
}) => {
