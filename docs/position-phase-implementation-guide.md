...
export const IdeaFinancialAllocationSchema = z.object({
  ...
  ideaRiskTolerance: z.enum(['low', 'medium', 'high', 'very_high']).nullable().default(null),
...
export async function getIdeaAllocation(slug: string): Promise<IdeaFinancialAllocation | null> {
...
export function useIdeaAllocation(slug: string | undefined) {
  ...
  const save = useCallback(async (data: Partial<IdeaFinancialAllocation>) => {
    if (!slug) return;
...
```typescript
import { StrategicApproach, IdeaFinancialAllocation } from '../types/incubation';
...
export function recommendApproach(
  ...
  if (!profile) {
    return {
      primary: 'create',
      reasoning: 'No profile linked. Defaulting to differentiation approach. Link a profile for personalized recommendations.',
  ...
  const riskTolerance = allocation.ideaRiskTolerance || profile.riskTolerance || 'medium';
  ...
  if (runway >= 18 &&
      ...
      goals.includes('exit')) {
    confidenceFactors.push('18+ months runway', 'High risk tolerance', 'Exit goal');
    return {
      primary: 'create',
      reasoning: 'Your long runway, high risk tolerance, and exit goals make differentiation viable. You can afford market education time.',
      confidenceFactors
    };
  }
  ...
  if (runway <= 8 &&
      (incomeType === 'full_replacement' || incomeType === 'partial_replacement' || goals.includes('income'))) {
    ...
    if (profile.city || profile.country) {
      ...
      return {
        ...
        reasoning: 'Your short runway and income goals favor proven models. Your local presence gives localization an edge.',
        confidenceFactors
      };
    }
    ...
    return {
      ...
      secondary: 'localize',
      reasoning: 'Your short runway and income goals favor proven models with faster time-to-revenue.',
      confidenceFactors
    };
  }
  ...
  if (hasDomainExpertise) {
    ...
    return {
      primary: 'specialize',
      secondary: 'create',
      reasoning: 'Your domain expertise enables vertical specialization. You understand niche pain points others miss.',
      confidenceFactors
    };
  }
  ...
  if (incomeType === 'learning' || goals.includes('learning')) {
    ...
    return {
      primary: 'combine',
      secondary: 'create',
      reasoning: 'Learning goals allow more experimentation. Combining concepts builds diverse skills.',
      confidenceFactors
    };
  }
  ...
  if (runway >= 9 && runway <= 17) {
    ...
    return {
      primary: 'specialize',
      secondary: 'combine',
      reasoning: 'Your moderate runway supports focused execution. Specialization balances speed with differentiation.',
      confidenceFactors
    };
  }
  ...
  return {
    primary: 'create',
    reasoning: 'Based on available data, differentiation through innovation is recommended. Consider linking more profile data for refined suggestions.',
...
const [currentSubStep, setCurrentSubStep] = useState<PositionSubStep>('allocation');
const [allocation, setAllocation] = useState<IdeaFinancialAllocation | null>(null);
...
export interface ExecutionComplexity {
  ...
  complexityFactors: Array<{ factor: string; level: 'low' | 'medium' | 'high' }>;
}
...
export function computeGoalAlignment(
  ...
  if (targetIncome !== null) {
    ...
    const relevantRevenue = (targetTimeline && targetTimeline <= 12)
      ? midYear1
      : midYear3;
  ...
  // Timeline assessment
  const monthsToFirstRevenue = strategy.investmentRequired.timeToBreakEvenMonths.mid;
  ...
  const runwayGap = runwaySufficient ? null : monthsToFirstRevenue - allocatedRunway;

  // Investment assessment
  const upfrontNeeded = strategy.investmentRequired.upfront.mid;
  const investmentFeasible = allocatedBudget >= upfrontNeeded;
  const investmentGap = investmentFeasible ? null : upfrontNeeded - allocatedBudget;
...
{positioningDecision && (
  <div className="card mb-6 bg-purple-50 border-purple-200">
    ...
    <p className="text-sm text-gray-700">
      Updating idea based on: <strong>{positioningDecision.primaryStrategyName}</strong>
    </p>
...
*Frontend Components - Phase 1 (UI Shell):*
...
*Frontend Components - Phase 3 (Financial Viability):*
...
*Frontend Components - Phase 4 (Comparison):*
...
*Frontend Components - Phase 5 (Decision):*
