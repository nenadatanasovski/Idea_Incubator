...
const QUESTION_PATTERNS: Record<string, RegExp[]> = {
  // Problem category
  'P1_CORE': [/core problem/i, /main problem/i, /what problem/i, /problem.*solving/i],
  ...
  'P3_WHO': [/target user/i, /who.*experience/i, /customer.*who/i, /who.*problem/i],
...
async function syncDevelopmentAnswers(
  ...
  if (qaPairs.length < 3 && content.length > 500 && costTracker) {
    ...
    qaPairs.push(...llmPairs.filter(p =>
      !qaPairs.some(existing => existing.question === p.question)
    ));
  }
...
async function syncIdea(ideaPath: string, costTracker?: CostTracker): Promise<SyncResult> {
  // ... existing sync logic for README.md ...

  // NEW: Sync development answers
  const devResult = await syncDevelopmentAnswers(idea.id, ideaPath, costTracker);
...
import { conductPreEvaluationResearch, ResearchResult, formatResearchForCategory } from '../agents/research.js';
...
logInfo(`Extracted claims: ${userClaims.competitors.length} competitors, tech: ${userClaims.technology.join(', ')}`);

const research = await conductPreEvaluationResearch(ideaContent, userClaims, costTracker);
