...
<div className="space-y-6">
  <div className="card text-center">
    ...
    <div className="inline-flex... w-24 h-24 rounded-full">
      {" "}
      // Large circle
...
<div className="flex items-start justify-between">
  ...
  <div className="flex flex-col items-end gap-2">
    {/* Score display */}
    {idea.avg_final_score !== null && <div className="text-right">...</div>}
  ...
  return (
    <div className="card p-0 overflow-hidden">
      ...
      <div className="p-6 max-h-[600px] overflow-y-auto">
        ...
        {activeTab === "redteam" && <RedTeamView slug={slug} runId={runId} />}
...
// Replace the card with:
{
  hasDebate && (
    <div className="card bg-gradient-to-r from-purple-50 to-blue-50 p-0 overflow-hidden">
      ...
      {showDebateSummary && (
        <div className="p-6 pt-0">{/* Existing grid content */}</div>
      )}
    </div>
  );
}
  ...
  return (
    <Wrapper
      className={clsx(
        ...
        collapsible &&
          "w-full hover:bg-gray-50 -mx-2 px-2 py-1 rounded transition",
      )}
...
<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
  ...
  <div className="lg:col-span-2">{/* Category cards - more prominence */}</div>
</div>
```
...
.sticky-below-context {
  top: calc(
    var(--header-height) + var(--context-header-height) + var(--spacing-normal)
  );
}
```
