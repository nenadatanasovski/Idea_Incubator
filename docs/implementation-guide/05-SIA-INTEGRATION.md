...
export class SIAAgent extends EventEmitter {
  ...
  private initializeTechniques(): void {
    this.techniqueLibrary.set(
      "decomposition",
      new DecompositionTechnique(this.llm),
    );
    this.techniqueLibrary.set(
      "prompt_restructure",
      new PromptRestructureTechnique(this.llm),
    );
    this.techniqueLibrary.set(
      "context_pruning",
      new ContextPruningTechnique(this.llm),
    );
    this.techniqueLibrary.set(
      "example_injection",
      new ExampleInjectionTechnique(this.llm),
    );
    this.techniqueLibrary.set(
      "constraint_relaxation",
      new ConstraintRelaxationTechnique(this.llm),
    );
...
describe("SIA Schema", () => {
  it("records intervention attempts", async () => {
    ...
    const attempts = await db.siaAttempts.findMany({
      where: { taskId: "task-1" },
    });
...
_Next: [06-TESTING-CRITERIA.md](./06-TESTING-CRITERIA.md) — Verification tests for all tasks_
