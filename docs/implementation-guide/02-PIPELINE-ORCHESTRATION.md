...
export class PipelineOrchestrator extends EventEmitter {
  ...
  private async executeTransition(
    ...
    try {
      ...
      // Emit event
      this.emit('transition', { ideaId, from: previousPhase, to: targetPhase, reason });
...
export class IdeationCompletionDetector {
  ...
  async analyzeReadiness(ideaId: string): Promise<IdeationProgress> {
    ...
    const confidenceScore = await this.calculateAIConfidence(messages, artifacts);
...
export function usePipelineStatus(ideaId: string) {
  ...
  const pause = () => fetch(`/api/pipeline/${ideaId}/pause`, { method: 'POST' });
  const resume = () => fetch(`/api/pipeline/${ideaId}/resume`, { method: 'POST' });
...
describe('Pipeline State Schema', () => {
  it('creates pipeline state for new idea', async () => {
    ...
    const state = await db.ideaPipelineState.findUnique({ where: { ideaId: idea.id } });
...
describe('PipelineOrchestrator', () => {
  ...
  it('executes valid transition', async () => {
    ...
    const result = await orch.requestTransition('idea-1', 'specification', 'test');
...
*Next: [03-SPEC-AGENT-COMPLETION.md](./03-SPEC-AGENT-COMPLETION.md) — Finishing the specification agent*
