...
test.describe('Chat Panel Interactions', () => {
  ...
  test('displays message history correctly', async ({ page }) => {
    ...
    const userMessage = page.locator('[data-testid="message"][data-role="user"]').first();
    const assistantMessage = page.locator('[data-testid="message"][data-role="assistant"]').first();
  ...
  test('sends message on button click', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="message"]').last())
      .toContainText('Button send test');
  });
  ...
  test('shows agent activity indicator', async ({ page }) => {
    ...
    await expect(activityIndicator).toContainText(/analyzing|processing|thinking/i);
  });
...
test.describe('Phase Transitions in the UI', () => {
  ...
  test('shows phase progress visually', async ({ page }) => {
    ...
    await expect(progressSteps.locator('[data-step="ideation"]')).toHaveClass(/complete/);
    await expect(progressSteps.locator('[data-step="specification"]')).toHaveClass(/active/);
    await expect(progressSteps.locator('[data-step="building"]')).toHaveClass(/pending/);
  });
  ...
  test('transitions between phases with animation', async ({ page }) => {
    ...
    // Verify new phase
    await expect(page.getByTestId('phase-indicator')).toContainText(/specification/i);
  });
  ...
  test('blocks invalid phase transitions', async ({ page }) => {
    ...
    await expect(page.getByTestId('phase-error')).toBeVisible()
      .catch(async () => {
        // Or redirected back
  ...
  test('real-time phase updates via WebSocket', async ({ page }) => {
    ...
    await expect(page.getByTestId('phase-indicator'))
      .toContainText(/ready/i, { timeout: 5000 });
  });
});
```
...
test.describe('Memory Graph Rendering', () => {
  ...
  test('shows node details on hover', async ({ page }) => {
    ...
    await expect(page.getByTestId('graph-tooltip')).toBeVisible({ timeout: 3000 });
  });
  ...
  test('highlights connected nodes on selection', async ({ page }) => {
    ...
    await expect(page.getByTestId('node-details-panel')).toBeVisible({ timeout: 3000 });
  });
  ...
  test('visual snapshot - graph layout', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="memory-graph-container"]'))
...
test.describe('Build Progress Display', () => {
  ...
  test('displays task list with correct statuses', async ({ page }) => {
    ...
    await expect(taskList.locator('[data-status="completed"]').first()).toBeVisible();
    await expect(taskList.locator('[data-status="in_progress"]')).toHaveCount(1);
    await expect(taskList.locator('[data-status="pending"]').first()).toBeVisible();
  });
  ...
  test('shows task completion checkmarks', async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      const checkmark = completedTasks.nth(i).locator('[data-testid="checkmark"]');
  ...
  test('pause button stops build', async ({ page }) => {
    ...
    await expect(page.locator('[data-status="in_progress"]')).toHaveAttribute('data-paused', 'true');
  });
  ...
  test('resume button continues build', async ({ page }) => {
    ...
    await expect(page.getByTestId('build-status')).toContainText(/running|active/i);
  });
  ...
  test('visual snapshot - build in progress', async ({ page }) => {
    await expect(page.getByTestId('build-progress-container'))
      .toHaveScreenshot('build-progress.png', {
        animations: 'disabled',
      });
  });
  ...
  test('visual snapshot - build complete', async ({ page }) => {
    ...
    await expect(page.getByTestId('build-progress-container'))
      .toHaveScreenshot('build-complete.png', {
        animations: 'disabled',
      });
  });
});
