    ...
    try {
      while (true) {
        ...
        if (result.success) {
          ...
          const shouldContinue = await this.handleTaskFailure(
            session,
            task,
            result,
          );
...
export function BuildProgressView({ ideaId }: BuildProgressViewProps) {
  const { session, events, pause, resume, skip, resolve } =
  ...
  const { status, tasks, completedTasks, failedTasks, currentTaskIndex } =
    session;
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <div className="mt-3 flex gap-2">
                ...
                <button onClick={skip} className="btn btn-sm btn-secondary">
                  Skip this task
                </button>
              </div>
            </div>
        ...
        <div className="space-y-2">
          {tasks.map((task, index) => (
            <TaskCard
              ...
              attempt={
                index === currentTaskIndex ? session.currentAttempt : undefined
              }
            />
          ))}
        </div>
    ...
    >
      <div className="flex items-center gap-3">
        ...
        {isCurrent && attempt && attempt > 1 && (
          <span className="text-xs text-amber-600">Attempt {attempt}</span>
        )}
      </div>
    </div>
  );
...
export function useBuildSession(ideaId: string) {
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      ...
      if (
        ["taskComplete", "taskFailed", "buildComplete", "humanNeeded"].includes(
          data.type,
        )
      ) {
        fetchStatus();
...
describe("Build API", () => {
  it("GET /status returns progress", async () => {
    ...
    const res = await request(app).get("/api/build/idea-1/status").expect(200);
  ...
  it("POST /skip advances past failed task", async () => {
    ...
    await request(app).post(`/api/build/${session.id}/skip`).expect(200);
...
_Next: [05-SIA-INTEGRATION.md](./05-SIA-INTEGRATION.md) — Self-Improvement Agent integration_
