...
export class TimingValidator {
  ...
  assertStageDuration(stage: string, maxMs: number): void {
    ...
    expect(duration, `Stage "${stage}" exceeded time limit`).toBeLessThan(maxMs);
  }
...
describe('FR-1: Bug Report E2E Flow', () => {
  ...
  it('should process bug report end-to-end within 30 seconds', async () => {
    ...
    const intakeResult = await intakeAgent.processFeedback({ id: submissionId, ...bugReport });
  ...
  it('should attach analytics data to created task', async () => {
    ...
    const bugReport = {
      ...
      metadata: {
        ...
        console_logs: ['ERROR: Type "string" is not assignable to type "number"'],
    ...
    const tasks = await query('SELECT * FROM tasks WHERE source_feedback_id = ?', [submissionId]);
    const task = tasks[0];
    ...
    expect(metadata.analytics.session_replay_url).toBe('https://replay.service/session-67890');
  });
});
```
...
describe('FR-4: Load Test - 100 Submissions Per Minute', () => {
  ...
  it('should handle 100 submissions/minute for 5 minutes', async () => {
    ...
    const feedbackRecords = await query('SELECT COUNT(*) as count FROM feedback_submissions');
