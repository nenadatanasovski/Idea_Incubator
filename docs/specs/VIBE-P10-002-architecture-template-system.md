...
export const webAppTemplate: ArchitectureTemplate = {
  ...
  description:
    "Modern web application with React/Next.js frontend and Express API backend",
  type: "web",
  ...
  packages: {
    core: [
      {
        name: "react",
        reason: "Component-based UI library with excellent ecosystem",
      },
      { name: "react-dom", reason: "React DOM rendering for web applications" },
      {
        name: "next",
        reason: "React framework with SSR, routing, and API routes",
      },
      {
        name: "typescript",
        reason: "Type safety across frontend and backend code",
      },
      {
        name: "tailwindcss",
        reason: "Utility-first CSS framework for rapid styling",
      },
      {
        name: "express",
        reason: "Fast, minimal backend framework with middleware ecosystem",
      },
  ...
  configs: [
    {
      ...
      description:
        "TypeScript configuration for modern ES modules with strict type checking",
    },
    {
...
function validateTemplate(template: ArchitectureTemplate): void {
  ...
  if (
    !template.type ||
    !["web", "mobile", "api", "monolith", "custom"].includes(template.type)
  ) {
  ...
  if (
    ...
    Object.keys(template.folder_structure).length === 0
  ) {
  ...
  if (
    ...
    template.deployment.platform.length === 0
  ) {
