...
export class AnalyticsService {
   ...
   * Get error hotspots
   */
  async getErrorHotspots(limit = 10): Promise<
    Array<{
  ...
  async enrichBugReport(
    ...
    `,
      ...
      .get(sessionId) as
      | { browser: string; os: string; duration_seconds: number }
      | undefined;

    // Get error history
    const errorHistory = this.db
      .prepare(
        `
...
router.get("/enrich/:sessionId", async (req, res) => {
  try {
    ...
    const enrichment = await analyticsService.enrichBugReport(
      userId,
      sessionId,
    );
...
describe("AnalyticsSDK", () => {
  test("tracks page view event", () => {
    ...
    expect(queue).toContainEvent({
      event_type: "page_view",
      event_name: "page_viewed_dashboard",
    });
  });
...
describe("Analytics Integration", () => {
  ...
  test("cleanup job removes old events", async () => {
    ...
    const oldEvents = db
      .prepare(
        'SELECT * FROM analytics_events WHERE recorded_at < datetime("now", "-30 days")',
      )
      .all();
