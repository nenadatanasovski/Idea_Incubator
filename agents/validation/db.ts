...
export async function saveValidatorResult(result: ValidatorResult): Promise<void> {
  const db = await getDb();
  db.run(`
...
export async function getValidatorResults(runId: string): Promise<ValidatorResult[]> {
  const db = await getDb();
