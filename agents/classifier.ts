...
export type IdeaDomain = 'business' | 'creative' | 'technical' | 'personal' | 'research';
export type RelationshipType = 'parent' | 'child' | 'related' | 'combines' | 'conflicts' | 'inspired_by';
...
export async function classifyIdea(
  ...
  const existingList = existingIdeas
    .map(i => `- ${i.slug}: "${i.title}" [${i.idea_type || 'untyped'}]${i.summary ? ` - ${i.summary}` : ''}`)
    .join('\n');
...
function validateDomain(domain: string): IdeaDomain {
  const valid: IdeaDomain[] = ['business', 'creative', 'technical', 'personal', 'research'];
  const normalized = domain?.toLowerCase().trim();
  return valid.includes(normalized as IdeaDomain) ? normalized as IdeaDomain : 'technical';
}
...
function validateRelationship(rel: any): IdeaRelationship {
  const validTypes: RelationshipType[] = ['parent', 'child', 'related', 'combines', 'conflicts', 'inspired_by'];
...
export async function saveClassification(
  ...
  // Update idea type
  await run(
    'UPDATE ideas SET idea_type = ? WHERE id = ?',
    [classification.domain, ideaId]
  );
 ...
 */
export async function getIdeaRelationships(ideaId: string): Promise<Array<{
  targetSlug: string;
  targetTitle: string;
  type: RelationshipType;
  strength: RelationshipStrength;
...
export async function findSimilarIdeas(
  ...
  const results = await Promise.all(similar.map(async s => {
