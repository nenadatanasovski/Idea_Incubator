...
const AMBIGUITY_PATTERNS = [
  ...
  {
    pattern: /as needed|when necessary|if required/i,
...
const ARCHITECTURE_CHOICES = [
  ...
  },
  {
    ...
    defaultRationale:
      "Simple to implement, suitable for single-server deployments",
  },
  {
...
export class QuestionGenerator {
  ...
  generate(
    ...
    const canProceedWithDefaults = questions.every(
      (q) => q.type !== "BLOCKING" || q.default !== undefined,
    );
    ...
    for (const { pattern, reason } of AMBIGUITY_PATTERNS) {
      ...
      if (match) {
        ...
        if (reason === "embedded question") {
          ...
          contextSentence =
            sentences.find((s) => pattern.test(s))?.trim() || match[0];
        }
  ...
  private checkMissingInformation(brief: ParsedBrief): Question[] {
    ...
    if (!brief.mvpScope.inScope || brief.mvpScope.inScope.length === 0) {
      questions.push({
        ...
        defaultRationale:
          "Without scope, only essential features will be built",
      });
    }

    // Check for missing database schema hints
    if (
      brief.solution.toLowerCase().includes("store") ||
