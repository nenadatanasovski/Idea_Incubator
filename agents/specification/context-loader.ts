...
const HARDCODED_GOTCHAS: Gotcha[] = [
  ...
  {
    id: 'G-008',
    content: 'Parse JSON columns when reading from SQLite, stringify when writing',
    filePattern: 'database/*.ts',
    actionType: 'UPDATE',
    confidence: 'high',
    source: 'experience'
  ...
  {
    id: 'G-009',
    content: 'Fire-and-forget async operations require try/catch to prevent unhandled rejections',
    filePattern: '*.ts',
    actionType: 'CREATE',
    confidence: 'high',
    source: 'experience'
  ...
  {
    id: 'G-010',
    content: 'Use multer.memoryStorage() for file uploads, not default disk storage',
  ...
  {
    id: 'G-011',
    content: 'WebSocket connections may disconnect - always handle reconnection',
...
export class ContextLoader {
  ...
  async loadTemplates(): Promise<Record<string, string>> {
    const templatesDir = path.join(this.projectRoot, 'templates', 'unified', 'build');
  ...
  getRelevantGotchas(filePattern: string, actionType: string): Gotcha[] {
    return HARDCODED_GOTCHAS.filter(gotcha => {
      ...
      const actionMatch = gotcha.actionType === actionType || gotcha.actionType === '*';
  ...
  async loadIdeaReadme(ideaSlug: string): Promise<string | undefined> {
    const readmePath = path.join(this.projectRoot, 'ideas', ideaSlug, 'README.md');
  ...
  async loadIdeaBrief(ideaSlug: string): Promise<string | undefined> {
    const briefPath = path.join(this.projectRoot, 'ideas', ideaSlug, 'planning', 'brief.md');
