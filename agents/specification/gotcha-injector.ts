...
const HARDCODED_GOTCHAS: Gotcha[] = [
  ...
  {
    id: 'SQL-004',
    content: 'Create indexes for foreign key columns and frequently queried fields',
    filePattern: 'database/migrations/*.sql',
    actionType: 'CREATE',
    severity: 'warning',
    category: 'sql'
  ...
  {
    id: 'SVC-003',
    content: 'Throw typed errors that route handlers can catch and map to HTTP status',
    filePattern: 'server/services/*',
    actionType: 'BOTH',
    severity: 'warning',
    category: 'services'
...
export class GotchaInjector {
  ...
  private injectIntoTask(task: AtomicTask): AtomicTask {
    ...
    const combinedGotchas = [...task.gotchas, ...newGotchas].slice(0, this.maxGotchasPerTask);
    ...
    if (this.includeComments && codeTemplate && matchedGotchas.length > 0) {
      codeTemplate = this.addGotchaComments(codeTemplate, matchedGotchas, task.file);
    }
  ...
  private matchGotchas(task: AtomicTask): Gotcha[] {
    ...
    for (const gotcha of this.gotchas) {
      ...
      const patternScore = this.getPatternMatchScore(gotcha.filePattern, task.file);
      if (patternScore === 0) {
        continue;
      }
   ...
   */
  private actionMatches(gotchaAction: 'CREATE' | 'UPDATE' | 'BOTH', taskAction: string): boolean {
  ...
  private getPatternMatchScore(pattern: string, filePath: string): number {
    ...
    if (patternDir && !patternDir.includes('*') && filePath.startsWith(patternDir + '/')) {
      const extensionMatch = pattern.endsWith('.ts') && filePath.endsWith('.ts');
   ...
   */
  private addGotchaComments(template: string, gotchas: Gotcha[], filePath: string): string {
    ...
    const comments = criticalGotchas
      .map(g => `${commentStyle.start} GOTCHA [${g.id}]: ${g.content} ${commentStyle.end}`)
      .join('\n');
 ...
 */
export function createGotchaInjector(options?: InjectorOptions): GotchaInjector {
