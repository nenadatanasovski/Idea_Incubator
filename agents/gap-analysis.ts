...
Respond in JSON:
{
  ...
  if (content.type !== 'text') {
    throw new EvaluationParseError('Unexpected response type from gap analysis agent');
  }
  ...
  logInfo(`Gap analysis complete: ${assumptions.length} assumptions identified, readiness: ${readinessScore}%`);
 ...
 */
export function generateViabilityAdvisory(gapAnalysis: GapAnalysis): ViabilityAdvisory {
  const { assumptions, criticalGapsCount, significantGapsCount, readinessScore } = gapAnalysis;
...
export function calculatePriorityScore(assumption: Assumption): number {
  return IMPACT_WEIGHTS[assumption.impact] * CONFIDENCE_WEIGHTS[assumption.confidence];
}
