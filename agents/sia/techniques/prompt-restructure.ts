...
export class PromptRestructureTechnique extends BaseTechnique {
  ...
  private restructureTask(
    ...
    const combinedGotchas = Array.from(
      new Set([...task.gotchas, ...additionalGotchas]),
    );
  ...
  private generateClarifications(
    ...
    if (
      analysis.errorPatterns.some((e) => e.toLowerCase().includes("import"))
    ) {
      clarifications.push(
  ...
  private generateErrorBasedRequirements(analysis: FailureAnalysis): string[] {
    ...
    for (const pattern of analysis.errorPatterns) {
      ...
      if (
        patternLower.includes("type error") ||
        patternLower.includes("type mismatch")
      ) {
        reqs.push(
          "Ensure type compatibility between function parameters and return values",
        );
      }

      if (patternLower.includes("syntax error")) {
        reqs.push(
          "Follow TypeScript/JavaScript syntax strictly, checking for missing brackets or semicolons",
        );
      }

      if (patternLower.includes("timeout")) {
        reqs.push(
          "Optimize performance - avoid expensive operations in the critical path",
        );
      }
    }
  ...
  private extractGotchasFromError(analysis: FailureAnalysis): string[] {
    ...
    if (analysis.rootCause) {
      gotchas.push(
        `Previous failure: ${analysis.rootCause} - avoid this pattern`,
      );
    }

    // Error pattern based gotchas
    for (const pattern of analysis.errorPatterns.slice(0, 3)) {
      // Limit to 3
  ...
  private improveValidation(
    ...
    if (!command || command.trim() === "") {
      ...
      if (task.file.endsWith(".ts") || task.file.endsWith(".tsx")) {
        ...
        task.file.endsWith(".test.ts") ||
        task.file.endsWith(".spec.ts")
      ) {
