...
export const STANDARD_JOURNEYS: Journey[] = [
  {
    ...
    tags: ['smoke', 'critical'],
    steps: [
      { action: 'wait', target: 'body', timeout: 5000, description: 'Wait for body' },
  ...
  },
  {
    ...
    steps: [
      ...
      { action: 'click', target: '[data-testid="ideas-link"]', description: 'Click ideas link' },
  ...
  },
  {
    ...
    steps: [
      ...
      { action: 'assert', target: '[data-testid="idea-title-input"]', value: '' },
  ...
  },
  {
    ...
    steps: [
      ...
      { action: 'wait', target: '[data-testid="metrics-panel"]', timeout: 5000 },
...
export function getJourneysByTag(tag: string): Journey[] {
  return Array.from(journeyRegistry.values()).filter(
    journey => journey.tags?.includes(tag)
  );
}
