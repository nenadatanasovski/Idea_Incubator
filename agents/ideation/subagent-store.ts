...
export async function saveSubAgent(subagent: {
  ...
  console.log(
    `[SubAgentStore] Saving sub-agent ${subagent.id}, status: ${subagent.status}`,
  );
 ...
 */
export async function getSubAgentsBySession(
  sessionId: string,
): Promise<StoredSubAgent[]> {
 ...
 */
export async function deleteSubAgentsBySession(
  sessionId: string,
): Promise<void> {
