...
export function calculateConfidence(
  ...
  const frustrations = Array.isArray(selfDiscovery.frustrations)
    ? (selfDiscovery.frustrations as unknown[])
    : [];
  if (frustrations.length > 0) {
    ...
    const highSeverity = frustrations.filter(
      (f) =>
        (typeof f === "object" &&
          (f as Record<string, unknown>)?.severity === "high") ||
  ...
  // Cast to unknown[] to handle legacy data that might be strings
  const gaps = Array.isArray(marketDiscovery.gaps)
    ? (marketDiscovery.gaps as unknown[])
    : [];
  if (gaps.length > 0) {
    ...
    const highRelevance = gaps.filter(
      (g) =>
        (typeof g === "object" &&
          (g as Record<string, unknown>)?.relevance === "high") ||
  ...
  const customerTypeValue =
    typeof customerType === "string"
      ...
      : typeof customerType === "object"
        ? customerType?.value
        : null;
  const customerTypeConfidence =
    typeof customerType === "object" ? (customerType?.confidence ?? 0.5) : 0.5;
  ...
  const hasLocation =
    ...
    (typeof marketDiscoveryAny.location === "string" &&
      (marketDiscoveryAny.location as string).length > 0) ||
    (typeof marketDiscoveryAny.location_context === "string" &&
      (marketDiscoveryAny.location_context as string).length > 0);
  if (hasLocation) {
    targetScore += 5;
  }
  ...
  const hasProductType =
    productTypeValue ||
    (narrowingState as Record<string, unknown>).product_type;
  if (hasProductType) {
    solutionScore += 7;
  } else {
  ...
  const technicalDepthValue =
    typeof technicalDepth === "string"
      ? technicalDepth
      : typeof technicalDepth === "object"
  ...
  const competitors = Array.isArray(marketDiscovery.competitors)
    ? marketDiscovery.competitors
    : [];
  // Also check competitors_mentioned alternative key
  const competitorsMentioned = Array.isArray(
    (marketDiscovery as Record<string, unknown>).competitors_mentioned,
  )
    ? ((marketDiscovery as Record<string, unknown>)
        .competitors_mentioned as unknown[])
    : [];
  const hasCompetitors =
    competitors.length > 0 || competitorsMentioned.length > 0;
  if (hasCompetitors) {
    diffScore += 8;
  } else {
  ...
  const hasCompetitorWeaknesses =
    competitors.some(
      (c) => typeof c === "object" && (c?.weaknesses || []).length > 0,
    ) ||
  ...
  const expertise = Array.isArray(selfDiscovery.expertise)
  ...
  const expertiseAreas = expertise
    .map((e: unknown) =>
      typeof e === "string"
        ? e.toLowerCase()
        : typeof e === "object" && (e as Record<string, unknown>)?.area
          ? String((e as Record<string, unknown>).area).toLowerCase()
          : "",
    )
    .filter(Boolean);
  // Check if any expertise term appears in any gap description
  const hasExpertiseMatch =
    expertiseAreas.length > 0 &&
    gaps.some((g: unknown) => {
      const gapText =
        typeof g === "string"
          ? g.toLowerCase()
          : typeof g === "object" && (g as Record<string, unknown>)?.description
            ? String((g as Record<string, unknown>).description).toLowerCase()
            : "";
      return expertiseAreas.some(
        (exp: string) =>
          gapText.includes(exp) ||
          exp.includes("app") ||
          exp.includes("development"),
      );
    });
  ...
  const skills = selfDiscovery.skills || {
    strengths: [],
    gaps: [],
    identified: [],
  };
  ...
  const hasConstraintsSet =
    (typeof constraints.location === "object" &&
      (constraints.location as Record<string, unknown>)?.target != null) ||
    (typeof constraintsAny.location === "string" &&
      (constraintsAny.location as string).length > 0) ||
