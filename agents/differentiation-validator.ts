...
Respond in JSON:
{
  ...
  if (content.type !== "text") {
    throw new EvaluationParseError(
      "Unexpected response type from validation agent",
    );
  }
  ...
  const marketOpportunities: ValidatedOpportunity[] =
    parsed.validatedOpportunities.map((o) => ({
  ...
  const differentiationStrategies: ValidatedStrategy[] =
    parsed.validatedStrategies.map((s) => ({
      name: s.name,
  ...
  logInfo(
    `Validation complete. Overall confidence: ${Math.round(parsed.overallConfidence * 100)}%`,
  );
...
export function filterValidatedSuggestions(
  ...
  const removedOpps =
    validated.marketOpportunities.length - filteredOpportunities.length;
  const removedStrats =
    validated.differentiationStrategies.length - filteredStrategies.length;

  if (removedOpps > 0 || removedStrats > 0) {
    logWarning(
      `Filtered out ${removedOpps} opportunities and ${removedStrats} strategies with low confidence`,
    );
  }
 ...
 */
export function formatValidatedAnalysis(
  validated: ValidatedDifferentiationAnalysis,
): string {
  let output = `
...
`;
  ...
  for (const opp of validated.marketOpportunities) {
    const confidenceIcon =
      opp.validationConfidence >= 0.7
        ? "✓"
        : opp.validationConfidence >= 0.5
          ? "~"
          : "!";
    output += `
...
`;
  ...
  for (let i = 0; i < validated.differentiationStrategies.length; i++) {
    ...
    const confidenceIcon =
      strat.validationConfidence >= 0.7
        ? "✓"
        : strat.validationConfidence >= 0.5
          ? "~"
          : "!";
...
function formatConfidenceBar(confidence: number): string {
  ...
  return (
    "█".repeat(filled) + "░".repeat(empty) + ` ${Math.round(confidence * 100)}%`
  );
}
