...
export function formatViabilityAdvisory(advisory: ViabilityAdvisory): string {
  const { criticalGaps, significantGaps, readinessScore, recommendation, reasoning } = advisory;
  ...
  const recColor = recommendation === 'proceed'
    ? chalk.green
...
${empty}`;
  ...
  if (criticalGaps.length > 0) {
    ...
    for (const gap of criticalGaps.slice(0, 3)) {
      const gapText = gap.text.length > 50 ? gap.text.substring(0, 47) + '...' : gap.text;
      output += `\n${line(`  • ${gapText}`)}`;
    }
...
export function generateEvaluationAdvisory(
  overallScore: number,
  confidence: number,
  criteriaScores: Array<{ code: string; name: string; score: number; confidence: number }>
): EvaluationAdvisory {
  ...
  if (overallScore >= 7.5) {
    ...
    reasoning = `Score of ${overallScore.toFixed(1)}/10 with ${weakCriteria.length} weak areas that can be improved. Iteration recommended to address: ${weakCriteria.slice(0, 3).map(c => c.name).join(', ')}.`;
  } else if (overallScore >= 5.0 && !hasAddressableWeaknesses) {
...
function getSuggestedAction(criterionCode: string): string {
  ...
  return actions[criterionCode] || 'Address this criterion with focused research';
}
...
export function formatEvaluationAdvisory(advisory: EvaluationAdvisory): string {
  const { overallScore, confidence, weakCriteria, recommendation, reasoning } = advisory;
  ...
  // Color based on recommendation
  const recColor = recommendation === 'pursue'
    ? chalk.green
    : recommendation === 'iterate'
      ? chalk.blue
      : recommendation === 'branch'
...
${empty}`;
  ...
  if (weakCriteria.length > 0) {
    ...
    for (const criterion of weakCriteria.slice(0, 4)) {
      ...
      const actionText = criterion.suggestedAction.length > 40
...
export async function logGateDecision(
  ...
  logInfo(`Gate decision logged: ${gateType} - ${recommendation} → ${userChoice}`);
}

/**
 * Get gate decisions for an idea
 */
export async function getGateDecisions(ideaId: string): Promise<GateDecision[]> {
 ...
 */
export function mapViabilityChoice(choice: number): ViabilityRecommendation | null {
 ...
 */
export function mapEvaluationChoice(choice: number): EvaluationRecommendation | null {
