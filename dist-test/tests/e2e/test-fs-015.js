...
import { addRelationship, getRelationships, getParent, syncRelationshipsToFile, syncRelationshipsFromFile } from '../../utils/relationship-manager.js';
...
async function test1_syncRelationshipsToFile_createsFile() {
    ...
    await addRelationship(TEST_USER, TEST_IDEA, TEST_USER, 'parent-idea', 'parent', {});
    // Sync to file
    ...
    if (!content.parent || content.parent.slug !== 'parent-idea') {
        throw new Error(`Parent relationship not in file. Got: ${JSON.stringify(content.parent)}`);
    }
...
async function test2_fileContainsAllDatabaseRelationships() {
    ...
    const ideaPath = path.join(path.dirname(getConfig().paths.ideas), 'users', TEST_USER, 'ideas', TEST_IDEA);
    const relPath = path.join(ideaPath, '.metadata', 'relationships.json');
    // Add more relationships
    await addRelationship(TEST_USER, TEST_IDEA, TEST_USER, 'integration-1', 'integrates_with', {});
    await addRelationship(TEST_USER, TEST_IDEA, TEST_USER, 'old-idea', 'evolved_from', {});
    await addRelationship(TEST_USER, TEST_IDEA, TEST_USER, 'competitor', 'competes_with', {});
    // Sync to file
    await syncRelationshipsToFile(TEST_USER, TEST_IDEA);
    // Verify file content
    const content = JSON.parse(fs.readFileSync(relPath, 'utf-8'));
    ...
    if (!content.ai_detected?.competes_with || content.ai_detected.competes_with.length === 0) {
...
async function test3_syncRelationshipsFromFile_updatesDatabase() {
    ...
    if (!parent || parent.slug !== 'new-parent') {
        throw new Error(`Parent not updated in database. Got: ${JSON.stringify(parent)}`);
    }
    const relationships = await getRelationships(TEST_USER, TEST_IDEA);
    const forkedRel = relationships.find(r => r.relationshipType === 'forked_from');
...
async function test4_bidirectionalSyncIsIdempotent() {
    ...
    const initialTypes = initialRelationships.map(r => r.relationshipType).sort();
...
async function test6_handlesEmptyRelationships() {
    ...
    if (!Array.isArray(content.integrates_with) || content.integrates_with.length !== 0) {
    ...
    const outgoing = relationships.filter(r => r.fromUser === TEST_USER && r.fromIdea === testIdea);
