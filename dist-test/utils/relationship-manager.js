 ...
 */
export async function addRelationship(fromUser, fromIdea, toUser, toIdea, type, metadata = {}, createdBy) {
    const id = uuidv4();
    const metadataJson = JSON.stringify(metadata);
    // Determine if this is an external relationship
    const toExternal = toUser === null && toIdea === null ? metadata.externalName || null : null;
    await run(`INSERT INTO idea_relationships (id, from_user, from_idea, to_user, to_idea, to_external, relationship_type, metadata, created_by)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [id, fromUser, fromIdea, toUser, toIdea, toExternal, type, metadataJson, createdBy || null]);
    await saveDb();
    return id;
}
/**
 ...
 */
export async function getRelationships(userSlug, ideaSlug) {
    const rows = await query(`SELECT * FROM idea_relationships
     WHERE (from_user = ? AND from_idea = ?)
        OR (to_user = ? AND to_idea = ?)
     ORDER BY created_at DESC`, [userSlug, ideaSlug, userSlug, ideaSlug]);
 ...
 */
export async function getChildren(userSlug, ideaSlug) {
    ...
    const rows = await query(`SELECT * FROM idea_relationships
     ...
     ORDER BY created_at DESC`, [userSlug, ideaSlug]);
    return rows.map(row => ({
        slug: row.from_idea,
 ...
 */
export async function getParent(userSlug, ideaSlug) {
    // Find relationship where this idea has a parent relationship
    const row = await getOne(`SELECT * FROM idea_relationships
     WHERE from_user = ? AND from_idea = ? AND relationship_type = 'parent'
     LIMIT 1`, [userSlug, ideaSlug]);
 ...
 */
export async function getRelationshipsByType(userSlug, ideaSlug, type) {
    const rows = await query(`SELECT * FROM idea_relationships
     WHERE ((from_user = ? AND from_idea = ?) OR (to_user = ? AND to_idea = ?))
       AND relationship_type = ?
     ORDER BY created_at DESC`, [userSlug, ideaSlug, userSlug, ideaSlug, type]);
    return rows.map(rowToRelationship);
}
// ============================================================================
// HELPER FUNCTIONS
 ...
 */
function getRelationshipsFilePath(userSlug, ideaSlug) {
 ...
 */
function createDefaultRelationshipsData() {
    return {
        idea_type: null,
        parent: null,
        integrates_with: [],
 ...
 * @param ideaSlug - Idea slug
 */
export async function syncRelationshipsToFile(userSlug, ideaSlug) {
    ...
    const outgoingRelationships = relationships.filter(r => r.fromUser === userSlug && r.fromIdea === ideaSlug);
 ...
 * @param ideaSlug - Idea slug
 */
export async function syncRelationshipsFromFile(userSlug, ideaSlug) {
    const filePath = getRelationshipsFilePath(userSlug, ideaSlug);
    const metadataDir = path.dirname(filePath);
    // Ensure .metadata directory exists
    if (!fs.existsSync(metadataDir)) {
    ...
    const outgoingRelationships = existingRelationships.filter(r => r.fromUser === userSlug && r.fromIdea === ideaSlug);
    ...
    if (fileData.parent) {
        if (fileData.parent.type === 'internal' && fileData.parent.slug) {
            await addRelationship(userSlug, ideaSlug, userSlug, // Assume same user for internal
            fileData.parent.slug, 'parent', {});
        }
        else if (fileData.parent.type === 'external' && fileData.parent.name) {
            await addRelationship(userSlug, ideaSlug, null, null, 'parent', { externalName: fileData.parent.name });
        }
    }
    ...
    if (fileData.integrates_with && Array.isArray(fileData.integrates_with)) {
        for (const integration of fileData.integrates_with) {
            if ('user' in integration && 'slug' in integration) {
                await addRelationship(userSlug, ideaSlug, integration.user, integration.slug, 'integrates_with', {});
            }
            else if ('external' in integration) {
                await addRelationship(userSlug, ideaSlug, null, null, 'integrates_with', { externalName: integration.external });
            }
        }
    }
    // Evolved_from relationship
    if (fileData.evolved_from && fileData.evolved_from.user && fileData.evolved_from.slug) {
        await addRelationship(userSlug, ideaSlug, fileData.evolved_from.user, fileData.evolved_from.slug, 'evolved_from', {});
    }
    // Forked_from relationship
    if (fileData.forked_from && fileData.forked_from.user && fileData.forked_from.slug) {
        await addRelationship(userSlug, ideaSlug, fileData.forked_from.user, fileData.forked_from.slug, 'forked_from', {});
    }
    // Branched_from relationship
    if (fileData.branched_from && fileData.branched_from.user && fileData.branched_from.slug) {
        await addRelationship(userSlug, ideaSlug, fileData.branched_from.user, fileData.branched_from.slug, 'branched_from', {});
    }
    ...
    if (fileData.ai_detected?.competes_with && Array.isArray(fileData.ai_detected.competes_with)) {
        for (const competitor of fileData.ai_detected.competes_with) {
            if (competitor.user && competitor.slug) {
                await addRelationship(userSlug, ideaSlug, competitor.user, competitor.slug, 'competes_with', {});
            }
        }
    }
    ...
    if (fileData.ai_detected?.shares_audience_with && Array.isArray(fileData.ai_detected.shares_audience_with)) {
        for (const shared of fileData.ai_detected.shares_audience_with) {
            if (shared.user && shared.slug) {
                await addRelationship(userSlug, ideaSlug, shared.user, shared.slug, 'shares_audience_with', {});
            }
        }
    }
}
