...
import { MonitoringAgent, DetectedIssue, MonitoringConfig } from './monitoring-agent';
...
export function integrateMonitoringWithHub(
  ...
  console.log('[HubIntegration] Connecting MonitoringAgent to CommunicationHub...');
  ...
  communicationHub.on('agent:ready', (data: { agentId: string; session: { agentType: string; sessionId?: string } }) => {
    const { agentId, session } = data;

    // Register with monitoring agent
    monitoringAgent.registerAgent(agentId, session.agentType, session.sessionId);
    ...
    console.log(`[HubIntegration] Agent ready: ${agentId} (${session.agentType})`);
  });

  // Agent blocked (waiting for answer)
  communicationHub.on('agent:blocked', (data: { agentId: string; agentType: string; reason: string; questionIds: string[] }) => {
    ...
    console.log(`[HubIntegration] Agent blocked: ${agentId} (${questionIds.length} questions)`);
  });

  // Agent unblocked (answer received)
  communicationHub.on('agent:unblocked', (data: { agentId: string; agentType: string; previousStatus: string }) => {
  ...
  // Agent halted
  communicationHub.on('agent:halted', (event: { agentId: string; agentType: string; reason: string; details: string }) => {
  ...
  // Agent resumed
  communicationHub.on('agent:resumed', (event: { agentId: string; agentType: string }) => {
    const { agentId } = event;
  ...
  communicationHub.on('answer:received', (answer: {
    questionId: string;
    agentId: string;
    value: string;
    receivedAt: Date;
    ...
    // Record answer
    monitoringAgent.recordQuestionAnswered(agentId, questionId, responseTimeMs);
    ...
    console.log(`[HubIntegration] Answer received for question ${questionId} (${responseTimeMs}ms)`);
  });
  ...
  monitoringAgent.on('issue:detected', async (issue: DetectedIssue) => {
    ...
    emitSystemAlert(issue.description, issue.severity as 'info' | 'warning' | 'error' | 'critical');
    ...
    if (responseEscalator) {
      try {
        ...
        console.log(`[HubIntegration] Issue ${issue.id} handled with ${action.level} response`);
      } catch (error) {
        console.error(`[HubIntegration] Failed to escalate issue ${issue.id}:`, error);
      }
    }
  });
  ...
  monitoringAgent.on('health:check', (metrics: Record<string, number>) => {
    ...
    console.log(`[HubIntegration] Health check: ${metrics.activeAgents} active, ${metrics.blockedAgents} blocked`);
  });
  ...
  if (responseEscalator) {
    responseEscalator.on('response:executed', (action: { level: string; issueId: string; result?: string }) => {
      console.log(`[HubIntegration] Escalation response executed: ${action.level} for ${action.issueId}`);
    });
...
export async function createIntegratedMonitoring(
...
): Promise<{ monitoringAgent: MonitoringAgent; responseEscalator: ResponseEscalator }> {
  ...
  integrateMonitoringWithHub(monitoringAgent, communicationHub, responseEscalator);
