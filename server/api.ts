...
import { updateIdeaStatus, getStatusHistory } from "../utils/status.js";
import { createBranch, getLineage } from "../utils/lineage.js";
import { getIterationHistory } from "../agents/iteration.js";
import { generateProactiveSuggestions } from "../agents/gap-suggestion.js";
import { runDifferentiationAnalysis } from "../agents/differentiation.js";
import { runPositioningAnalysis } from "../agents/positioning.js";
import { validateDifferentiationAnalysis } from "../agents/differentiation-validator.js";
import { generateUpdateSuggestions } from "../agents/update-generator.js";
...
app.use((err: Error, _req: Request, res: Response, _next: NextFunction) => {
  ...
  res
    .status(500)
    .json({ success: false, error: err.message } as ApiResponse<null>);
});
...
app.get(
  "/api/ideas/:slug/category-scores",
  asyncHandler(async (req, res) => {
    const { slug } = req.params;
    const { runId } = req.query;
    ...
    for (const eval_ of evaluations) {
      if (!categoryMap.has(eval_.category)) {
        categoryMap.set(eval_.category, {
          scores: [],
          initialScores: [],
          confidences: [],
          criteria: [],
    ...
    const categoryScores = Array.from(categoryMap.entries()).map(
      ([category, data]) => ({
        ...
        avg_initial_score:
          data.initialScores.reduce((a, b) => a + b, 0) /
          data.initialScores.length,
        avg_confidence:
...
app.get(
  "/api/ideas/:slug/evaluation-runs",
  asyncHandler(async (req, res) => {
    const { slug } = req.params;
    ...
    respond(
      res,
      runs.map((r) => r.evaluation_run_id),
    );
  }),
);
...
app.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (challenges.length === 0) {
      const preliminaryChallenges = await generatePreliminaryChallenges(
        idea.id,
      );
      respond(
        res,
        preliminaryChallenges.map((c) => ({
          ...c,
          is_preliminary: true,
        })),
      );
...
app.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (synthesis) {
      ...
      const recalculatedRecommendation =
        getRecommendationFromScore(recalculatedScore);

      // Get actual weak criteria from evaluations (final_score < 6)
      // final_score is now correctly updated after debate
      const weakEvaluations = await query<{
        criterion: string;
        final_score: number;
      }>(
      ...
      const recalculatedWeaknesses = weakEvaluations.map(
        (e) =>
      ...
      respond(res, {
        ...
        key_weaknesses:
          recalculatedWeaknesses.length > 0
            ? recalculatedWeaknesses
            : JSON.parse(synthesis.key_weaknesses || "[]"),
        critical_assumptions: JSON.parse(
          synthesis.critical_assumptions || "[]",
        ),
        unresolved_questions: JSON.parse(
  ...
  asyncHandler(async (req, res) => {
    ...
    respond(res, {
      ...
      byAgent: Object.fromEntries(
        byOperation.map((a) => [a.operation, a.total]),
      ),
    });
  }),
);
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const idea = await getOne<{ id: string; title: string }>(
      "SELECT id, title FROM ideas WHERE slug = ?",
      [slug],
    );
    if (!idea) {
      res.status(404).json({ success: false, error: "Idea not found" });
    ...
    child.on("exit", (code, signal) => {
      if (code !== 0) {
        ...
        emitDebateEvent("error", slug, runId, {
          error: `Evaluation process exited with code ${code}`,
        });
      } else {
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!type || !ideaSlug || !runId) {
      res.status(400).json({
        success: false,
        error: "Missing required fields: type, ideaSlug, runId",
      });
      return;
    }
...
app.get(
  "/api/ideas/:slug/events/sessions",
  asyncHandler(async (req, res) => {
    const { slug } = req.params;
...
app.get(
  "/api/ideas/:slug/evaluate/status",
  asyncHandler(async (req, res) => {
    const { slug } = req.params;
...
app.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    }>("SELECT id, idea_type, lifecycle_stage FROM ideas WHERE slug = ?", [
      slug,
    ]);
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!questionId || !answer) {
      res
        .status(400)
        .json({ success: false, error: "questionId and answer are required" });
      return;
    }
    ...
    const nextQuestions = await getNextQuestionsAfterAnswer(
      idea.id,
      questionId,
      3,
    );
...
app.delete(
  "/api/ideas/:slug/answers/:questionId",
  asyncHandler(async (req, res) => {
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (mode === "start") {
      ...
      res
        .status(400)
        .json({ success: false, error: "Invalid mode or missing sessionId" });
    }
  }),
);
...
app.get(
  "/api/ideas/:slug/readiness/coverage",
  asyncHandler(async (req, res) => {
    const { slug } = req.params;
...
app.get(
  "/api/ideas/:slug/develop/history",
  asyncHandler(async (req, res) => {
    const { slug } = req.params;
...
app.get(
  "/api/ideas/:slug/questions/smart",
  asyncHandler(async (req, res) => {
    const { slug } = req.params;
...
app.get(
  "/api/ideas/:slug/questions/criterion/:criterion",
  asyncHandler(async (req, res) => {
...
app.post(
  "/api/ideas/:slug/questions/:questionId/suggestions",
  asyncHandler(async (req, res) => {
    const { slug, questionId } = req.params;
    ...
    const ideaContext: IdeaContext = {
      ...
      solution:
        content.match(/## Solution\s*\n([\s\S]*?)(?=\n##|$)/)?.[1]?.trim() ||
        "",
      targetUser:
        content.match(/## Target User\s*\n([\s\S]*?)(?=\n##|$)/)?.[1]?.trim() ||
        "",
      ...
      if (profile) {
        profileContext = {
          ...
          skills: profile.technical_skills
            ...
            : [],
          network: profile.industry_connections
            ? profile.industry_connections.split(",").map((s) => s.trim())
            : [],
...
app.get(
  "/api/ideas/:slug/structured-data",
  asyncHandler(async (req, res) => {
    const { slug } = req.params;
...
app.get(
  "/api/ideas/:slug/versions/:version",
  asyncHandler(async (req, res) => {
    ...
    const versionData = await getVersionSnapshot(
      idea.id,
      parseInt(version, 10),
    );
    if (!versionData) {
...
app.get(
  "/api/ideas/:slug/versions/compare/:v1/:v2",
  asyncHandler(async (req, res) => {
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!title || !reason) {
      res
        .status(400)
        .json({ success: false, error: "Title and reason are required" });
      return;
    }
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const idea = await getOne<{ id: string; status: string }>(
      "SELECT id, status FROM ideas WHERE slug = ?",
      [slug],
    );
    if (!idea) {
      res.status(404).json({ success: false, error: "Idea not found" });
...
app.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const assumptions = await query<{
      ...
      "SELECT * FROM idea_assumptions WHERE idea_id = ? ORDER BY risk_level DESC, created_at DESC",
      [idea.id],
    );
...
app.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const gates = await query<{
      ...
      "SELECT * FROM gate_decisions WHERE idea_id = ? ORDER BY decided_at DESC",
      [idea.id],
    );
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!gateType || !advisoryShown || !userChoice) {
      res.status(400).json({
        success: false,
        error: "gateType, advisoryShown, and userChoice are required",
      });
      return;
    }
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!phase || !uiPhases.includes(phase)) {
      res.status(400).json({
        success: false,
        error: `Invalid phase. Must be one of: ${uiPhases.join(", ")}`,
      });
      return;
    }
    ...
    const idea = await getOne<{ id: string; incubation_phase: string }>(
      "SELECT id, incubation_phase FROM ideas WHERE slug = ?",
      [slug],
    );
    if (!idea) {
      res.status(404).json({ success: false, error: "Idea not found" });
    ...
    await query(
      "UPDATE ideas SET incubation_phase = ?, updated_at = ? WHERE id = ?",
      [dbPhase, new Date().toISOString(), idea.id],
    );
    await saveDb();
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const idea = await getOne<{
      ...
      summary: string | null;
      folder_path: string;
    }>("SELECT id, title, summary, folder_path FROM ideas WHERE slug = ?", [
      slug,
    ]);
    ...
    if (profileLink) {
      const profile = await getOne<{
        ...
        "SELECT primary_goals, technical_skills, professional_network, other_commitments, interests FROM user_profiles WHERE id = ?",
    ...
    const gapAnalysis = {
      assumptions:
        readiness.blockingGaps?.map((gap, idx) => ({
          id: `gap-${idx}`,
          text: gap,
    ...
    try {
      ...
      const opportunities = validatedAnalysis.marketOpportunities.map(
        (opp, idx) => ({
          id: `opp-${idx}`,
          ...
          reasons:
            opp.validationWarnings.length > 0
              ? opp.validationWarnings
              : ["Aligned with profile"],
        }),
      );

      const strategies = validatedAnalysis.differentiationStrategies.map(
        (strat, idx) => ({
          id: `strat-${idx}`,
          approach: strat.name,
      ...
      const competitiveRisks = validatedAnalysis.competitiveRisks.map(
        (risk, idx) => ({
          id: `risk-${idx}`,
      ...
      console.log(
        `Differentiation analysis complete. Cost: $${costReport.estimatedCost.toFixed(4)}, API calls: ${costReport.apiCalls}`,
      );
      ...
      respond(res, {
        ...
        overallConfidence: Math.round(
          validatedAnalysis.overallConfidence * 100,
        ),
      });
    } catch (err) {
...
app.get(
  "/api/ideas/:slug/differentiation",
  asyncHandler(async (req, res) => {
    const { slug } = req.params;
...
app.post(
  "/api/ideas/:slug/generate-update",
  asyncHandler(async (req, res) => {
    const { slug } = req.params;
    ...
    if (!diffResult) {
      res.status(400).json({
        success: false,
        error:
          "No differentiation analysis found. Run differentiation analysis first.",
      });
      return;
    }
    ...
    const qaContext = answersResult.map(
      (a: { question_text: string; answer: string }) => ({
    ...
    // Create cost tracker
    const costTracker = new CostTracker(3.0);

    // Build positioning context from user's decisions
    const positioningContext = positioningDecision
      ? {
          primaryStrategyId:
            positioningDecision.primary_strategy_id || undefined,
          primaryStrategyName:
            positioningDecision.primary_strategy_name || undefined,
          secondaryStrategyName:
            positioningDecision.secondary_strategy_name || undefined,
          timingDecision: positioningDecision.timing_decision as
            | "proceed_now"
            | "wait"
            | "urgent"
            | undefined,
    ...
    const financialCtx = financialAllocation
      ? {
          ...
          allocatedWeeklyHours:
            financialAllocation.allocated_weekly_hours || undefined,
          allocatedRunwayMonths:
            financialAllocation.allocated_runway_months || undefined,
          targetIncome:
            financialAllocation.target_income_from_idea || undefined,
          incomeTimelineMonths:
            financialAllocation.income_timeline_months || undefined,
          ...
          approachRationale:
            financialAllocation.approach_rationale || undefined,
        }
      : undefined;
    ...
    try {
      ...
      const costReport = costTracker.getReport();
      console.log(
        `Update suggestion generated. Cost: $${costReport.estimatedCost.toFixed(4)}`,
      );
      ...
      res.status(500).json({
        success: false,
        error:
          (err as Error).message || "Failed to generate update suggestions",
      });
    }
  }),
);
...
app.get(
  "/api/ideas/:slug/update-suggestion",
  asyncHandler(async (req, res) => {
    const { slug } = req.params;
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!suggestion) {
      res
        .status(404)
        .json({ success: false, error: "Update suggestion not found" });
      return;
    }
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const existing = await getOne<{ id: string }>(
      "SELECT id FROM idea_financial_allocations WHERE idea_id = ?",
      [idea.id],
    );
...
app.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    await query("DELETE FROM idea_financial_allocations WHERE idea_id = ?", [
      idea.id,
    ]);
    await saveDb();
...
app.get(
  "/api/ideas/:slug/positioning-decision",
  asyncHandler(async (req, res) => {
    const { slug } = req.params;
    ...
    const decision = await getOne<{
      ...
      "SELECT * FROM positioning_decisions WHERE idea_id = ? ORDER BY created_at DESC LIMIT 1",
      [idea.id],
    );
    ...
    respond(res, {
      ...
      riskResponseStats: decision.risk_response_stats
        ? JSON.parse(decision.risk_response_stats)
        : null,
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const stats =
      riskResponsesArray.length > 0
        ? calculateRiskResponseStats(riskResponsesArray)
        : null;
...
app.get(
  "/api/ideas/:slug/positioning-decisions",
  asyncHandler(async (req, res) => {
    const { slug } = req.params;
    ...
    const decisions = await query<{
      ...
      "SELECT * FROM positioning_decisions WHERE idea_id = ? ORDER BY created_at DESC",
      [idea.id],
    );
    ...
    respond(
      ...
      decisions.map((d) => ({
        ...
        riskResponseStats: d.risk_response_stats
          ? JSON.parse(d.risk_response_stats)
          : null,
...
app.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const strategicSummary = result.strategic_summary
      ? JSON.parse(result.strategic_summary)
      : null;
...
app.post(
  "/api/ideas/:slug/position",
  asyncHandler(async (req, res) => {
    const { slug } = req.params;
    const { approach } = req.body as { approach: StrategicApproach };

    // Validate approach
    const validApproaches: StrategicApproach[] = [
      "create",
      "copy_improve",
      "combine",
      "localize",
    ...
    if (profileLink) {
      const profile = await getOne<{
        ...
        "SELECT primary_goals, technical_skills, professional_network, other_commitments, interests, current_monthly_income, financial_runway_months, weekly_hours_available, risk_tolerance FROM user_profiles WHERE id = ?",
        [profileLink.profile_id],
      );
      ...
      if (profile) {
        ...
        profileContext = {
          ...
          constraints: parseField(profile.other_commitments),
          interests: parseField(profile.interests),
          currentAnnualIncome: profile.current_monthly_income
            ? profile.current_monthly_income * 12
            : undefined,
    ...
    if (allocationRow) {
      allocation = {
        ...
        allocationPriority: (allocationRow.allocation_priority ||
          "exploration") as "primary" | "secondary" | "exploration" | "parked",
        strategicApproach:
          (allocationRow.strategic_approach as StrategicApproach) || undefined,
        targetIncomeFromIdea:
          allocationRow.target_income_from_idea ?? undefined,
        incomeTimelineMonths: allocationRow.income_timeline_months ?? undefined,
        incomeType: (allocationRow.income_type || "supplement") as
          | "full_replacement"
          | "partial_replacement"
          | "supplement"
          | "wealth_building"
        ...
        ideaRiskTolerance: allocationRow.idea_risk_tolerance as
          | "low"
          | "medium"
          | "high"
          | "very_high"
        ...
        pivotWillingness: (allocationRow.pivot_willingness || "moderate") as
          | "rigid"
          | "moderate"
          | "flexible"
          | "very_flexible",
        ...
        maxTimeToValidateMonths:
          allocationRow.max_time_to_validate_months ?? undefined,
    ...
    try {
      ...
      const costReport = costTracker.getReport();
      console.log(
        `Positioning analysis complete. Cost: $${costReport.estimatedCost.toFixed(4)}, API calls: ${costReport.apiCalls}`,
      );
      ...
      res.status(500).json({
        success: false,
        error:
          err instanceof Error ? err.message : "Positioning analysis failed",
      });
    }
  }),
);
...
export async function startServer(): Promise<void> {
  ...
  try {
    ...
    await hub.start();
    console.log(
      "[TaskAgent] Communication hub started - Telegram polling enabled",
    );
  } catch (error) {
