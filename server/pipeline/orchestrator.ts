...
const VALID_TRANSITIONS: Record<IdeaPhase, IdeaPhase[]> = {
  ...
  'deployed': ['building'],
  'paused': ['ideation', 'ideation_ready', 'specification', 'spec_ready', 'building'],
...
export class PipelineOrchestrator extends EventEmitter {
   ...
   */
  async updateIdeationProgress(ideaId: string, progress: Partial<IdeationProgress>): Promise<void> {
    const state = await this.getState(ideaId);
   ...
   */
  async updateSpecProgress(ideaId: string, progress: Partial<SpecProgress>): Promise<void> {
    const state = await this.getState(ideaId);
   ...
   */
  async updateBuildProgress(ideaId: string, progress: Partial<BuildProgress>): Promise<void> {
    const state = await this.getState(ideaId);
  ...
  private async loadState(ideaId: string): Promise<IdeaState> {
    ...
    return {
      ...
      lastTransition: row.last_transition ? new Date(row.last_transition) : null,
      ...
      specProgress: row.spec_session_id ? {
        ...
        pendingQuestions: row.spec_pending_questions ? JSON.parse(row.spec_pending_questions) : [],
  ...
  private async saveState(ideaId: string, state: IdeaState): Promise<void> {
    ...
    `, [
      ...
      state.specProgress?.pendingQuestions ? JSON.stringify(state.specProgress.pendingQuestions) : null,
  ...
  private async executeTransition(
    ...
    try {
      ...
      // Log transition
      await this.logTransition(ideaId, previousPhase, targetPhase, reason, triggeredBy, true);
      ...
      await this.logTransition(ideaId, previousPhase, targetPhase, reason, triggeredBy, false, errorMsg);
  ...
  private async prepareIdeationHandoff(ideaId: string): Promise<any> {
    ...
    const artifacts = await this.db.all<{
      type: string;
      content: string;
    }>('SELECT type, content FROM ideation_artifacts WHERE idea_id = ?', [ideaId]);
    ...
    const solutionArtifact = artifacts.find(a => a.type === 'solution_description');
  ...
  private async logTransition(
    ...
    await this.db.run(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `, [ideaId, fromPhase, toPhase, reason, triggeredBy, success ? 1 : 0, errorMessage || null]);
  }
