...
export class SpecBridge extends EventEmitter {
   ...
   */
  private async runSpecGeneration(
    session: SpecSession,
    handoff: IdeationHandoff,
  ): Promise<void> {
    ...
    try {
      ...
      const blockingQuestions = output.questions.filter(
        (q) => q.type === "BLOCKING",
      );
...
`;
  ...
  private extractTitle(handoff: IdeationHandoff): string {
    ...
    const titleArtifact = handoff.artifacts.find(
      (a) => a.type === "title" || a.type === "name",
    );
  ...
  private extractRequirements(handoff: IdeationHandoff): string {
    const reqArtifacts = handoff.artifacts.filter(
      (a) =>
  ...
  private extractTechnical(handoff: IdeationHandoff): string {
    const techArtifacts = handoff.artifacts.filter(
      (a) =>
  ...
  private async saveSpecOutput(
    ...
    // Write spec.md
    fs.writeFileSync(path.join(outputDir, "spec.md"), output.spec, "utf-8");

    // Write tasks.md
    fs.writeFileSync(path.join(outputDir, "tasks.md"), output.tasks, "utf-8");
  ...
  async answerQuestions(
    ...
    if (session.status !== "questions") {
      throw new Error(
        `Session is not waiting for questions: ${session.status}`,
      );
    }
