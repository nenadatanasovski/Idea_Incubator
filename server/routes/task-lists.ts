...
router.put('/status', (req: Request, res: Response): void => {
  try {
    ...
    if (!validStatuses.includes(status)) {
      res.status(400).json({ error: 'Invalid status. Must be pending, in_progress, or complete' });
      return;
    }
...
router.post('/task', (req: Request, res: Response): void => {
  try {
    ...
    if (!validPriorities.includes(priority)) {
      res.status(400).json({ error: 'Invalid priority. Must be P1, P2, P3, or P4' });
      return;
    }
    ...
    const success = addTask(absolutePath, section, { id, description, priority });

    if (success) {
      res.json({ success: true, message: `Task ${id} added to section ${section}` });
    } else {
      res.status(404).json({ error: `Section ${section} not found or has no table` });
    }
  } catch (error) {
  ...
  try {
    ...
    const blockingQuestions = await query<{
      ...
      WHERE status = 'pending'
        AND blocking = 1
        AND task_list_name = ?
    `, [path.basename(absolutePath)]).catch(() => []);
    ...
    const enrichedTasks: EnrichedTask[] = taskList.tasks.map(task => {
      ...
      if (blockingQ) {
        enriched.blockingQuestion = {
          ...
          type: blockingQ.type,
          priority: blockingQ.priority >= 8 ? 'critical' : blockingQ.priority >= 6 ? 'high' : 'medium',
          createdAt: blockingQ.created_at,
          options: blockingQ.options ? JSON.parse(blockingQ.options) : undefined,
        };
      }
      ...
      if (task.status === 'in_progress') {
        const now = Date.now();
        const startTime = enriched.startedAt ? new Date(enriched.startedAt).getTime() : 0;
        ...
        } else if (!enriched.assignedAgent && enriched.elapsedMs && enriched.elapsedMs > 30 * 60 * 1000) {
          enriched.isStuck = true;
          enriched.stuckReason = 'No agent assigned';
        } else if (enriched.elapsedMs && enriched.elapsedMs > 2 * 60 * 60 * 1000) {
          enriched.isStuck = true;
...
router.get('/task-detail', (req: Request, res: Response): void => {
  try {
    ...
    const taskDetailPath = path.join(BASE_PATH, 'docs/bootstrap/tasks', `${id}.md`);

    // Check if file exists
    if (!fs.existsSync(taskDetailPath)) {
      res.status(404).json({ error: 'Task detail file not found', path: taskDetailPath });
      return;
    }
    ...
    const passMatch = content.match(/## Pass Criteria[\s\S]*?\| # \| Criterion[\s\S]*?\n([\s\S]*?)(?=\n\n\*\*FAIL|\n---|\n##)/);
    if (passMatch) {
      const criteriaRows = passMatch[1].split('\n').filter(l => l.startsWith('|'));
      passCriteria = criteriaRows.map(row => {
