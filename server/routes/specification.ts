 ...
 */
async function prepareIdeationHandoff(
  ideaId: string,
  ...
  const idea = await getOne<{
    ...
    `SELECT id, title, slug, problem_statement, solution, target_user FROM ideas WHERE id = ?`,
    [ideaId],
  );
  ...
  const ideationSession = await getOne<{
    ...
    `SELECT id, idea_id FROM ideation_sessions WHERE idea_id = ? ORDER BY created_at DESC LIMIT 1`,
    [ideaId],
  );
  ...
  return {
    ...
    artifacts,
    conversationSummary:
      artifacts.find((a) => a.type === "conversation")?.content || "",
  };
}
...
router.post("/:ideaId/start", async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({
      success: false,
      error:
        error instanceof Error ? error.message : "Failed to start spec session",
    });
  }
});
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      res.status(500).json({
        success: false,
        error:
          error instanceof Error ? error.message : "Failed to get spec session",
      });
    }
  },
);
    ...
    res.status(400).json({
      ...
      error:
        error instanceof Error
          ? error.message
          : "Failed to finalize specification",
    });
  }
});
...
router.delete("/:sessionId", async (req: Request, res: Response) => {
  ...
  try {
    const { getSessionManager } =
      await import("../../agents/specification/session-manager.js");
