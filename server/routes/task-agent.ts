...
router.post('/tasks', async (req: Request, res: Response) => {
  try {
    const { title, description, projectId, category, targetTaskListId } = req.body;
...
router.get('/evaluation-queue/stale', async (req: Request, res: Response) => {
  try {
    ...
    const tasks = await evaluationQueueManager.getStaleQueuedTasks(daysThreshold);
    return res.json(tasks);
  } catch (err) {
...
router.get('/grouping-suggestions', async (req: Request, res: Response) => {
  try {
    const projectId = req.query.projectId as string | undefined;
    const suggestions = await autoGroupingEngine.getPendingSuggestions(projectId);
 ...
 */
router.post('/grouping-suggestions/analyze', async (req: Request, res: Response) => {
  try {
 ...
 */
router.post('/grouping-suggestions/:id/accept', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
 ...
 */
router.post('/grouping-suggestions/:id/reject', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
 ...
 */
router.get('/task-lists/:id/parallelism', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const parallelism = await parallelismCalculator.getTaskListParallelism(id);
...
router.post('/agents/:id/heartbeat', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { status, taskId, progressPercent, currentStep, memoryMb, cpuPercent } = req.body;
 ...
 */
router.post('/dependencies/check-cycle', async (req: Request, res: Response) => {
  try {
    const { sourceTaskId, targetTaskId } = req.body;

    if (!sourceTaskId || !targetTaskId) {
      return res.status(400).json({ error: 'sourceTaskId and targetTaskId are required' });
    }

    const result = await circularDependencyPrevention.wouldCreateCycle(sourceTaskId, targetTaskId);
    return res.json(result);
  } catch (err) {
...
router.get('/dependencies/cycles', async (req: Request, res: Response) => {
  try {
    const projectId = req.query.projectId as string | undefined;
    const cycles = await circularDependencyPrevention.detectExistingCycles(projectId);
 ...
 * Body: { sourceTaskId, targetTaskId }
 */
router.post('/dependencies/resolve-cycle', async (req: Request, res: Response) => {
  try {
    const { sourceTaskId, targetTaskId } = req.body;
    ...
    const success = await circularDependencyPrevention.applyResolution(sourceTaskId, targetTaskId);
...
router.post('/dependencies', async (req: Request, res: Response) => {
  try {
    ...
    const result = await circularDependencyPrevention.safeAddDependency(sourceTaskId, targetTaskId);
...
router.get('/tasks/:id/dependencies', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const dependencies = await circularDependencyPrevention.getTaskDependencies(id);
