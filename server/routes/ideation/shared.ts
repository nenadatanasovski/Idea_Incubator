...
import { ideaFolderExists, createDraftFolder, renameDraftToIdea } from '../../../utils/folder-structure.js';
...
export const timeoutMiddleware = (req: Request, res: Response, next: NextFunction) => {
  ...
  req.setTimeout(REQUEST_TIMEOUT_MS, () => {
    if (!res.headersSent) {
      console.error(`[Ideation] Request timeout after ${REQUEST_TIMEOUT_MS}ms for ${req.method} ${req.path}`);
      res.status(504).json({
  ...
  res.setHeader('Keep-Alive', `timeout=${Math.floor(REQUEST_TIMEOUT_MS / 1000)}`);
