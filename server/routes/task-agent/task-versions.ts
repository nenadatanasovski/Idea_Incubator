 ...
 */
router.get(
  "/:taskId/versions/:version",
  async (req: Request, res: Response) => {
    try {
      const { taskId, version } = req.params;
      const v = await taskVersionService.getVersion(
        taskId,
        parseInt(version, 10),
      );
 ...
 */
router.post(
  "/:taskId/versions/restore/preview",
  async (req: Request, res: Response) => {
    try {
      const { taskId } = req.params;
      ...
      const diff = await taskVersionService.previewRestore(
        taskId,
        targetVersion,
      );
      return res.json(diff);
