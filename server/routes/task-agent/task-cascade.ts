 ...
 */
router.post('/tasks/:taskId/cascade/analyze', async (req: Request, res: Response) => {
  try {
    ...
    const analysis = await cascadeAnalyzerService.analyze(taskId, changeType, changes || {});
 ...
 */
router.post('/tasks/:taskId/cascade/execute', async (req: Request, res: Response) => {
  try {
    const { taskId } = req.params;
    ...
    if (selectedTaskIds && Array.isArray(selectedTaskIds)) {
      result = await cascadeExecutorService.executeSelected(analysis, selectedTaskIds);
    } else {
 ...
 */
router.get('/task-lists/:listId/auto-approve', async (req: Request, res: Response) => {
  try {
 ...
 */
router.put('/task-lists/:listId/auto-approve', async (req: Request, res: Response) => {
  try {
    const { listId } = req.params;
