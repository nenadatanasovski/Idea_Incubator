 ...
 */
router.get('/:taskId/history/analytics', async (req: Request, res: Response) => {
  try {
    const { taskId } = req.params;

    // Get various analytics
    const transitionCount = await taskStateHistoryService.getTransitionCount(taskId);
    const lastTransition = await taskStateHistoryService.getLastTransition(taskId);

    // Get time in each status
    const statuses = ['draft', 'pending', 'in_progress', 'completed', 'failed', 'blocked'];
    const timeInStatus: Record<string, number> = {};

    for (const status of statuses) {
      timeInStatus[status] = await taskStateHistoryService.getTimeInStatus(taskId, status as any);
    }
...
router.get('/history/recent', async (req: Request, res: Response) => {
  try {
    ...
    const transitions = await taskStateHistoryService.getRecentTransitions(limit);
