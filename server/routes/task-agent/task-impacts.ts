...
import { estimateFileImpacts, getFileImpacts, validateFileImpacts } from '../../services/task-agent/file-impact-analyzer.js';
 ...
 */
router.put('/:taskId/impacts/:impactId', async (req: Request, res: Response) => {
  try {
 ...
 */
router.delete('/:taskId/impacts/:impactId', async (req: Request, res: Response) => {
  try {
    const { impactId } = req.params;
 ...
 */
router.post('/:taskId/impacts/estimate', async (req: Request, res: Response) => {
  try {
    const { taskId } = req.params;

    // Get task details
    const task = await getOne<{ title: string; description: string; category: string }>(
 ...
 */
router.post('/:taskId/impacts/validate', async (req: Request, res: Response) => {
  try {
    const { taskId } = req.params;
    ...
    const result = await validateFileImpacts(taskId, actualFiles.map(f => ({
