...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!session) {
      res
        .status(404)
        .json({ success: false, error: "Debate session not found" });
      return;
    }
    ...
    if (synthesis) {
      ...
      const categoryScoresForRecalc = await query<{
        category: string;
        avg_score: number;
      }>(
    ...
    const maxRoundNumber =
      rounds.length > 0 ? Math.max(...rounds.map((r) => r.round_number)) : 1;
