...
router.get(
  ...
  asyncHandler(async (_req, res) => {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="ideas-export-${new Date().toISOString().split("T")[0]}.json"`,
    );
    res.json({
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!data || !data.ideas || !Array.isArray(data.ideas)) {
      res.status(400).json({
        success: false,
        error: "Invalid import format. Expected { ideas: [...] }",
      });
      return;
    }
    ...
    for (const ideaData of data.ideas) {
      try {
        ...
        const tagsArray =
          ideaData.tags && Array.isArray(ideaData.tags) ? ideaData.tags : [];
...
`;
        ...
        results.errors.push(
          `Failed to import ${ideaData.slug}: ${(error as Error).message}`,
        );
      }
    }
...
router.get(
  ...
  asyncHandler(async (_req, res) => {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="ideas-export-${new Date().toISOString().split("T")[0]}.csv"`,
    );
    res.send(csv);
  }),
);
