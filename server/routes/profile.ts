...
router.put("/", async (req, res) => {
  try {
    return res.json(
      await profileService.updateProfile(getUserId(req), req.body),
    );
  } catch (error: unknown) {
...
router.post("/avatar", upload.single("avatar"), async (req, res) => {
  try {
    ...
    return res.json(
      await avatarHandler.processUpload(getUserId(req), req.file.buffer),
    );
  } catch (error: unknown) {
...
router.put("/preferences", async (req, res) => {
  try {
    return res.json(
      await preferencesManager.updatePreferences(getUserId(req), req.body),
    );
  } catch (error: unknown) {
