...
export type BindingStatus = 'assigned' | 'active' | 'completed' | 'failed' | 'abandoned';
export type ExitReason = 'success' | 'error' | 'timeout' | 'cancelled' | 'reassigned';
    ...
    if (specPath) {
      ...
      if (fs.existsSync(tasksPath)) {
        try {
          ...
          const taskBlocks = content.split(/---\s*\n/).filter(b => b.includes('id:'));
...
router.put('/:id/status', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const validStatuses = ['pending', 'in_progress', 'complete', 'failed', 'blocked'];
    ...
    for (const block of taskBlocks) {
      ...
      for (const line of lines) {
        ...
        switch (key.trim()) {
          ...
          case 'phase': task.phase = value; break;
          case 'action': task.action = value; break;
          case 'file': task.file = value; break;
          case 'status': task.status = value as TaskStatus; break;
