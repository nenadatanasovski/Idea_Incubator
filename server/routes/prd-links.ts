 ...
 */
router.delete('/:prdId/link-list/:taskListId', async (req: Request, res: Response) => {
  try {
...
router.post('/:prdId/link-task', async (req: Request, res: Response) => {
  try {
    ...
    const link = await prdLinkService.linkTask(prdId, taskId, requirementRef, linkType);
    return res.status(201).json(link);
  } catch (err) {
 ...
 */
router.delete('/:prdId/link-task/:taskId', async (req: Request, res: Response) => {
  try {
