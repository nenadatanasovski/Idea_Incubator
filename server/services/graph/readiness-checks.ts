 ...
 * Memory Graph Migration: Provides comprehensive readiness checking
 * for spec, build, and launch phases.
 */
...
export async function checkSpecReadiness(
  ...
  if (!result.stats || result.blocks.length === 0) {
    return {
      ...
      recommendations: [
        "Start by describing the core problem you are solving",
        "Add blocks describing your solution approach",
        "Define specific requirements for the solution",
      ],
    };
  }
  ...
  if (!result.stats?.byGraphMembership.problem) {
    ...
    recommendations.push(
      "Add blocks describing the core problem you are solving",
    );
  }
  ...
  if (!result.stats || result.blocks.length === 0) {
    return {
      ...
      recommendations: [
        ...
        "Define your market positioning vs competitors",
...
/**
 * Get overall readiness status for an idea
 */
export async function getOverallReadiness(ideaId: string): Promise<{
  ...
  const [spec, build, launch] = await Promise.all([
    checkSpecReadiness(ideaId),
    checkBuildReadiness(ideaId),
    checkLaunchReadiness(ideaId),
  ]);

  // Determine current phase based on readiness
  let currentPhase: "ideation" | "spec" | "build" | "launch";
  const nextSteps: string[] = [];

  if (!spec.ready) {
    currentPhase = "ideation";
    nextSteps.push(...spec.recommendations.slice(0, 3));
  } else if (!build.ready) {
    currentPhase = "spec";
    nextSteps.push(...build.recommendations.slice(0, 3));
  } else if (!launch.ready) {
    currentPhase = "build";
    nextSteps.push(...launch.recommendations.slice(0, 3));
  } else {
    currentPhase = "launch";
    nextSteps.push("Idea is ready for launch!");
  }
