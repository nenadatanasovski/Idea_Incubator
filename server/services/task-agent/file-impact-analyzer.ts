 ...
 */
const DEFAULT_CATEGORY_PATTERNS: Partial<Record<TaskCategory, Array<{ pattern: string; operation: FileOperation }>>> = {
  feature: [
  ...
  research: [
    { pattern: 'docs/research/**/*.md', operation: 'CREATE' },
  ],
  infrastructure: [
  ...
  security: [
    { pattern: 'server/middleware/**/*.ts', operation: 'UPDATE' },
  ],
  performance: [
    { pattern: '**/*.ts', operation: 'UPDATE' },
  ],
...
export async function estimateFileImpacts(
  ...
  const historicalPatterns = await matchHistoricalPatterns(title, description || '', category);
  ...
  const fileIndicators = [
    { keywords: ['database', 'migration', 'schema', 'sql'], pattern: 'database/**/*', operation: 'UPDATE' as FileOperation },
    { keywords: ['api', 'route', 'endpoint'], pattern: 'server/routes/**/*.ts', operation: 'UPDATE' as FileOperation },
    { keywords: ['component', 'ui', 'frontend', 'react'], pattern: 'components/**/*.tsx', operation: 'UPDATE' as FileOperation },
    { keywords: ['test', 'spec'], pattern: 'tests/**/*', operation: 'UPDATE' as FileOperation },
    { keywords: ['type', 'interface', 'typescript'], pattern: 'types/**/*.ts', operation: 'UPDATE' as FileOperation },
    { keywords: ['config', 'configuration', 'settings'], pattern: 'config/**/*', operation: 'UPDATE' as FileOperation },
    { keywords: ['service', 'business logic'], pattern: 'server/services/**/*.ts', operation: 'UPDATE' as FileOperation },
  ];
  ...
  for (const impact of impacts) {
    await run(
      `INSERT INTO task_file_impacts (id, task_id, file_path, operation, confidence, source)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [impact.id, impact.taskId, impact.filePath, impact.operation, impact.confidence, impact.source]
    );
  }
...
export async function matchHistoricalPatterns(
  ...
  if (titleWords.length > 0) {
    ...
    for (const pattern of keywordPatterns) {
      if (!patterns.some((p) => p.file_pattern === pattern.file_pattern && p.operation === pattern.operation)) {
...
export async function mergeEstimates(
  taskId: string,
  estimates: Array<{ filePath: string; operation: FileOperation; confidence: number; source: FileImpactSource }>
): Promise<FileImpact[]> {
  ...
  for (const [, group] of grouped) {
    ...
    const sourcePriority: FileImpactSource[] = ['user_declared', 'validated', 'ai_estimate', 'pattern_match'];
    const bestSource = group.reduce((best, curr) =>
      sourcePriority.indexOf(curr.source) < sourcePriority.indexOf(best.source) ? curr : best
    );
...
export async function validateFileImpacts(
  ...
  const actualSet = new Set(actualChanges.map((c) => `${c.filePath}:${c.operation}`));
  const predictedSet = new Set(predicted.map((p) => `${p.file_path}:${p.operation}`));
  ...
  const missed = actualChanges.filter((a) => !predictedSet.has(`${a.filePath}:${a.operation}`)).length;
...
export async function recordActualImpact(
  ...
  await run(
    `INSERT INTO task_file_changes (id, task_id, file_path, operation, lines_added, lines_removed)
     VALUES (?, ?, ?, ?, ?, ?)`,
    [uuidv4(), taskId, filePath, operation, linesAdded || null, linesRemoved || null]
  );
  await saveDb();
}
...
export async function updatePatternAccuracy(
  ...
  for (const impact of validatedImpacts) {
    ...
    const existing = await getOne<{ id: string; match_count: number; accuracy_rate: number }>(
    ...
    if (existing) {
      ...
      const newAccuracy = (existing.accuracy_rate * existing.match_count + (impact.was_accurate === 1 ? 1 : 0)) / newCount;
