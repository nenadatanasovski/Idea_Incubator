...
export class CascadeAnalyzerService {
  ...
  async analyze(
    ...
    const transitiveEffects = await this.findTransitivelyAffected(directEffects, this.maxDepth);
  ...
  async findDirectlyAffected(taskId: string, changeType: CascadeTrigger): Promise<CascadeEffect[]> {
    ...
    // Get the source task
    const sourceTask = await getOne<{ task_list_id: string; display_id: string }>(
      'SELECT task_list_id, display_id FROM tasks WHERE id = ?',
      [taskId]
    );
    ...
    const dependentTasks = await query<{ source_task_id: string; display_id: string; title: string }>(
    ...
    if (changeType.includes('impact_changed')) {
      ...
      const overlappingTasks = await this.findOverlappingTasks(taskId, sourceImpacts);
  ...
  async detectFileConflicts(taskId: string, newImpacts: TaskImpact[]): Promise<ImpactConflict[]> {
    ...
    for (const impact of newImpacts) {
      ...
      const overlapping = await query<{ task_id: string; operation: string; impact_type: string }>(
      ...
      for (const other of overlapping) {
        const conflictType = this.getConflictType(impact.operation, other.operation as any);
        if (conflictType) {
          conflicts.push({
            taskAId: taskId,
  ...
  ): Promise<{ taskId: string; displayId: string; impactType: string; targetPath: string }[]> {
    const overlapping: { taskId: string; displayId: string; impactType: string; targetPath: string }[] = [];

    for (const impact of sourceImpacts) {
      const tasks = await query<{ task_id: string; display_id: string; impact_type: string }>(
   ...
   */
  private determineSuggestedAction(trigger: CascadeTrigger): CascadeSuggestedAction {
    switch (trigger) {
      case 'status_changed':
        return 'notify';
