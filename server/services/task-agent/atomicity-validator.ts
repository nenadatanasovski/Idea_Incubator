...
export class AtomicityValidator {
  ...
  async validate(task: Task): Promise<AtomicityResult> {
    ...
    const isAtomic = avgScore >= THRESHOLDS.minScore && rules.every(r => r.score >= 50);

    // Generate split suggestions if not atomic
    const suggestedSplits = isAtomic ? undefined : await this.suggestSplits(task, rules);
  ...
  async checkSingleConcern(task: Task): Promise<RuleResult> {
    ...
    const componentIndicators = ['database', 'api', 'ui', 'frontend', 'backend', 'test'];
    const mentionedComponents = componentIndicators.filter(c => text.includes(c));
   ...
   */
  async checkBoundedFiles(task: Task, maxFiles: number = THRESHOLDS.maxFiles): Promise<RuleResult> {
   ...
   */
  async checkTimeBounded(task: Task, maxHours: number = THRESHOLDS.maxHours): Promise<RuleResult> {
    ...
    const score = estimatedHours <= maxHours ? 100 : Math.max(20, 100 - (estimatedHours - maxHours) * 10);

    return {
      rule: 'time_bounded',
      passed: estimatedHours <= maxHours,
      score,
      reason: estimatedHours > maxHours
  ...
  async checkTestable(task: Task): Promise<RuleResult> {
    ...
    const hasTestableLanguage = testableIndicators.some(re => re.test(description));
  ...
  async checkClearCompletion(task: Task): Promise<RuleResult> {
    ...
    return {
      ...
      reason: score < 70
        ? 'Task has vague completion criteria'
        : undefined,
    };
  }
   ...
   */
  private async suggestSplits(task: Task, rules: RuleResult[]): Promise<string[]> {
    ...
    for (const rule of failedRules) {
      switch (rule.rule) {
        case 'single_concern':
          suggestions.push('Split into separate tasks for each component/concern');
          break;
        case 'bounded_files':
        ...
        case 'time_bounded':
          ...
          break;
        case 'testable':
          suggestions.push('Add explicit acceptance criteria before proceeding');
          break;
        case 'independent':
