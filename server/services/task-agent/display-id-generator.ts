...
export async function generateDisplayId(
  ...
  while (attempts < MAX_RETRY_ATTEMPTS) {
    ...
    attempts++;
    console.warn(
      `[DisplayIdGenerator] Collision on ${displayId}, retrying (attempt ${attempts})`,
    );
  }
...
export function parseDisplayId(displayId: string): {
  ...
  const match = displayId.match(
    /^([A-Z]{2})-([A-Z]{2,4})-([A-Z]{3})-(\d{3,})(?:-[A-Z0-9]+)?$/,
  );
...
export async function backfillDisplayIds(): Promise<{
  ...
  for (const task of tasksWithoutDisplayId) {
    try {
      ...
      await run("UPDATE tasks SET display_id = ? WHERE id = ?", [
        displayId,
        task.id,
      ]);
      ...
      failed++;
      errors.push(
        `Failed to generate display ID for task ${task.id}: ${error}`,
      );
    }
  }
