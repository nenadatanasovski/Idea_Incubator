...
const coverageGapChallenges: Record<string, (ideaTitle: string) => PreliminaryChallenge[]> = {
  problem: (title) => [
    {
...
function generateRecommendation(readinessPercent: number): {
  ...
  if (readinessPercent >= 80) {
    return {
      ...
      reasoning: 'The idea has strong coverage across evaluation criteria. Core assumptions have been documented and the foundation is solid for moving to validation.'
    };
  } else if (readinessPercent >= 50) {
    return {
      recommendation: 'REFINE',
      reasoning: 'The idea shows promise but has gaps in critical areas. More development work is needed to address unanswered questions before committing significant resources.'
    };
  } else if (readinessPercent >= 25) {
    return {
      recommendation: 'PAUSE',
      reasoning: 'The idea is still too early-stage for meaningful evaluation. Significant work is needed to clarify the problem, solution, and market assumptions.'
    };
  } else {
    return {
      recommendation: 'ABANDON',
      reasoning: 'The idea lacks sufficient development to evaluate. Consider whether this is worth pursuing or if there are fundamental issues to address first.'
    };
  }
}
 ...
 */
export async function generatePreliminaryChallenges(ideaId: string): Promise<PreliminaryChallenge[]> {
  ...
  const categoryCoverage = coverage.reduce((acc, c) => {
    if (!acc[c.category]) {
  ...
  for (const [category, data] of Object.entries(categoryCoverage)) {
    const coveragePercent = data.total > 0 ? (data.answered / data.total) * 100 : 0;
  ...
  if (idea.lifecycle_stage === 'spark' || idea.lifecycle_stage === 'clarify') {
    challenges.push({
      ...
      category: 'problem',
      criterion: 'P4',
      challenge: 'At this early stage, validation is critical. What is the cheapest, fastest experiment to test the core assumption?',
      source: 'idea_pattern',
      addressed: false
    });
  }
 ...
 */
export async function generatePreliminarySynthesis(ideaId: string): Promise<PreliminarySynthesis | null> {
  const idea = await getOne<IdeaData>(`
    SELECT id, title, summary, idea_type, lifecycle_stage
    FROM ideas WHERE id = ?
  `, [ideaId]);
  ...
  for (const [category, data] of Object.entries(categoryCoverage)) {
    ...
    if (percent >= 70) {
      strengths.push(`${categoryName} is well-documented with ${Math.round(percent)}% of questions answered`);
    } else if (percent < 30) {
      weaknesses.push(`${categoryName} needs more development - only ${Math.round(percent)}% of questions answered`);
    }
  }
  ...
  for (const [category, data] of Object.entries(categoryCoverage)) {
    const percent = data.total > 0 ? (data.answered / data.total) * 100 : 0;
    if (percent < 20) {
      assumptions.push(`${category.replace(/_/g, ' ')} aspects are assumed but unvalidated`);
    }
  }
  ...
  return {
    ...
    key_strengths: strengths.length > 0 ? strengths : ['Idea has been captured and is being developed'],
    key_weaknesses: weaknesses.length > 0 ? weaknesses : ['Full evaluation has not been completed'],
    critical_assumptions: assumptions,
    unresolved_questions: unresolvedQuestions.length > 0 ? unresolvedQuestions : ['What problem does this solve?', 'Who is the target user?', 'How will this be built?'],
