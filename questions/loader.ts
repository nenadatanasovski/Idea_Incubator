...
export async function populateQuestionBank(force: boolean = false): Promise<number> {
...
export async function getAllQuestions(): Promise<QuestionWithCategory[]> {
  ...
  return rows.map(row => ({
    ...
    lifecycle_stages: row.lifecycle_stages ? JSON.parse(row.lifecycle_stages) : null,
...
export async function getRelevantQuestions(
  ...
  return allQuestions.filter(q => {
    ...
    if (lifecycleStage && q.lifecycle_stages && !q.lifecycle_stages.includes(lifecycleStage)) {
      return false;
    }
...
export async function getQuestionStats(): Promise<{
  ...
  return {
    ...
    byPriority: Object.fromEntries(priorityResult.map(r => [r.priority, r.count])),
    byCriterion: Object.fromEntries(criterionResult.map(r => [r.criterion, r.count]))
  };
}
