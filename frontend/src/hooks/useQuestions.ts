...
export function useAnswers(slug: string | undefined) {
  ...
  const fetchAnswers = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err : new Error("Failed to fetch answers"),
      );
    } finally {
      setLoading(false);
    }
  ...
  const submit = useCallback(
    async (
      ...
      if (!slug) return null;
      try {
        const result = await submitAnswer(slug, {
          questionId,
          answer,
          sessionId,
        });
  ...
  const remove = useCallback(
    async (questionId: string): Promise<void> => {
      ...
      try {
        ...
        setError(
          err instanceof Error ? err : new Error("Failed to delete answer"),
        );
        throw err;
      }
    },
...
export function useReadiness(slug: string | undefined) {
  ...
  const fetchReadiness = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err : new Error("Failed to fetch readiness"),
      );
    } finally {
      setLoading(false);
    }
...
export function useDevelopment(slug: string | undefined) {
  ...
  const submitAndUpdate = useCallback(
    async (questionId: string, answer: string, sessionId?: string) => {
