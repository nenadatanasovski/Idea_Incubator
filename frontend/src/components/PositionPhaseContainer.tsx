...
type PositionStep = 'allocation' | 'approach' | 'analysis' | 'compare' | 'decide';
...
export default function PositionPhaseContainer({
  ...
  const [allocation, setAllocation] = useState<IdeaFinancialAllocation | null>(null);
  const [selectedApproach, setSelectedApproach] = useState<StrategicApproach | null>(null);
  const [analyzedApproach, setAnalyzedApproach] = useState<StrategicApproach | null>(null); // Track which approach was analyzed
  ...
  const [risks, setRisks] = useState<Array<{ id: string; description: string; severity: 'high' | 'medium' | 'low'; mitigation?: string }>>([]);
  const [strategicSummary, setStrategicSummary] = useState<StrategicSummary | null>(null);
  const [selectedStrategyId, setSelectedStrategyId] = useState<string | null>(null);
  const [detailStrategy, setDetailStrategy] = useState<EnhancedStrategy | null>(null);
  ...
  const populateFromResults = (results: PositioningAnalysisResult) => {
    setStrategies(results.strategies.map((s, i) => ({
      ...s,
      id: s.id || `strategy-${i}`,
    })));
    setOpportunities(results.marketOpportunities.map((o, i) => ({
      ...o,
      id: o.id || `opp-${i}`,
    })) as ValidatedOpportunity[]);
    setRisks(results.competitiveRisks.map((r, i) => ({
      id: r.id || `risk-${i}`,
  ...
  const handleApproachComplete = async (forceRerun = false) => {
    ...
    if (!forceRerun && strategies.length > 0 && analyzedApproach === selectedApproach) {
      setCurrentStep('compare');
      return;
    }
    ...
    try {
      ...
      setAnalysisProgress('Running AI analysis with your strategic approach...');
      ...
      if (results && results.strategies && results.strategies.length > 0) {
        ...
        setAnalysisError('Analysis completed but no strategies were generated. Please try again or adjust your approach.');
      }
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Analysis failed. Please try again.';
      // Check for specific error cases
      if (message.includes('viability gate') || message.includes('readiness')) {
        setAnalysisError('Your idea needs to pass the viability gate first. Please complete more questions in the Clarify phase to reach 50% readiness.');
      } else {
          ...
          <div className="text-center py-12">
            {analysisLoading ? (
              <div>
                ...
                <p className="text-lg font-medium text-gray-900">Running Positioning Analysis...</p>
                <p className="text-gray-500 mt-2">
                  {analysisProgress || 'Analyzing market opportunities and generating strategies'}
                </p>
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between bg-gray-50 border border-gray-200 rounded-lg px-4 py-3">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm text-gray-600">
                  Analysis complete for <span className="font-medium text-gray-900">{analyzedApproach}</span> approach
                </span>
                <span className="text-xs text-gray-400">•</span>
                <span className="text-xs text-gray-500">{strategies.length} strategies generated</span>
                <span className="text-xs text-gray-400">•</span>
                <span className="text-xs text-gray-500">{opportunities.length} opportunities</span>
                <span className="text-xs text-gray-400">•</span>
                <span className="text-xs text-gray-500">{risks.length} risks</span>
              </div>
              <button
            ...
            {strategicSummary && (
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-5">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  <div>
                    <h4 className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">Recommended Strategy</h4>
                    <p className="font-semibold text-gray-900 text-lg">{strategicSummary.recommendedStrategy.name}</p>
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-sm font-medium text-blue-600">{strategicSummary.recommendedStrategy.fitScore}/10 fit</span>
                      <span className="text-xs text-gray-400">profile match</span>
                    </div>
                  </div>
                  <div>
                    <h4 className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">Primary Opportunity</h4>
                    <p className="font-medium text-gray-900 text-sm leading-relaxed">{strategicSummary.primaryOpportunity.segment}</p>
                  </div>
                  <div>
                    <h4 className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">Key Risk to Mitigate</h4>
                    <p className="font-medium text-gray-900 text-sm leading-relaxed" title={strategicSummary.criticalRisk.description}>
                      {strategicSummary.criticalRisk.description}
                    </p>
                  </div>
                  <div>
                    <h4 className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">Market Timing</h4>
                    <div className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-full text-xs font-semibold ${
                      strategicSummary.timingAssessment.urgency === 'high'
                        ...
                        : strategicSummary.timingAssessment.urgency === 'medium'
                      ...
                      {strategicSummary.timingAssessment.urgency.toUpperCase()} URGENCY
                    </div>
                    <p className="text-xs text-gray-600 mt-2 leading-relaxed">{strategicSummary.timingAssessment.window}</p>
                  </div>
                </div>
              </div>
            )}
            ...
            {detailStrategy && (
              <div className="fixed inset-0 z-50 overflow-hidden">
                <div className="absolute inset-0 bg-black/30" onClick={handleDetailClose} />
                <div className="absolute right-0 top-0 bottom-0 w-full max-w-xl bg-white shadow-xl overflow-y-auto">
                  <StrategyDetailPanel
                    ...
                    onSelect={() => {
                      setSelectedStrategyId(detailStrategy.id || detailStrategy.name);
                      handleProceedToDecision();
                    }}
                    isSelected={selectedStrategyId === (detailStrategy.id || detailStrategy.name)}
                  />
                </div>
              </div>
            )}
