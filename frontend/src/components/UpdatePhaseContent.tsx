...
import { Loader2, Sparkles, Check, X, ChevronDown, ChevronUp, RefreshCw } from 'lucide-react'
  ...
  if (isGenerating) {
    return (
      <div className="card text-center py-12">
        ...
        <h3 className="text-lg font-semibold text-gray-900 mb-2">Generating AI Updates</h3>
        <p className="text-gray-500">
          Analysing Q&A, User Profile, chosen positioning strategy, and market analysis to suggest changes to the current Idea...
        </p>
      </div>
    )
  }
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-gray-600 mb-4">
                Based on your positioning analysis, our AI can suggest updates to make your idea
                more compelling, focused, and aligned with your chosen strategic approach.
              </p>

              <div className="bg-gray-50 rounded-lg p-4 mb-4">
                <h4 className="text-sm font-medium text-gray-700 mb-2">Current Idea</h4>
    ...
    <div className="space-y-6">
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-900">AI Suggestions Ready</h3>
          ...
          <div className="flex gap-4 mb-4">
            {suggestion.positioningStrategy && (
              <div className="flex-1 p-3 bg-purple-50 rounded-lg">
                <div className="text-xs text-purple-600 font-medium">Positioning Strategy</div>
                <div className="text-sm text-gray-900">{suggestion.positioningStrategy}</div>
              </div>
            )}
            {suggestion.targetSegment && (
              <div className="flex-1 p-3 bg-cyan-50 rounded-lg">
                <div className="text-xs text-cyan-600 font-medium">Target Segment</div>
                <div className="text-sm text-gray-900">{suggestion.targetSegment}</div>
              </div>
            )}
          </div>
        )}
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
            ...
            <div className="text-gray-900 font-medium">{suggestion.suggestedTitle}</div>
          </div>
        </div>
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="p-3 bg-gray-50 rounded-lg">
            <div className="text-xs text-gray-500 mb-1">Current</div>
            <div className="text-gray-900 text-sm">{currentSummary || 'No summary'}</div>
          </div>
          <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
            <div className="text-xs text-green-600 mb-1">Suggested</div>
            <div className="text-gray-900 text-sm">{suggestion.suggestedSummary}</div>
          </div>
        </div>
      ...
      <div className="card">
        <button
          ...
          className="w-full flex items-center justify-between"
        >
          <h4 className="text-sm font-medium text-gray-500">Why These Changes?</h4>
          {showRationale ? (
            <ChevronUp className="h-4 w-4 text-gray-400" />
          ) : (
            <ChevronDown className="h-4 w-4 text-gray-400" />
        ...
        {showRationale && (
          <div className="mt-4 space-y-3">
            <p className="text-sm text-gray-700">{suggestion.changeRationale?.overall}</p>
            {suggestion.keyInsightsIncorporated && suggestion.keyInsightsIncorporated.length > 0 && (
              <div>
                <div className="text-xs text-gray-500 mb-2">Key insights incorporated:</div>
