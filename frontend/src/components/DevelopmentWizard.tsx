...
import { X, ChevronLeft, ChevronRight, CheckCircle, List, Sparkles, ChevronDown, ChevronUp } from 'lucide-react'
import QuestionCard from './QuestionCard'
import ReadinessMeter from './ReadinessMeter'
import type { Question, ReadinessScore, DevelopmentSession, Answer, QuestionCategory } from '../types'
...
export default function DevelopmentWizard({
  ...
  const [selectedQuestion, setSelectedQuestion] = useState<GroupedQuestion | null>(null)
  ...
  useEffect(() => {
    ...
    const startSession = async () => {
      ...
      try {
        ...
        const sessionRes = await fetch(`${API_BASE}/api/ideas/${ideaSlug}/develop`, {
          method: 'POST',
  ...
  const fetchAllQuestions = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/ideas/${ideaSlug}/questions/all`)
  ...
  const handleAnswer = async (questionId: string, answer: string) => {
    ...
    try {
      ...
      setSubmitError((err as Error).message || 'Failed to save answer. Please try again.')
  ...
  const loadMoreQuestions = async () => {
    try {
      ...
      let res = await fetch(`${API_BASE}/api/ideas/${ideaSlug}/questions/smart?limit=10`)
      if (!res.ok) throw new Error('Failed to load questions')
      ...
      if (!data.data || data.data.length === 0) {
        res = await fetch(`${API_BASE}/api/ideas/${ideaSlug}/questions/smart?limit=10&skipDependencies=true`)
        if (!res.ok) throw new Error('Failed to load questions')
        data = await res.json()
      }
      ...
      if (data.success && data.data?.length > 0) {
        ...
        const existingIds = new Set(questions.map(q => q.id))
        const newQuestions = data.data.filter((q: Question) =>
          !existingIds.has(q.id) && !answers.has(q.id)
        )
  ...
  const answeredInSession = Array.from(answers.keys()).filter(
    id => questions.some(q => q.id === id)
  ).length
  const progressPercent = questions.length > 0 ? (answeredInSession / questions.length) * 100 : 0
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col">
        ...
        <div className="px-6 py-4 border-b border-gray-200">
          <div className="flex items-center justify-between mb-3">
            <div>
              <h2 className="text-xl font-bold text-gray-900">Develop Your Idea</h2>
        ...
        {activeTab === 'session' && (
          <div className="px-6 py-2 bg-gray-50 border-b border-gray-200">
            <div className="flex items-center justify-between text-sm text-gray-600 mb-1">
              ...
              <span>{answeredInSession} of {questions.length} questions answered</span>
            </div>
              ...
              <>
                {loading ? (
                  ...
                  <div>
                    ...
                    <div className="mb-4 flex items-center justify-between">
                      <span className="text-sm font-medium text-gray-500">
                        {categoryNames[currentQuestion.category as QuestionCategory]}
                      </span>
                  ...
                  <div className="text-center py-8">
                    ...
                    <h3 className="text-xl font-semibold text-gray-900 mb-2">Session Questions Complete</h3>
                    <p className="text-gray-500 mb-4">
                      You've answered all questions in this session, but there are {totalQuestions - totalAnswered} more questions available.
                    </p>
                    <div className="flex gap-3 justify-center">
                      <button onClick={loadMoreQuestions} className="btn btn-primary">
                        Load More Questions
                      </button>
                      <button onClick={() => handleTabChange('all')} className="btn btn-secondary">
                  ...
                  <div className="text-center py-8">
                    ...
                    <h3 className="text-xl font-semibold text-gray-900 mb-2">All Questions Answered!</h3>
                    <p className="text-gray-500 mb-4">
                      You've answered all available questions. Your idea is ready for evaluation.
                    </p>
                ...
                {loadingAllQuestions ? (
                  ...
                  <div>
                    ...
                    <div className="mb-4 flex items-center gap-2">
                      <span className="text-sm font-medium text-gray-500">
                        {categoryNames[selectedQuestion.category as QuestionCategory]}
                      </span>
                      <span className={`text-xs px-2 py-0.5 rounded ${
                        selectedQuestion.priority === 'critical' ? 'bg-red-100 text-red-700' :
                        selectedQuestion.priority === 'important' ? 'bg-amber-100 text-amber-700' :
                        'bg-gray-100 text-gray-600'
                      }`}>
                        {priorityMeta[selectedQuestion.priority as keyof typeof priorityMeta]?.label || selectedQuestion.priority}
                      </span>
                    </div>
                    ...
                    <QuestionCard
                      ...
                      question={{
                        ...
                        priority: selectedQuestion.priority as 'critical' | 'important' | 'nice-to-have',
                        type: 'factual',
                        idea_types: null,
                              ...
                              {questions.map(q => (
                                ...
                                >
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <p className={`text-sm ${q.answered ? 'text-gray-500' : 'text-gray-900'}`}>
                                      {q.text}
                                    </p>
                                    <div className="flex items-center gap-2 mt-1">
                                      ...
                                      <span className={`text-xs px-1.5 py-0.5 rounded ${
                                        ...
                                        {priorityMeta[q.priority as keyof typeof priorityMeta]?.label || q.priority}
                                      </span>
                                    </div>
                                  </div>
