...
export default function StrategicApproachSelector({
  ...
  const [selectedApproach, setSelectedApproach] = useState<StrategicApproach | null>(
    allocation?.strategicApproach ?? null
  );
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [showDetails, setShowDetails] = useState<StrategicApproach | null>(null);
    ...
    for (const approach of approaches) {
      ...
      if (allocation) {
        ...
        if (approach === 'create') {
          if (runway >= 18) {
            score += 15;
            reasons.push('Your 18+ month runway supports long development cycles');
          } else if (runway < 12) {
            score -= 20;
        ...
        if (allocation.incomeType === 'full_replacement') {
          if (approach === 'copy_improve' || approach === 'localize' || approach === 'specialize') {
            score += 15;
            reasons.push('Proven models provide more predictable income');
          } else if (approach === 'create') {
            score -= 10;
            concerns.push('New category creation has unpredictable income timeline');
          }
        ...
        if (risk === 'high' || risk === 'very_high') {
          ...
          if (approach === 'copy_improve' || approach === 'localize') {
            score += 15;
            reasons.push('Lower risk approaches match your conservative preference');
          } else if (approach === 'create') {
            score -= 15;
      ...
      if (profile) {
        ...
        if (profileAny.city || profileAny.country) {
          if (approach === 'localize') {
            score += 10;
            reasons.push(`Your knowledge of ${profileAny.city || profileAny.country} is an advantage`);
          }
        }
      }
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h3 className="text-lg font-semibold">Choose Your Strategic Approach</h3>
        <p className="text-gray-600 mt-1">
          Based on your goals and resources, select how you want to approach this market.
          The system will tailor its analysis accordingly.
        </p>
      </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="font-medium text-green-800">
                Recommended: {strategicApproachMeta[recommendations[0].approach].label}
              </p>
          ...
          return (
            ...
            >
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  setShowDetails(showDetails === rec.approach ? null : rec.approach);
                }}
                  ...
                  {rec.reasons.length > 0 && (
                    <div>
                      ...
                      <ul className="mt-1 space-y-1">
                        {rec.reasons.map((r, i) => (
                          <li key={i} className="text-green-600 text-xs flex gap-1">
                            <span>+</span>
                            <span>{r}</span>
                          </li>
                        ))}
                    ...
                    <div>
                      ...
                      <ul className="mt-1 space-y-1">
                        {rec.concerns.map((c, i) => (
                          <li key={i} className="text-amber-600 text-xs flex gap-1">
                            <span>!</span>
                            <span>{c}</span>
                          </li>
                        ))}
