...
function runFeasibilityChecks(
  ...
  if (approach === "create" && budget < 25000) {
    ...
    ["copy_improve", "localize", "specialize"].includes(approach || "") &&
    budget < 5000
  ) {
    results.push({
      category: "Budget",
  ...
  if (approach === "create" && runway < 12) {
    ...
    ["copy_improve", "localize"].includes(approach || "") &&
    runway < 3
  ) {
    results.push({
      category: "Runway",
  ...
  if (meta) {
    ...
    if (riskLevel === "low" && approachRisk === "high") {
      ...
      results.push({
        ...
        status: "pass",
        message:
          "Conservative approach with high risk tolerance - room to pivot",
      });
    } else {
      results.push({
        category: "Risk Alignment",
...
export default function AllocationFeasibilityCheck({
  allocation,
  strategy,
  className = "",
}: Props) {
  ...
  return (
    ...
    >
      ...
      {overallStatus !== "pass" && (
        ...
        >
          <p
            className={`text-sm ${overallStatus === "fail" ? "text-red-700" : "text-yellow-700"}`}
          >
            {overallStatus === "fail"
