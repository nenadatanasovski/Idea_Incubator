...
import { Clock, GitCommit, ChevronDown, ChevronUp, Eye, ArrowRightLeft } from 'lucide-react'
...
function VersionItem({ version, isCurrent, isSelected, onSelect, onView }: VersionItemProps) {
  const [expanded, setExpanded] = useState(false)
  const changeType = changeTypeLabels[version.changeType] || { label: version.changeType, color: 'bg-gray-500' }
      ...
      <div className="ml-4">
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className={`text-xs text-white px-2 py-0.5 rounded-full ${changeType.color}`}>
              {changeType.label}
            </span>
          </div>
          <div className="flex items-center gap-2">
        ...
        {expanded && (
          <div className="mt-3 p-3 bg-gray-50 rounded-lg text-sm">
            <div className="text-xs text-gray-500 mb-2">Phase: {version.phase}</div>
      ...
      <div className="mt-4">
        {versions.map((version, index) => (
          <VersionItem
            ...
            onView={onViewVersion ? () => onViewVersion(version.versionNumber) : undefined}
          />
        ))}
      </div>
    </div>
