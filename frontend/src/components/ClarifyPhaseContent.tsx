...
export default function ClarifyPhaseContent({
  ...
  >(new Map());
  const [isFetchingBulkSuggestions, setIsFetchingBulkSuggestions] =
    useState(false);
  const [bulkSuggestionsProgress, setBulkSuggestionsProgress] = useState({
    current: 0,
    total: 0,
  });
  ...
  const handleBulkSuggestions = async () => {
    ...
    for (let i = 0; i < unansweredQuestions.length; i += batchSize) {
      ...
      const promises = batch.map(async (q) => {
        try {
          const res = await fetch(
            `${API_BASE}/api/ideas/${ideaSlug}/questions/${q.id}/suggestions`,
            {
              method: "POST",
              headers: { "Content-Type": "application/json" },
            },
      ...
      setBulkSuggestionsProgress({
        current: Math.min(i + batchSize, unansweredQuestions.length),
        total: unansweredQuestions.length,
      });
    }
  ...
  const unansweredCount = allQuestionsData
    ? Object.values(allQuestionsData.grouped)
        .flat()
  ...
  if (loading) {
    return (
      <div
        className={clsx(
          "card flex items-center justify-center py-12",
          className,
        )}
      >
            ...
            >
              {isFetchingBulkSuggestions ? (
                <>
                  ...
                  <span>
                    Generating suggestions... ({bulkSuggestionsProgress.current}
                    /{bulkSuggestionsProgress.total})
                  </span>
                </>
              ) : (
                <>
                  ...
                  <span>
                    Provide AI Suggestions for all {unansweredCount} open
                    questions
                  </span>
                </>
              )}
                      ...
                      <div className="flex items-center gap-2">
                        <div className="w-20 h-2 bg-gray-200 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${(answered / catQuestions.length) * 100}%`,
                            }}
                          />
                        </div>
                        {isCategoryExpanded ? (
