...
export default function TaskImpactViewer({
  taskId,
  readOnly = false,
  onImpactChange,
}: TaskImpactViewerProps) {
  ...
  const [expandedTypes, setExpandedTypes] = useState<Set<ImpactType>>(
    new Set(["file"]),
  );
  ...
  const handleRemoveImpact = async (impactId: string) => {
    try {
      const response = await fetch(
        `/api/task-agent/tasks/${taskId}/impacts/${impactId}`,
        {
          method: "DELETE",
        },
      );
  ...
  const groupedImpacts = impacts.reduce(
    (acc, impact) => {
          ...
          <div className="space-y-1">
            {conflicts.slice(0, 3).map((c, i) => (
              <p key={i} className="text-sm text-amber-600">
                {c.conflictType} on{" "}
                <code className="bg-amber-100 px-1 rounded">{c.target}</code> (
                {c.severity} severity)
              </p>
            ))}
            {conflicts.length > 3 && (
        ...
        <div className="p-4 border border-gray-200 rounded-lg space-y-3">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setNewImpact({
                    ...newImpact,
                    operation: e.target.value as ImpactOperation,
                  })
                }
                className="w-full px-3 py-2 border rounded-lg"
              >
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setNewImpact({ ...newImpact, target: e.target.value })
              }
                    ...
                    {typeImpacts.map((impact) => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <span
                            className={`px-2 py-0.5 rounded text-xs font-medium ${operationColors[impact.operation]}`}
                          >
                            {impact.operation}
                          </span>
