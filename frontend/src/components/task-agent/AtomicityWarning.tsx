...
export default function AtomicityWarning({
  taskId,
  onDecompose,
  compact = false,
}: AtomicityWarningProps) {
  const [validation, setValidation] = useState<AtomicityValidation | null>(
    null,
  );
  ...
  const validateAtomicity = async () => {
    try {
      ...
      const response = await fetch(
        `/api/task-agent/tasks/${taskId}/validate-atomicity`,
        {
          method: "POST",
        },
      );
      ...
      >
        ...
        {!validation.isAtomic && validation.suggestedSubtasks && (
          <span className="text-amber-600">
            → {validation.suggestedSubtasks} subtasks
          </span>
        )}
      </div>
    );
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-amber-600">
            Score: {validation.score}/100 • {validation.violations.length}{" "}
            issues
          </span>
        </div>
        <div className="flex items-center gap-2">
        ...
        <div className="p-4 space-y-3">
          {validation.suggestedSubtasks && (
            <p className="text-sm text-gray-600">
              ...
              <strong>{validation.suggestedSubtasks} subtasks</strong>
            </p>
          )}
