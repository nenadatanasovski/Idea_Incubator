...
export default function PRDDetail({ prdId, onClose, onApprove }: PRDDetailProps) {
  ...
  const fetchPRDData = async () => {
    try {
      setLoading(true)
      const [prdRes, coverageRes, progressRes, taskListsRes, tasksRes] = await Promise.all([
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          ...
          {coverage.uncoveredRequirements.length > 0 && (
            <div className="mt-4">
              ...
              >
                {showUncovered ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
          ...
          <div className="space-y-2">
            {taskLists.map(list => (
              ...
              >
                ...
                <span className="text-sm text-gray-500">{list.taskCount} tasks</span>
              </div>
            ))}
          </div>
        </div>
