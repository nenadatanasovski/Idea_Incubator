...
import { Check, Lightbulb, Search, Target, RefreshCw, BarChart3, Repeat, ChevronRight, Lock, AlertCircle } from 'lucide-react'
...
export type LegacyIncubationPhase = 'capture' | 'clarify' | 'differentiate' | 'update' | 'evaluate' | 'iterate'

// Map legacy phase names to new ones
export const mapPhase = (phase: string): IncubationPhase => {
  if (phase === 'differentiate' || phase === 'differentiation') return 'position'
...
export const incubationPhases: PhaseConfig[] = [
  {
    ...
    instructions: 'Capture the core concept, problem, and initial solution approach.',
  ...
  },
  {
    ...
    instructions: 'Answer critical questions to develop your idea and identify gaps.',
  ...
  },
  {
    ...
    instructions: 'Set resources, choose strategic approach, and select positioning strategy.',
    completionCriteria: 'Financial allocation, approach, and strategy decisions captured'
  },
  {
    id: 'update',
    label: 'Update',
    ...
    instructions: 'Update your idea based on insights from clarification and positioning.',
  ...
  },
  {
    ...
    instructions: 'Run the AI evaluation to score your idea against 30 criteria.',
...
export default function IncubationStepper({
  ...
  const [expandedPhase, setExpandedPhase] = useState<IncubationPhase | null>(currentPhase)

  const currentIndex = incubationPhases.findIndex(p => p.id === currentPhase)

  const getPhaseStatus = (phase: IncubationPhase, index: number): 'completed' | 'current' | 'locked' | 'upcoming' => {
  ...
  const canNavigateToPhase = (phase: IncubationPhase, index: number): boolean => {
    const status = getPhaseStatus(phase, index)
    return status === 'completed' || status === 'current' || (status === 'upcoming' && canAdvance)
  }
  ...
  return (
    <div className={clsx('card', className)}>
      ...
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-medium text-gray-500">Incubation Journey</h3>
      ...
      <div className="relative mb-6">
        <div className="h-2 bg-gray-200 rounded-full">
          <div
            ...
            style={{ width: `${((currentIndex + (canAdvance ? 1 : 0.5)) / incubationPhases.length) * 100}%` }}
          />
        </div>
      </div>
              ...
              <div key={phase.id} className="relative">
                ...
                <button
                  ...
                  className={clsx(
                    ...
                    status === 'current' && 'bg-primary-50 border-2 border-primary-200',
                ...
                >
                  ...
                  <div
                    className={clsx(
                      ...
                      status === 'current' && `${phase.bgColor} border-white text-white shadow-lg`,
                      status === 'upcoming' && canAdvance && 'bg-white border-gray-300 text-gray-400',
                      status === 'upcoming' && !canAdvance && 'bg-gray-100 border-gray-200 text-gray-300',
                      status === 'locked' && 'bg-gray-100 border-gray-200 text-gray-300'
                    )}
                  >
                    {status === 'completed' ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <span
                        className={clsx(
                          ...
                          (status === 'upcoming' || status === 'locked') && 'text-gray-500'
                        )}
                      >
                        {phase.label}
                      </span>
                    ...
                    {isExpanded && status === 'current' && (
                      <div className="mt-3 pt-3 border-t border-gray-200">
                        <p className="text-sm text-gray-600 mb-2">{phase.instructions}</p>

                        {/* Progress indicator for clarify phase */}
                        {phase.id === 'clarify' && readinessPercent !== undefined && (
                          <div className="mb-3">
                            ...
                            <div className="h-1.5 bg-gray-200 rounded-full">
                              <div
                                className={clsx(
                                  ...
                                  readinessPercent >= 50 ? 'bg-yellow-500' : 'bg-red-400'
                                )}
                        ...
                        {blockingGaps.length > 0 && (
                          <div className="flex items-start gap-2 p-2 bg-amber-50 rounded-lg text-xs">
                            ...
                            <div>
                              <span className="font-medium text-amber-700">Blocking gaps:</span>
                              <span className="text-amber-600 ml-1">
                                {blockingGaps.slice(0, 2).join(', ')}
                                {blockingGaps.length > 2 && ` +${blockingGaps.length - 2} more`}
                              </span>
                            </div>
                          </div>
                        )}
                        ...
                        <p className="text-xs text-gray-400 mt-2">
                          <span className="font-medium">To complete:</span> {phase.completionCriteria}
                        </p>
                      </div>
                    )}
                  </div>
      ...
      <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-200">
        ...
        <button
          ...
          className={clsx(
            'btn',
            canAdvance ? 'btn-primary' : 'btn-secondary opacity-50 cursor-not-allowed'
          )}
        >
          {currentIndex === incubationPhases.length - 1 ? 'Complete' : 'Continue →'}
        </button>
      </div>
    </div>
  )
