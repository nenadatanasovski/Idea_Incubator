import { AlertTriangle, Play, RefreshCw, Pause, XOctagon, ChevronDown, ChevronUp } from 'lucide-react'
import { useState } from 'react'
import clsx from 'clsx'
import { scoreInterpretation } from '../types'
import type { Synthesis } from '../types'
import { useEvaluations, useDebateRounds, usePreviousRunScores } from '../hooks/useEvaluations'
...
export default function EvaluationSummaryCard({
  ...
  const weakCriteria: WeakCriterion[] = evaluations
    ...
    .map(e => {
      ...
      return {
        criterion: e.criterion.replace(/_/g, ' ').replace(/^\w/, c => c.toUpperCase()),
        category: e.category,
  ...
  const recConfig = recommendationConfig[synthesis.recommendation] || recommendationConfig.REFINE
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg shadow-sm">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <div className={clsx(
                ...
                synthesis.overall_score >= 7 ? 'bg-green-100' :
                synthesis.overall_score >= 5 ? 'bg-yellow-100' : 'bg-red-100'
              )}>
              ...
              <div>
                <p className="text-sm font-medium text-gray-900">{scoreLevel}</p>
          ...
          {weakCriteria.length > 0 && (
            ...
            >
              ...
              <span>{weakCriteria.length} weakness{weakCriteria.length !== 1 ? 'es' : ''}</span>
              {expanded ? (
        ...
        <div className="px-4 pb-4 border-t border-gray-100 pt-3">
          ...
          <div className="space-y-2">
            {weakCriteria.map((criterion, idx) => {
              const hasDebateForCriterion = criterion.debateChallenges.length > 0
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-red-900">
                      {criterion.criterion}
                      <span className="text-red-600 font-normal ml-1">({criterion.category})</span>
      ...
      {!expanded && synthesis.key_strengths && synthesis.key_strengths.length > 0 && (
