...
export default function ProfileFitBreakdown({ strategy, profile, className = '' }: Props) {
    ...
    return (
      <div className={`bg-white border border-gray-200 rounded-lg p-4 ${className}`}>
        ...
        <div className="flex items-center gap-4">
          <div className={`px-4 py-3 rounded-lg border ${getFitScoreBackground(fitWithProfile)}`}>
            <span className={`text-3xl font-bold ${getFitScoreColor(fitWithProfile)}`}>
              {fitWithProfile}
            </span>
  ...
  return (
    <div className={`bg-white border border-gray-200 rounded-lg p-4 ${className}`}>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className={`px-3 py-1 rounded-full border ${getFitScoreBackground(score)}`}>
          <span className={`text-lg font-bold ${getFitScoreColor(score)}`}>{score}</span>
          ...
          <div className="pt-3 border-t border-gray-100">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs text-gray-600">
              {profile.risk_tolerance && (
                <div>
                  <span className="text-gray-400">Risk:</span> {profile.risk_tolerance}
                </div>
              )}
              {profile.weekly_hours_available && (
                <div>
                  <span className="text-gray-400">Hours:</span> {profile.weekly_hours_available}/week
                </div>
              )}
              {profile.domain_expertise && (
                <div className="col-span-2">
                  <span className="text-gray-400">Domain:</span> {profile.domain_expertise}
                </div>
              )}
            </div>
          </div>
