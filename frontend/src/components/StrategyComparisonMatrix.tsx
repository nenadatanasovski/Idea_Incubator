...
type FilterType = 'all' | 'quick-win' | 'best-fit' | 'high-upside' | 'lowest-risk';
type SortColumn = 'name' | 'fitWithProfile' | 'timeToValue' | 'riskLevel' | 'resourceNeed';
...
const getRiskLevel = (strategy: EnhancedStrategy): 'low' | 'medium' | 'high' => {
...
const getResourceNeed = (strategy: EnhancedStrategy): 'low' | 'medium' | 'high' => {
  // investmentRequired is an optional extension field
  const stratAny = strategy as any;
  if (stratAny.investmentRequired) {
    const upfront = stratAny.investmentRequired.upfront.mid ||
                    (stratAny.investmentRequired.upfront.low + stratAny.investmentRequired.upfront.high) / 2;
...
const getOpportunityAlignment = (strategy: EnhancedStrategy): 'high' | 'medium' | 'low' => {
...
const getCellColor = (value: 'high' | 'medium' | 'low', isRisk = false): string => {
  if (isRisk) {
...
export default function StrategyComparisonMatrix({
  ...
  const toggleExpand = (strategyId: string) => {
    setExpandedStrategyId(expandedStrategyId === strategyId ? null : strategyId);
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        {(['all', 'quick-win', 'best-fit', 'high-upside', 'lowest-risk'] as FilterType[]).map(
          (f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
                  ...
                  <React.Fragment key={strategyId}>
                    ...
                    >
                      ...
                      <td className="px-4 py-4 whitespace-nowrap text-center">
                        <span className={`text-lg font-bold ${getScoreColor(strategy.fitWithProfile)}`}>
                      ...
                      {/* Opportunity */}
                      <td className="px-4 py-4 whitespace-nowrap text-center">
                        <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getCellColor(opportunity)}`}>
                          {opportunity}
                        </span>
                      </td>
                      ...
                      {/* Risk Level */}
                      <td className="px-4 py-4 whitespace-nowrap text-center">
                        <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getCellColor(risk, true)}`}>
                          {risk}
                        </span>
                      </td>
                      ...
                      {/* Resources */}
                      <td className="px-4 py-4 whitespace-nowrap text-center">
                        <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getCellColor(resource, true)}`}>
                          {'$'.repeat(resource === 'low' ? 1 : resource === 'medium' ? 2 : 3)}
                        </span>
                      </td>
                        ...
                        <td colSpan={7} className="px-4 py-4">
                          <div className="ml-8 grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div className="bg-white rounded-lg border border-gray-200 p-4">
                              <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Revenue Projections</h4>
                              {year1Revenue ? (
                                <div className="space-y-2">
                                  <div className="flex justify-between items-center">
                                    <span className="text-sm text-gray-600">Year 1 (Low)</span>
                                    <span className="text-sm font-medium text-gray-900">{formatCurrency(year1Revenue.low)}</span>
                                  </div>
                                  <div className="flex justify-between items-center">
                                    <span className="text-sm text-gray-600">Year 1 (Mid)</span>
                                    <span className="text-sm font-semibold text-blue-600">{formatCurrency(year1Revenue.mid)}</span>
                                  </div>
                                  <div className="flex justify-between items-center">
                                    <span className="text-sm text-gray-600">Year 1 (High)</span>
                                    <span className="text-sm font-medium text-gray-900">{formatCurrency(year1Revenue.high)}</span>
                                  </div>
                                  {strategy.revenueEstimates?.year3 && (
                                    <div className="pt-2 mt-2 border-t border-gray-100">
                                      <div className="flex justify-between items-center">
                                        <span className="text-sm text-gray-600">Year 3 (Mid)</span>
                                        <span className="text-sm font-medium text-green-600">{formatCurrency(strategy.revenueEstimates.year3.mid)}</span>
                                      </div>
                                    </div>
                                  )}
                                </div>
                              ) : (
                                <p className="text-sm text-gray-500">No revenue projections available</p>
                              )}
                            </div>

                            {/* Goal Alignment */}
                            <div className="bg-white rounded-lg border border-gray-200 p-4">
                              <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Goal Alignment</h4>
                                  ...
                                  <div className="flex items-center gap-2">
                                    {meetsGoal ? (
                                      ...
                                      <span className="inline-flex items-center gap-1 px-2 py-1 bg-yellow-100 text-yellow-700 text-xs font-medium rounded-full">
                                        ...
                                        Gap: {formatCurrency(strategy.goalAlignment.gapToTarget || 0)}
                                      </span>
                                    )}
                                  </div>
                                  {allocation?.allocatedRunwayMonths && strategy.goalAlignment.runwaySufficient !== undefined && (
                                    <div className="text-sm">
                                      <span className="text-gray-600">Runway: </span>
                                      <span className={strategy.goalAlignment.runwaySufficient ? 'text-green-600' : 'text-red-600'}>
                                        {strategy.goalAlignment.runwaySufficient ? 'Sufficient' : 'May need extension'}
                                      </span>
                                    </div>
                                  )}
                                  <div className="text-sm">
                                    <span className="text-gray-600">Investment: </span>
                                    <span className={strategy.goalAlignment.investmentFeasible ? 'text-green-600' : 'text-red-600'}>
                                      {strategy.goalAlignment.investmentFeasible ? 'Feasible' : 'May exceed budget'}
                                    </span>
                                  </div>
                                </div>
                              ) : (
                                <p className="text-sm text-gray-500">Goal alignment data not available</p>
                              )}
                            </div>

                            {/* Key Actions */}
                            <div className="bg-white rounded-lg border border-gray-200 p-4">
                              <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Key Differentiators</h4>
                              {strategy.differentiators && strategy.differentiators.length > 0 ? (
                                <ul className="space-y-1.5">
                                  {strategy.differentiators.slice(0, 3).map((diff, i) => (
                                  ...
                                  {strategy.differentiators.length > 3 && (
                                    <li className="text-xs text-gray-400">+{strategy.differentiators.length - 3} more</li>
                                  )}
                                </ul>
                              ) : (
                                <p className="text-sm text-gray-500">No differentiators listed</p>
                              )}
                            </div>
                          </div>
