...
function calculateConnections(props: Props): ConnectionStrength[] {
  ...
  const { approach, allocation, profile, opportunities, risks, strategy } = props;
  ...
  if (approach && risks && risks.length > 0) {
    ...
    connections.push({
      from: 'approach',
      to: 'risks',
      strength: highRisks >= 2 ? 'weak' : highRisks === 1 ? 'moderate' : 'strong',
...
const nodeStyles = {
  profile: { bg: 'bg-blue-100', border: 'border-blue-300', text: 'text-blue-800', icon: '👤' },
  allocation: { bg: 'bg-green-100', border: 'border-green-300', text: 'text-green-800', icon: '💰' },
  approach: { bg: 'bg-purple-100', border: 'border-purple-300', text: 'text-purple-800', icon: '🎯' },
  opportunities: { bg: 'bg-amber-100', border: 'border-amber-300', text: 'text-amber-800', icon: '💡' },
  risks: { bg: 'bg-red-100', border: 'border-red-300', text: 'text-red-800', icon: '⚠️' },
  strategy: { bg: 'bg-indigo-100', border: 'border-indigo-300', text: 'text-indigo-800', icon: '🎲' },
};
...
export default function RelationshipDiagram({
  ...
  const nodes: NodeProps[] = [
    { id: 'profile', label: 'Your Profile', sublabel: profile?.name, x: 80, y: 50, active: !!profile },
    { id: 'allocation', label: 'Allocation', sublabel: allocation ? `${allocation.allocatedRunwayMonths}mo runway` : undefined, x: 80, y: 150, active: !!allocation },
    { id: 'approach', label: 'Approach', sublabel: approach ? strategicApproachMeta[approach].label : undefined, x: 220, y: 100, active: hasApproach },
    { id: 'opportunities', label: 'Opportunities', sublabel: hasOpportunities ? `${opportunities!.length} identified` : undefined, x: 360, y: 50, active: Boolean(hasOpportunities) },
    { id: 'risks', label: 'Risks', sublabel: hasRisks ? `${risks!.length} identified` : undefined, x: 360, y: 150, active: Boolean(hasRisks) },
    { id: 'strategy', label: 'Strategy', sublabel: hasStrategy ? 'Defined' : undefined, x: 500, y: 100, active: hasStrategy },
  ];

  // Connection lines
  const lines: { x1: number; y1: number; x2: number; y2: number; strength: 'strong' | 'moderate' | 'weak'; label?: string }[] = [];
  ...
  return (
    <div className={`bg-white rounded-lg border border-gray-200 p-6 ${className}`}>
      ...
      <p className="text-sm text-gray-600 mb-6">
        How your profile, resources, and chosen approach connect to opportunities and risks.
      </p>
      ...
      <div className="mt-6 flex flex-wrap gap-6 text-xs text-gray-600">
        ...
        <div className="flex items-center gap-2">
          <div className="w-8 h-0.5 bg-red-500" style={{ backgroundImage: 'repeating-linear-gradient(90deg, #ef4444, #ef4444 4px, transparent 4px, transparent 8px)' }} />
      ...
      {hasApproach && (
        <div className="mt-6 p-4 bg-gray-50 rounded-lg">
          <h4 className="text-sm font-medium text-gray-800 mb-2">Connection Summary</h4>
          <ul className="space-y-1 text-sm text-gray-600">
            {connections.map((conn, idx) => (
              <li key={idx} className="flex items-center gap-2">
                ...
                <span className="capitalize">{conn.from.replace('_', ' ')}</span>
