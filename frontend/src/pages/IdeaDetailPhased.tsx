...
import IncubationStepper, {
  type IncubationPhase,
  incubationPhases,
} from "../components/IncubationStepper";
import PhaseContainer, {
  CapturePhaseContent,
  EvaluatePhaseHeaderContent,
} from "../components/PhaseContainer";
...
import ViabilityAdvisoryModal, {
  type ViabilityDecision,
} from "../components/ViabilityAdvisoryModal";
...
export default function IdeaDetailPhased() {
  ...
  // Development state
  const { readiness } = useDevelopment(slug);
  ...
  const mapDbPhaseToUi = (dbPhase: string | undefined): IncubationPhase => {
    ...
    const validPhases: IncubationPhase[] = [
      "capture",
      "clarify",
      "position",
      "update",
  ...
  const [optimisticPhase, setOptimisticPhase] =
    useState<IncubationPhase | null>(null);
  ...
  useEffect(() => {
    ...
    const loadLifecycleData = async () => {
      try {
        const [versionsData, lineageData, iterationsData, debateRoundsData] =
          await Promise.all([
  ...
  const handleEvaluateWithSettings = async (settings: {
    budget: number;
    unlimited: boolean;
    debateRounds: number;
  }) => {
  ...
  const iterationNumber =
    (idea as { iteration_number?: number })?.iteration_number || 1;
  ...
  const viabilityRecommendation: ViabilityDecision =
    criticalGaps.length > 0
      ? "research_more"
      : (readiness?.overall ?? 0) < 0.8
        ? "research_more"
        : "proceed";

  // Derive evaluation recommendation
  const evaluationRecommendation: EvaluationDecision =
    synthesis?.recommendation === "PURSUE"
      ? "pursue"
      : synthesis?.recommendation === "REFINE"
        ? "iterate"
        : synthesis?.recommendation === "PAUSE"
          ? "pause"
          : "iterate";
    ...
    <div className="space-y-4">
        ...
        <div className="lg:col-span-3">
            ...
            {currentPhase === "iterate" && (
              <div className="space-y-6">
                <IterationHeader
                  ...
                  currentScore={idea.avg_final_score ?? undefined}
                  focusAreas={
                    latestIteration?.focusCategories ||
                    weakCriteria.map((c) => c.category)
                  }
                  weakCategories={weakCriteria.map((c) => ({
                    category: c.category,
                    score: c.finalScore,
                ...
                {positioningDecision?.riskResponses &&
          ...
          {secondaryView === "history" && (
            <div className="card mt-6">
              ...
              {lineage && <LineageTree lineage={lineage} currentSlug={slug!} />}
      ...
      <EvaluationAdvisoryModal
        ...
        recommendationReasoning={
          ...
          "Consider your options based on the evaluation results."
        }
        onDecision={handleEvaluationDecision}
