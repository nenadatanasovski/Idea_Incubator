...
test.describe('Ideation Session', () => {
  ...
  test('displays message history correctly', async ({ page }) => {
    ...
    await expect(page.getByText('I want to build a todo app')).toBeVisible({ timeout: 10000 });
  ...
  test('differentiates user and assistant messages', async ({ page }) => {
    await page.route('**/api/ideation/sessions/*/messages', async (route) => {
      await route.fulfill({
        ...
        contentType: 'application/json',
        body: JSON.stringify([
          { id: 'msg-1', role: 'user', content: 'User message', created_at: new Date().toISOString() },
          { id: 'msg-2', role: 'assistant', content: 'Assistant message', created_at: new Date().toISOString() },
        ]),
      });
    });
    ...
    const userMsg = page.locator('[class*="primary"], [class*="user"]').filter({ hasText: 'User message' });
    const assistantMsg = page.locator('[class*="gray"], [class*="assistant"]').filter({ hasText: 'Assistant message' });
...
test.describe('Chat Interactions', () => {
  ...
  test('sends message on Enter key', async ({ page }) => {
    ...
    await expect(page.getByText('My test message')).toBeVisible({ timeout: 5000 });
  });
  ...
  test('sends message on button click', async ({ page }) => {
    ...
    await expect(page.getByText('Button test message')).toBeVisible({ timeout: 5000 });
  });
  ...
  test('shows loading indicator during AI response', async ({ page }) => {
    ...
    const loadingIndicator = page.locator('.animate-spin, [data-testid="streaming-indicator"]').first();
  ...
  test('displays AI response after sending message', async ({ page }) => {
    ...
    await expect(page.getByText(/you mentioned/i)).toBeVisible({ timeout: 10000 });
  });
  ...
  test('shows agent activity indicator', async ({ page }) => {
    await page.route('**/api/ideation/sessions/*/message', async (route) => {
      ...
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ messageId: 'activity-msg', content: 'Done processing' }),
      });
    });
    ...
    const activityText = page.getByText(/processing|thinking|analyzing/i).first();
  ...
  test('input disabled while waiting for response', async ({ page }) => {
    await page.route('**/api/ideation/sessions/*/message', async (route) => {
      ...
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ messageId: 'disabled-msg', content: 'Response' }),
      });
    });
...
test.describe('Start New Session', () => {
  test('shows start session button when no session exists', async ({ page }) => {
  ...
  test('clicking start session creates new session', async ({ page }) => {
    ...
    const chatInput = page.locator('textarea[name="message"], textarea').first();
...
test.describe('Error Handling', () => {
  test('handles message send failure gracefully', async ({ page, setupMocks }) => {
    await setupMocks(page);
