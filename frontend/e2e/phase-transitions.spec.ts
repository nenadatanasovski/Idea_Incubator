...
import { waitForNetworkIdle, waitForLoadingComplete, checkPhase } from './utils';
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          state: {
            ...
            autoAdvance: true,
            specProgress: { sectionsComplete: 3, sectionsTotal: 5, generatedTasks: 0 },
          },
        }),
      });
    });
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          state: {
            ...
            autoAdvance: true,
            buildProgress: { tasksComplete: 2, tasksTotal: 5, currentTask: 'Create UI' },
          },
        }),
      });
    });
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          state: {
            ideaId: 'test-idea-123',
            currentPhase: 'building',
            buildProgress: { tasksComplete: 1, tasksTotal: 3, currentTask: 'Setup' },
          },
        }),
      });
    });
...
test.describe('Phase Updates via Polling', () => {
  test('updates phase indicator when phase changes', async ({ page }) => {
    ...
    await expect(page.getByText(/ideation/i).first()).toBeVisible({ timeout: 10000 });
...
test.describe('Phase Styling', () => {
  test('ideation phase has correct color scheme', async ({ page }) => {
    ...
    const phaseIndicator = page.locator('[class*="blue"], [class*="primary"]').filter({
      hasText: /ideation/i
    });
