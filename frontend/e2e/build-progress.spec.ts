...
import { waitForNetworkIdle, waitForLoadingComplete, clickButton } from './utils';
      ...
      body: JSON.stringify({
        state: {
          ...
          buildProgress: buildSession ? {
            ...
            currentTask: buildSession.tasks.find(t => t.status === 'in_progress')?.name || null,
          } : null,
        },
      }),
    });
...
test.describe('Build Progress View - Active Build', () => {
  test('displays task list with correct statuses', async ({ page }) => {
    const buildSession = {
      ...
      tasks: [
        { id: 't1', name: 'Setup project', description: 'Init', status: 'completed' as const },
        { id: 't2', name: 'Create components', description: 'Build UI', status: 'in_progress' as const },
        { id: 't3', name: 'Add tests', description: 'Write tests', status: 'pending' as const },
      ],
    ...
    await expect(page.getByText(/building/i).first()).toBeVisible({ timeout: 10000 });
  ...
  test('shows progress bar', async ({ page }) => {
    ...
    const progressBar = page.locator('[role="progressbar"], [class*="progress"], .progress-bar').first();
  ...
  test('highlights current task', async ({ page }) => {
    const buildSession = {
      ...
      status: 'active' as const,
      tasks: [
        { id: 't1', name: 'Done task', description: 'Complete', status: 'completed' as const },
        { id: 't2', name: 'Current task', description: 'In progress', status: 'in_progress' as const },
        { id: 't3', name: 'Future task', description: 'Pending', status: 'pending' as const },
      ],
    };
...
test.describe('Build Progress View - Human Intervention', () => {
  test('shows intervention banner when human needed', async ({ page }) => {
    const buildSession = {
      ...
      tasks: [
        { id: 't1', name: 'Setup', description: 'Done', status: 'completed' as const },
        { id: 't2', name: 'Database setup', description: 'Failed', status: 'failed' as const },
        { id: 't3', name: 'API routes', description: 'Pending', status: 'pending' as const },
      ],
    };
  ...
  test('skip button advances past stuck task', async ({ page }) => {
    const buildSession = {
      ...
      status: 'human_needed' as const,
      tasks: [
        { id: 't1', name: 'Done', description: 'Complete', status: 'completed' as const },
        { id: 't2', name: 'Stuck', description: 'Failed', status: 'failed' as const },
        { id: 't3', name: 'Next', description: 'Pending', status: 'pending' as const },
      ],
    };
...
test.describe('Build Progress View - Complete State', () => {
  test('shows completion banner when build is done', async ({ page }) => {
    const buildSession = {
      ...
      tasks: [
        { id: 't1', name: 'Task 1', description: 'Done', status: 'completed' as const },
        { id: 't2', name: 'Task 2', description: 'Done', status: 'completed' as const },
        { id: 't3', name: 'Task 3', description: 'Done', status: 'completed' as const },
      ],
...
test.describe('Build Progress View - Failed State', () => {
  test('shows failed tasks with error indication', async ({ page }) => {
    const buildSession = {
      ...
      tasks: [
        { id: 't1', name: 'Success', description: 'Done', status: 'completed' as const },
        { id: 't2', name: 'Failed', description: 'Error', status: 'failed' as const },
      ],
...
test.describe('Build Progress Visual Regression', () => {
  ...
  test('complete build snapshot', async ({ page }) => {
    const buildSession = {
      ...DEFAULT_BUILD_SESSION,
      status: 'complete' as const,
      tasks: DEFAULT_BUILD_SESSION.tasks.map(t => ({ ...t, status: 'completed' as const })),
